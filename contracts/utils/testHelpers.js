...
class TestHelper {
  ...
  /*
   * given the requested Usdx amomunt in openTrove, returns the total debt
   * So, it adds the gas compensation and the borrowing fee
   */
  static async getOpenTroveTotalDebt(contracts, usdxAmount) {
    return (await this.getCompositeDebt(contracts, usdxAmount));
  }

  /*
   * given the desired total debt, returns the Usdx amount that needs to be requested in openTrove
   * So, it subtracts the gas compensation and then the borrowing fee
   */
  static async getOpenTroveUsdxAmount(contracts, totalDebt) {
  ...
  // TODO: remove this, since we won't use the old borrow fee scheme
  static async getAmountWithBorrowingFee(contracts, usdxAmount) {
    return usdxAmount;
  }
  ...
  static getEmittedRedemptionValues(redemptionTx) {
    for (let i = 0; i < redemptionTx.logs.length; i++) {
      if (redemptionTx.logs[i].event === "Redemption") {
        const UsdxAmount = redemptionTx.logs[i].args[0];
        const totalUsdxRedeemed = redemptionTx.logs[i].args[1];
        const totalCollDrawn = redemptionTx.logs[i].args[2];
        const collFee = redemptionTx.logs[i].args[3];

        return [UsdxAmount, totalUsdxRedeemed, totalCollDrawn, collFee];
      }
    }
  ...
  static getEmittedLiquidationValues(liquidationTx) {
    for (let i = 0; i < liquidationTx.logs.length; i++) {
      if (liquidationTx.logs[i].event === "Liquidation") {
        ...
        const usdxGasComp = liquidationTx.logs[i].args._usdxGasCompensation;

        const liquidatedColl = collSentToSP.add(collRedistributed);
        const liquidatedDebt = debtOffsetBySP.add(debtRedistributed);

        return [liquidatedDebt, liquidatedColl, collGasComp, usdxGasComp];
      }
    }
  ...
  static getUsdxFeeFromUsdxBorrowingEvent(tx) {
    for (let i = 0; i < tx.logs.length; i++) {
      if (tx.logs[i].event === "UsdxBorrowingFeePaid") {
        return tx.logs[i].args[1].toString();
      }
    }
    throw "The transaction logs do not contain an UsdxBorrowingFeePaid event";
  }
  ...
  static async getEntireCollAndDebt(contracts, troveId) {
    ...
    const pendingUsdxDebtReward = await contracts.troveManager.getPendingUsdxDebtReward(troveId);
    const entireColl = rawColl.add(pendingCollReward);
    const entireDebt = rawDebt.add(pendingUsdxDebtReward);
  ...
  static async getCollAndDebtFromWithdrawUsdx(contracts, account, amount) {
  ...
  static async getCollAndDebtFromRepayUsdx(contracts, account, amount) {
    const { entireColl, entireDebt } = await this.getEntireCollAndDebt(
      contracts,
      account,
    );
  ...
  static async getCollAndDebtFromAdjustment(
    ...
    const fee = UsdxChange.gt(this.toBN("0"))
      ? await contracts.troveManager.getBorrowingFee(UsdxChange)
      : this.toBN("0");
    const newColl = entireColl.add(collChange);
    const newDebt = entireDebt.add(UsdxChange).add(fee);
  ...
  static async openTrove_allAccounts(
    ...
    const gasCostList = [];
    const totalDebt = await this.getOpenTroveTotalDebt(contracts, UsdxAmount);
  ...
  static async openTrove_allAccounts_randomETH_ProportionalUsdx(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randCollAmount = this.randAmountInWei(minETH, maxETH);
      const proportionalUsdx = web3.utils
        .toBN(proportion)
  ...
  static async openTrove_allAccounts_randomETH_randomUsdx(
    minETH,
    maxETH,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      ...
      const randUsdxProportion = this.randAmountInWei(
        minUsdxProportion,
        maxUsdxProportion,
      );
      const proportionalUsdx = web3.utils
        .toBN(randUsdxProportion)
      ...
      if (logging && tx.receipt.status) {
        i++;
        const ICR = await contracts.troveManager.getCurrentICR(account, price);
        // console.log(`${i}. Trove opened. addr: ${this.squeezeAddr(account)} coll: ${randCollAmount} debt: ${proportionalUsdx} ICR: ${ICR}`)
      }
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
    }
  ...
  static async openTrove_allAccounts_randomUsdx(
    minUsdx,
    maxUsdx,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randUsdxAmount = this.randAmountInWei(minUsdx, maxUsdx);
      const totalDebt = await this.getOpenTroveTotalDebt(
        contracts,
        randUsdxAmount,
      );
  ...
  static async openTrove(
    ...
    if (!extraUsdxAmount) extraUsdxAmount = this.toBN(0);
    else if (typeof extraUsdxAmount == "string") {
      extraUsdxAmount = this.toBN(extraUsdxAmount);
    }
  ...
  static async withdrawUsdx(
    contracts,
    { troveId, usdxAmount, ICR, extraParams },
  ) {
    if (!troveId) troveId = this.addressToTroveId(extraParams.from);

    assert(
      !(usdxAmount && ICR) && (usdxAmount || ICR),
      "Specify either usdx amount or target ICR, but not both",
    );
    ...
    if (ICR) {
      ...
      usdxAmount = await this.getNetBorrowingAmount(
        contracts,
        increasedTotalDebt,
      );
    } else {
    ...
    await contracts.borrowerOperations.withdrawUsdx(
      troveId,
      usdxAmount,
      TestHelper.MAX_UINT256,
      extraParams,
  ...
  static async adjustTrove_allAccounts(
    ...
    for (const account of accounts) {
      ...
      let UsdxChangeBN = this.toBN(UsdxAmount);
      ...
      let isDebtIncrease = UsdxChangeBN.gt(zero);
      UsdxChangeBN = UsdxChangeBN.abs();
  ...
  static async adjustTrove_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      ...
      let UsdxChangeBN = this.toBN(this.randAmountInWei(UsdxMin, UsdxMax));
      ...
      const gas = this.gasUsed(tx);
      // console.log(`ETH change: ${collChangeBN},  UsdxChange: ${UsdxChangeBN}, gas: ${gas} `)
  ...
  static async withdrawUsdx_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromWithdrawUsdx(
        contracts,
        account,
        amount,
      );
      ...
      const tx = await contracts.borrowerOperations.withdrawUsdx(
        this._100pct,
        amount,
        upperHint,
        lowerHint,
  ...
  static async withdrawUsdx_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts,
    ...
    for (const account of accounts) {
      const randUsdxAmount = this.randAmountInWei(min, max);
  ...
  static async repayUsdx_allAccounts(accounts, contracts, amount) {
    const gasCostList = [];

    for (const account of accounts) {
      const { newColl, newDebt } = await this.getCollAndDebtFromRepayUsdx(
        contracts,
        account,
        amount,
      );
      ...
      const tx = await contracts.borrowerOperations.repayUsdx(
        amount,
        upperHint,
        lowerHint,
        { from: account },
  ...
  static async repayUsdx_allAccounts_randomAmount(
    min,
    max,
    accounts,
    contracts,
  ...
  static async provideToSP_allAccounts_randomAmount(
    ...
    const gasCostList = [];
    for (const account of accounts) {
      const randomUsdxAmount = this.randAmountInWei(min, max);
  ...
  static async withdrawFromSP_allAccounts_randomAmount(
    ...
    for (const account of accounts) {
      const randomUsdxAmount = this.randAmountInWei(min, max);
      const tx = await stabilityPool.withdrawFromSP(randomUsdxAmount, {
        from: account,
      });
      const gas = this.gasUsed(tx);
      gasCostList.push(gas);
  ...
  // --- ACCEL & Lockup Contract functions ---
  ...
  static async getTimeFromSystemDeployment(
    ...
    const deploymentTime = await accelToken.getDeploymentStartTime();
