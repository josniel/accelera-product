...
`;
  ...
  if ("CI" in process.env) {
    echo("Workaround: deleting variable 'CI' from environment"); // See https://github.com/liquity/usdx/pull/113
  ...
  if (options.mode === "usdx-only") {
    echo("UsdxToken address:", deploymentManifest.usdxToken);
    return;
  }

  const protocolContracts = {
    UsdxToken: deploymentManifest.usdxToken,
