...
const UsdxToken = artifacts.require("./UsdxTokenTester.sol");
...
class DeploymentHelper {
  ...
  static async deployLiquityCoreHardhat(mocks = {
    ...
    const usdxToken = await Contracts.UsdxToken.new();

    const collateralRegistry = await Contracts.CollateralRegistry.new(usdxToken.address, [WETH.address]);
    ...
    Contracts.UsdxToken.setAsDeployed(usdxToken);
  ...
  static async connectCoreContracts(contracts) {
    await contracts.usdxToken.setBranchAddresses(
    ...
    await contracts.usdxToken.setCollateralRegistry(
