...
contract InvariantsTest is Assertions, Logging, BaseInvariantTest, BaseMultiCollateralTest {
    ...
    function setUp() public override {
        ...
        (contracts.branches, contracts.collateralRegistry, contracts.usdxToken, contracts.hintHelpers,, contracts.weth,)
    ...
    function invariant_FundsAreSwept() external view {
        for (uint256 i = 0; i < actors.length; ++i) {
            ...
            assertEqDecimal(usdxToken.balanceOf(actor), 0, 18, "Incomplete USDX sweep");
    ...
    function invariant_SystemStateMatchesGhostState() external view {
        for (uint256 i = 0; i < branches.length; ++i) {
            ...
            assertApproxEqAbsDecimal(
                c.stabilityPool.getTotalUsdxDeposits(),
                handler.spUsdxDeposits(i),
                100,
                18,
    ...
    function invariant_AllUsdxBackedByTroveDebt() external view {
        uint256 totalUsdx = usdxToken.totalSupply();
        ...
        assertApproxEqAbsDecimal(
            totalUsdx + totalPendingInterest + totalPendingBatchManagementFees,
            totalDebt,
            1e-10 ether,
            18,
            "Total Usdx !~= total debt"
        );
    }
    ...
    function invariant_StabilityPool_AllUsdxClaimable_ExceptYieldReceivedWhileEmpty() external view {
        for (uint256 j = 0; j < branches.length; ++j) {
            IStabilityPool stabilityPool = branches[j].stabilityPool;

            uint256 sumUsdxDeposit = 0;
            uint256 sumYieldGain = 0;
            uint256 yieldGainsPending = stabilityPool.getYieldGainsPending();

            for (uint256 i = 0; i < actors.length; ++i) {
                sumUsdxDeposit += stabilityPool.getCompoundedUsdxDeposit(actors[i].account);
                sumYieldGain += stabilityPool.getDepositorYieldGain(actors[i].account);
            }

            assertLt(
                stabilityPool.getTotalUsdxDeposits() - sumUsdxDeposit, 1000, "totalUsdxDeposits !~= sum(usdxDeposit)"
            );

            assertLt(stabilityPool.getYieldGainsOwed() - sumYieldGain, 1000, "yieldGainsOwed !~= sum(yieldGain)");

            // This only holds as long as no one sends USDX directly to the SP's address other than ActivePool
            assertLt(
                usdxToken.balanceOf(address(stabilityPool)) - sumUsdxDeposit - sumYieldGain - yieldGainsPending,
                1000,
                "SP USDX balance !~= claimable + pending"
            );
        }
    }
