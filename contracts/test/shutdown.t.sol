...
contract ShutdownTest is DevTestSetup {
    ...
    function testCannotWithdrawUsdxAfterShutdown() public {
        uint256 troveId = prepareAndShutdownFirstBranch();

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.IsShutDown.selector);
        borrowerOperations.withdrawUsdx(troveId, 1, 10000e18);
        vm.stopPrank();
    }

    function testCannotRepayUsdxAfterShutdown() public {
        uint256 troveId = prepareAndShutdownFirstBranch();

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.IsShutDown.selector);
        borrowerOperations.repayUsdx(troveId, 1000e18);
        vm.stopPrank();
    }
    ...
    function testCannotAdjustZombieTroveAfterShutdown() public {
        ...
        //deal(address(usdxToken), B, 20000e18);
        vm.startPrank(B);
    ...
    function testCanCloseTroveAfterShutdown() public {
        ...
        deal(address(usdxToken), A, 20000e18);
        vm.startPrank(A);
    ...
    function testCannotUrgentRedeemWithoutEnoughBalance() public {
        ...
        // A sends 900 Usdx to B
        vm.startPrank(A);
        usdxToken.transfer(B, 900e18);
        vm.stopPrank();

        // B tries to redeem 1000 Usdx
        vm.startPrank(B);
        vm.expectRevert(TroveManager.NotEnoughUsdxBalance.selector);
    ...
    function testCanUrgentReedemPartiallyBelow100() external {
        ...
        assertEq(usdxToken.balanceOf(A), usdxBalanceBefore - redemptionAmount, "Usdx balance mismatch");
        assertEq(
    ...
    function testShutdownZerosMintsPendingAggInterestToSP() public {
        ...
        uint256 spBal1 = usdxToken.balanceOf(address(contractsArray[0].stabilityPool));
        ...
        uint256 spBal2 = usdxToken.balanceOf(address(contractsArray[0].stabilityPool));
    ...
    function testUrgentRedemptionMintsNoInterestToSP() public {
        ...
        uint256 spUsdxBal_t0 = usdxToken.balanceOf(address(contractsArray[0].stabilityPool));
        ...
        uint256 spUsdxBal_t1 = usdxToken.balanceOf(address(contractsArray[0].stabilityPool));

        // Check SP usdx bal didnt change from urgent redemption
        assertEq(spUsdxBal_t1, spUsdxBal_t0);
    }
