...
contract TroveEventsTest is EventsTest, ITroveEvents {
    ...
    function test_CloseTroveEmitsTroveUpdated() external {
        ...
        uint256 debt = troveManager.getTroveEntireDebt(troveId);
        uint256 balance = usdxToken.balanceOf(A);
        assertGe(debt, balance, "expected debt >= balance");

        // B helps A close their Trove
        vm.prank(B);
        usdxToken.transfer(A, debt - balance);
    ...
    function test_BatchLiquidateTrovesEmitsLiquidation() external {
        ...
        uint256 usdxInSP = 25_000 ether;
        makeSPDepositNoClaim(B, usdxInSP);
        ...
        for (uint256 i = 0; i < liquidatedDebt.length; ++i) {
            ...
            LiquidationParams memory l;
            l.debtOffsetBySP = Math.min(liquidatedDebt[i], usdxInSP - t.debtOffsetBySP - 1e18);
    ...
    function test_RedeemCollateralEmitsTroveUpdatedForEachRedeemedTrove() external {
        ...
        uint256[3] memory redeemUsdx;
        redeemUsdx[0] = troveManager.getTroveEntireDebt(troveId[0]);
        redeemUsdx[1] = troveManager.getTroveEntireDebt(troveId[1]);
        redeemUsdx[2] = troveManager.getTroveEntireDebt(troveId[2]) / 2;

        uint256 price = priceFeed.getPrice();
        uint256 totalRedeemedUsdx = redeemUsdx[0] + redeemUsdx[1] + redeemUsdx[2];
        uint256 redemptionRate = collateralRegistry.getRedemptionRateForRedeemedAmount(totalRedeemedUsdx);

        vm.recordLogs();
        for (uint256 i = 0; i < 3; ++i) {
            uint256 redeemEth = redeemUsdx[i] * DECIMAL_PRECISION / price;
            uint256 redeemFee = redeemEth * redemptionRate / DECIMAL_PRECISION;
            uint256 debt = troveManager.getTroveEntireDebt(troveId[i]) - redeemUsdx[i];
        ...
        vm.prank(A);
        collateralRegistry.redeemCollateral(totalRedeemedUsdx, 3, _100pct);
...
struct Deposit {
    uint256 recordedUsdx;
    uint256 stashedColl;
    uint256 pendingUsdxLoss;
    uint256 pendingCollGain;
    uint256 pendingUsdxYieldGain;
}
...
contract StabilityPoolEventsTest is EventsTest, IStabilityPoolEvents {
        ...
        returns (Deposit memory deposit, StabilityPoolRewardsState memory current)
    {
        ...
        deposit.recordedUsdx = stabilityPool.getCompoundedUsdxDeposit(A);
        assertGt(deposit.recordedUsdx, 0, "Recorded USDX deposit should be > 0");
        ...
        deposit.pendingUsdxLoss = deposit.recordedUsdx - stabilityPool.getCompoundedUsdxDeposit(A);
        assertGt(deposit.pendingUsdxLoss, 0, "Pending USDX loss should be > 0");
        ...
        deposit.pendingUsdxYieldGain = stabilityPool.getDepositorYieldGain(A);
        assertGt(deposit.pendingUsdxYieldGain, 0, "Pending USDX yield gain should be > 0");
    }
    ...
    function test_ProvideToSPNoClaimEmitsDepositUpdated() external {
        ...
        emit DepositUpdated(
            A,
            deposit.recordedUsdx - deposit.pendingUsdxLoss + deposit.pendingUsdxYieldGain + topUp,
    ...
    function test_ProvideToSPAndClaimEmitsDepositUpdated() external {
        ...
        vm.expectEmit();
        emit DepositUpdated(
            A, deposit.recordedUsdx - deposit.pendingUsdxLoss + topUp, 0, current.P, current.S, current.B, current.scale
        );
    ...
    function test_ProvideToSPNoClaimEmitsDepositOperation() external {
        ...
        emit DepositOperation(
            ...
            deposit.pendingUsdxLoss,
            int256(topUp),
            deposit.pendingUsdxYieldGain,
            0, // _yieldGainClaimed
    ...
    function test_WithdrawFromSPNoClaimEmitsDepositUpdated() external {
        ...
        uint256 withdrawal = deposit.recordedUsdx / 2;

        vm.expectEmit();
        emit DepositUpdated(
            A,
            deposit.recordedUsdx - deposit.pendingUsdxLoss + deposit.pendingUsdxYieldGain - withdrawal,
            deposit.stashedColl + deposit.pendingCollGain,
            current.P,
            current.S,
            current.B,
    ...
    function test_WithdrawFromSPAndClaimEmitsDepositUpdated() external {
        ...
        emit DepositUpdated(
            A,
            deposit.recordedUsdx - deposit.pendingUsdxLoss - withdrawal,
            0,
            current.P,
            current.S,
            current.B,
    ...
    function test_ClaimAllCollGainsEmitsDepositUpdated() external {
        ...
        vm.expectEmit();
        emit DepositUpdated(A, deposit.pendingUsdxYieldGain, stashedColl, curr.P, curr.S, curr.B, curr.scale);
        makeSPWithdrawalNoClaim(A, deposit.recordedUsdx - deposit.pendingUsdxLoss); // can't withdraw pending yield

        vm.expectEmit();
        emit DepositUpdated(A, 0, stashedColl, 0, 0, 0, 0);
        makeSPWithdrawalNoClaim(A, deposit.pendingUsdxYieldGain); // now we can withdraw previously pending yield
