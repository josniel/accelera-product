...
contract Redemptions is DevTestSetup {
    ...
    function testRedemptionIncludesAccruedTroveInterest() public {
        ...
        assertEq(trove_A.redistUsdxDebtGain, 0);
    ...
    function testRedemption1TroveLeavesCollFeeInTrove() public {
        ...
        // Expect Trove's coll reduced by the Coll corresponding to the USDX redeemed (less the Coll fee)
    ...
    function testRedemption3TroveLeavesCollFeesInTroves() public {
        ...
        UsdxRedeemAmounts memory usdxRedeemAmounts;
        CorrespondingColl memory correspondingColl;

        usdxRedeemAmounts.A = troveManager.getTroveDebt(troveIDs.A);
        usdxRedeemAmounts.B = troveManager.getTroveDebt(troveIDs.B);
        usdxRedeemAmounts.C = troveManager.getTroveDebt(troveIDs.C) / 2;
        uint256 totalUsdxRedeemAmount = usdxRedeemAmounts.A + usdxRedeemAmounts.B + usdxRedeemAmounts.C;

        correspondingColl.A = usdxRedeemAmounts.A * DECIMAL_PRECISION / price;
        correspondingColl.B = usdxRedeemAmounts.B * DECIMAL_PRECISION / price;
        correspondingColl.C = usdxRedeemAmounts.C * DECIMAL_PRECISION / price;

        uint256 redemptionFeePct = collateralRegistry.getEffectiveRedemptionFeeInUsdx(totalUsdxRedeemAmount)
            * DECIMAL_PRECISION / totalUsdxRedeemAmount;
        ...
        // Expect each Trove's coll to reduce by the Coll corresponding to the usdx redeemed, less the Coll fee
        ...
        redeem(E, totalUsdxRedeemAmount);
    ...
    function testRedemption3TroveLeavesCollFeesInActivePool() public {
        ...
        uint256 totalUsdxRedeemAmount = usdxRedeemAmounts.A + usdxRedeemAmounts.B + usdxRedeemAmounts.C;
        uint256 totalCorrespondingColl = totalUsdxRedeemAmount * DECIMAL_PRECISION / price;
    ...
    function testZombieTrovePointerGetsResetIfTroveIsClosed() public {
        ...
        deal(address(usdxToken), B, debt_B);
    ...
    function testZombieBorrowerCanCloseZombieTrove() public {
        ...
        // E sends usdx back to A and B so they can close
        transferUsdx(E, A, usdxToken.balanceOf(E) / 2);
        transferUsdx(E, B, usdxToken.balanceOf(E));
    ...
    function testZombieBorrowerCanDrawFreshDebtToAboveMIN_DEBT() public {
        ...
        // A and B withdraw Usdx from their zombie Trove
    ...
    function testZombieTroveBorrowerCanNotDrawFreshDebtToBelowMIN_DEBT() public {
        ...
        // A and B attempt to withdraw Usdx, but not enough
    ...
    function testZombieTroveBorrowerCanNotRepayDebt() public {
        ...
        // E sends Usdx back to A and B
        transferUsdx(E, A, usdxToken.balanceOf(E) / 2);
        transferUsdx(E, B, usdxToken.balanceOf(E));

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.repayUsdx(troveIDs.A, debtRepayment);
        vm.stopPrank();

        vm.startPrank(B);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.repayUsdx(troveIDs.B, debtRepayment);
        vm.stopPrank();
    }

    function testZombieTroveBorrowerCanNotUseNormalWithdrawUsdxFunction() public {
        (,, ABCDEF memory troveIDs) = _setupForRedemptionAscendingInterest();
        ...
        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.withdrawUsdx(troveIDs.A, debtWithdrawal, 0);
        vm.stopPrank();

        vm.startPrank(B);
        vm.expectRevert(BorrowerOperations.TroveNotActive.selector);
        borrowerOperations.withdrawUsdx(troveIDs.B, debtWithdrawal, 0);
        vm.stopPrank();
    }
    ...
    function testZombieTroveCanBeLiquidated() public {
        ...
        assertGt(stabilityPool.getTotalUsdxDeposits(), troveManager.getTroveEntireDebt(troveIDs.B));
