...
contract BorrowerOperationsOnBehalfTroveManagamentTest is DevTestSetup {
    ...
    function testCannotReopenTrove() public {
        ...
        deal(address(usdxToken), A, troveManager.getTroveEntireDebt(ATroveId));
    ...
    function testRepayUsdxWithAddManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Owner can repay usdx
        vm.startPrank(A);
        uint256 AInitialUsdxBalance = usdxToken.balanceOf(A);
        uint256 initialDebt = troveManager.getTroveEntireDebt(ATroveId);

        borrowerOperations.repayUsdx(ATroveId, 10e18);
        vm.stopPrank();

        assertEq(troveManager.getTroveEntireDebt(ATroveId), initialDebt - 10e18, "Wrong trove debt 1");
        assertEq(usdxToken.balanceOf(A), AInitialUsdxBalance - 10e18, "Wrong owner balance 1");

        // Manager can repay usdx
        deal(address(usdxToken), B, 100e18);
        vm.startPrank(B);
        uint256 BInitialUsdxBalance = usdxToken.balanceOf(B);

        borrowerOperations.repayUsdx(ATroveId, 10e18);
        vm.stopPrank();

        assertEq(troveManager.getTroveEntireDebt(ATroveId), initialDebt - 20e18, "Wrong trove debt 2");
        assertEq(usdxToken.balanceOf(B), BInitialUsdxBalance - 10e18, "Wrong manager balance 2");

        // Others can’t repay usdx
        deal(address(usdxToken), C, 100e18);
        vm.startPrank(C);
        uint256 CInitialUsdxBalance = usdxToken.balanceOf(C);
        ...
        assertEq(usdxToken.balanceOf(C), CInitialUsdxBalance, "Wrong manager balance 3");
    }

    function testRepayUsdxWithoutAddManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        assertEq(usdxToken.balanceOf(A), AInitialUsdxBalance - 10e18, "Wrong owner balance");
        vm.stopPrank();

        assertEq(borrowerOperations.addManagerOf(ATroveId), address(0));

        // Others can repay usdx
        deal(address(usdxToken), B, 100e18);
        uint256 BInitialUsdxBalance = usdxToken.balanceOf(B);
        ...
        assertEq(usdxToken.balanceOf(B), BInitialUsdxBalance - 10e18, "Wrong manager balance");
    }

    function testWithdrawUsdxWithRemoveManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Owner can withdraw usdx
        uint256 AInitialUsdxBalance = usdxToken.balanceOf(A);

        withdrawUsdx100pct(A, ATroveId, 10e18);

        assertEq(usdxToken.balanceOf(A), AInitialUsdxBalance + 10e18, "Wrong owner balance");

        // Manager can withdraw usdx
        uint256 BInitialUsdxBalance = usdxToken.balanceOf(B);

        withdrawUsdx100pct(B, ATroveId, 10e18);

        assertEq(usdxToken.balanceOf(A), AInitialUsdxBalance + 20e18, "Wrong owner balance");
        assertEq(usdxToken.balanceOf(B), BInitialUsdxBalance, "Wrong manager balance");
    }

    function testWithdrawUsdxWithoutRemoveManager() public {
        uint256 ATroveId = openTroveNoHints100pct(A, 100 ether, 10000e18, 1e17);
        ...
        // Manager can’t withdraw usdx
        uint256 BInitialUsdxBalance = usdxToken.balanceOf(B);

        vm.expectRevert(AddRemoveManagers.NotOwnerNorRemoveManager.selector);
        this.withdrawUsdx100pct(B, ATroveId, 10e18);
    ...
    function testCloseTroveByManagerWithRemoveManager() public {
        ...
        deal(address(usdxToken), B, troveManager.getTroveEntireDebt(ATroveId));
        vm.startPrank(B);
        borrowerOperations.closeTrove(ATroveId);
        vm.stopPrank();
    ...
    function testCloseTroveByOtherWithRemoveManager() public {
        ...
        deal(address(usdxToken), C, troveManager.getTroveEntireDebt(ATroveId));
        vm.startPrank(C);
        vm.expectRevert(AddRemoveManagers.NotOwnerNorRemoveManager.selector);
        borrowerOperations.closeTrove(ATroveId);
        vm.stopPrank();
