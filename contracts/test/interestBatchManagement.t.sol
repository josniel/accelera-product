...
contract InterestBatchManagementTest is DevTestSetup {
    ...
    function testCannotSetBatchManagerIfTroveIsClosed() public {
        ...
        // Close trove
        deal(address(usdxToken), A, 6000e18); // Needs more Usdx for interest and upfront fee
    ...
    function testCannotOpenAndJoinIfBelowMCRPlusBCR() public {
        ...
        uint256 collAmount = usdxAmount * (MCR + BCR) / price; // upfront fee will put it slightly below
    ...
    function testCannotJoinBatchAfterOpeningIfBelowMCRPlusBCR() public {
        ...
        uint256 troveId = openTroveNoHints100pct(A, collAmount, usdxAmount, MIN_ANNUAL_INTEREST_RATE);
    ...
    function testCannotBorrowInsideABatchIfBelowMCRPlusBCR() public {
        ...
        uint256 usdxAmount = coll * price / (MCR + BCR) - debt;

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.ICRBelowMCRPlusBCR.selector);
        borrowerOperations.withdrawUsdx(troveId, usdxAmount, usdxAmount);
        vm.stopPrank();
    }
    ...
    function testCannotAdjustInsideABatchIfBelowMCRPlusBCR1() public {
        ...
        uint256 repayUsdx = debt - (coll - withdrawColl) * price / (MCR + BCR) - 1;
        //console2.log(coll - withdrawColl, "coll - withdrawColl");
        //console2.log(debt - repayUsdx, "debt - repayUsdx");

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.ICRBelowMCRPlusBCR.selector);
        borrowerOperations.adjustTrove(troveId, withdrawColl, false, repayUsdx, false, 10000e18);
        vm.stopPrank();
    }
    ...
    function testCannotAdjustInsideABatchIfBelowMCRPlusBCR2() public {
        ...
        uint256 borrowUsdx = 195000e18;
        uint256 addColl = (debt + borrowUsdx) * (MCR + BCR) / price - coll;
        //console2.log(debt+borrowUsdx, "debt+borrowUsdx");
        //console2.log(coll+addColl, "coll+addColl");

        vm.startPrank(A);
        vm.expectRevert(BorrowerOperations.ICRBelowMCRPlusBCR.selector);
        borrowerOperations.adjustTrove(troveId, addColl, true, borrowUsdx, true, 10000e18);
        vm.stopPrank();
    }
