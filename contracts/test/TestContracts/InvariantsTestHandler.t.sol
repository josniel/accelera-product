...
contract InvariantsTestHandler is Assertions, BaseHandler, BaseMultiCollateralTest {
    ...
    struct ProvideToSPContext {
        TestDeployer.LiquityContractsDev c;
        uint256 pendingInterest;
        uint256 totalUsdxDeposits;
        uint256 blockedSPYield;
        uint256 initialUsdxDeposit;
        uint256 usdxDeposit;
        uint256 usdxYield;
        uint256 ethGain;
        uint256 ethStash;
    ...
    mapping(uint256 branchIdx => uint256) public spUsdxDeposits;
    mapping(uint256 branchIdx => uint256) public spUsdxYield;
    ...
    // All free-floating USDX is kept in the handler, to be dealt out to actors as needed
    uint256 _handlerUsdx;
    ...
    function _openTrove(OpenTroveContext memory v) internal {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(msg.sender, v.borrowed);
            if (v.join) _sweepUsdx(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function adjustTrove(
        ...
        if (!isDebtInc) debtChange = Math.min(debtChange, _handlerUsdx);
        ...
        if (!isDebtInc) _dealUsdx(msg.sender, debtChange);
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            if (!isDebtInc) _sweepUsdx(msg.sender, debtChange);
        } else {
            // Cleanup (success)
            if (!isCollInc) _sweepColl(i, msg.sender, collChange);
            if (isDebtInc) _sweepUsdx(msg.sender, debtChange);
            // Take back remaining USDX after repayment truncation
            if (!isDebtInc) _sweepUsdx(msg.sender, debtChange - uint256(-v.debtDelta));
            if (v.batchManager != address(0)) _sweepUsdx(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function closeTrove(uint256 i) external {
        ...
        v.dealt = Math.min(v.t.entireDebt, _handlerUsdx);
        _dealUsdx(msg.sender, v.dealt);
        ...
        try v.c.borrowerOperations.closeTrove(v.troveId) {
            ...
            // Justify failures
            if (selector == BorrowerOperations.NotEnoughUsdxBalance.selector) {
                assertLtDecimal(v.dealt, v.t.entireDebt, 18, "Shouldn't have failed as caller had enough Usdx");
            } else if (selector == BorrowerOperations.TCRBelowCCR.selector) {
    ...
    function batchLiquidateTroves(uint256 i) external {
        ...
        try c.troveManager.batchLiquidateTroves(_troveIdsFrom(i, l.batch)) {
            info("SP USDX: ", c.stabilityPool.getTotalUsdxDeposits().decimal());
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < l.batchManagers.size(); ++j) {
                _sweepUsdx(l.batchManagers.get(j), batchManagementFee[j]);
            }
        }
    ...
    function redeemCollateral(uint256 amount, uint256 maxIterationsPerCollateral) external {
        ...
        amount = _bound(amount, 0, _handlerUsdx);
        maxIterationsPerCollateral = _bound(maxIterationsPerCollateral, 0, maxNumTroves * 11 / 10);

        uint256 oldBaseRate = _getBaseRate();
        uint256 usdxSupply = usdxToken.totalSupply();
        uint256 redemptionRate = _getRedemptionRate(oldBaseRate + _getBaseRateIncrease(usdxSupply, amount));
        ...
        info("redeemed USDX: ", totalDebtRedeemed.decimal());
        ...
        try collateralRegistry.redeemCollateral(amount, maxIterationsPerCollateral, redemptionRate) {
            ...
            // Effects (global)
            _baseRate = Math.min(oldBaseRate + _getBaseRateIncrease(usdxSupply, totalDebtRedeemed), _100pct);
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < branches.length; ++j) {
                ...
                for (uint256 i = 0; i < r[j].batchManagers.size(); ++i) {
                    _sweepUsdx(r[j].batchManagers.get(i), batchManagementFee[j][i]);
                }
            }

            // There can be a slight discrepancy when hitting batched Troves
            uint256 remainingAmount = usdxToken.balanceOf(msg.sender);
            assertApproxEqAbsDecimal(remainingAmount, amount - totalDebtRedeemed, 1e5, 18, "Wrong remaining USDX");
            _sweepUsdx(msg.sender, remainingAmount);
        }
    ...
    function urgentRedemption(uint256 i, uint256 amount) external {
        ...
        info("redeemed USDX: ", r.totalDebtRedeemed.decimal());
        ...
        if (bytes(errorString).length > 0) {
            ...
            for (uint256 j = 0; j < r.batchManagers.size(); ++j) {
                _sweepUsdx(r.batchManagers.get(j), batchManagementFee[j]);
            }

            // There can be a slight discrepancy when hitting batched Troves
            uint256 remainingAmount = usdxToken.balanceOf(msg.sender);
            assertApproxEqAbsDecimal(remainingAmount, amount - r.totalDebtRedeemed, 1e5, 18, "Wrong remaining USDX");
            _sweepUsdx(msg.sender, remainingAmount);
        }
    ...
    function provideToSP(uint256 i, uint256 amount, bool claim) external {
        ...
        v.c = branches[i];
        v.pendingInterest = v.c.activePool.calcPendingAggInterest();
        v.totalUsdxDeposits = v.c.stabilityPool.getTotalUsdxDeposits();
        v.blockedSPYield = v.totalUsdxDeposits < DECIMAL_PRECISION
            ? v.c.activePool.calcPendingSPYield() + v.c.stabilityPool.getYieldGainsPending()
            : 0;
        v.initialUsdxDeposit = v.c.stabilityPool.deposits(msg.sender);
        v.usdxDeposit = v.c.stabilityPool.getCompoundedUsdxDeposit(msg.sender);
        v.usdxYield = v.c.stabilityPool.getDepositorYieldGainWithPending(msg.sender);
        ...
        v.usdxClaimed = claim ? v.usdxYield : 0;

        info("initial deposit: ", v.initialUsdxDeposit.decimal());
        info("compounded deposit: ", v.usdxDeposit.decimal());
        info("yield gain: ", v.usdxYield.decimal());
        ...
        try v.c.stabilityPool.provideToSP(amount, claim) {
            ...
            v.usdxDeposit += amount;
            v.usdxDeposit += v.usdxYield;
            v.usdxDeposit -= v.usdxClaimed;

            // See if the change unblocked any pending yield
            v.totalUsdxDeposits += amount;
            v.totalUsdxDeposits += v.usdxYield;
            v.totalUsdxDeposits -= v.usdxClaimed;

            uint256 newUsdxYield =
                v.totalUsdxDeposits >= DECIMAL_PRECISION ? v.blockedSPYield * v.usdxDeposit / v.totalUsdxDeposits : 0;

            assertApproxEqAbsDecimal(
                v.c.stabilityPool.getCompoundedUsdxDeposit(msg.sender), v.usdxDeposit, 1e7, 18, "Wrong deposit"
            );
            assertApproxEq(v.c.stabilityPool.getDepositorYieldGain(msg.sender), newUsdxYield, 1e11, "Wrong yield gain");
            ...
            spUsdxDeposits[i] += amount;
            spUsdxDeposits[i] += v.usdxYield;
            spUsdxDeposits[i] -= v.usdxClaimed;
            spUsdxYield[i] -= v.usdxYield;
        } catch Error(string memory reason) {
            v.errorString = reason;
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (failure)
            _sweepUsdx(msg.sender, amount); // Take back the USDX that was dealt
        } else {
            // Cleanup (success)
            _sweepUsdx(msg.sender, v.usdxClaimed);
    ...
    function withdrawFromSP(uint256 i, uint256 amount, bool claim) external {
        ...
        amount = _bound(amount, 0, v.usdxDeposit * 11 / 10); // sometimes try withdrawing too much
        v.withdrawn = Math.min(amount, v.usdxDeposit);
        ...
        try v.c.stabilityPool.withdrawFromSP(amount, claim) {
            // Preconditions
            assertGtDecimal(v.initialUsdxDeposit, 0, 18, "Should have failed as user had zero deposit");
            assertGeDecimal(
                v.totalUsdxDeposits + v.usdxYield,
                v.usdxClaimed + v.withdrawn + MIN_USDX_IN_SP,
                18,
                "Should have failed as withdrawal left less than MIN_USDX_IN_SP"
            );
            ...
            if (reason.equals("StabilityPool: User must have a non-zero deposit")) {
                assertEqDecimal(v.initialUsdxDeposit, 0, 18, "Shouldn't have failed as user had a non-zero deposit");
            } else if (reason.equals("Withdrawal must leave totalUsdxDeposits >= MIN_USDX_IN_SP")) {
                assertLtDecimal(
                    v.totalUsdxDeposits + v.usdxYield,
                    v.usdxClaimed + v.withdrawn + MIN_USDX_IN_SP,
                    18,
                    "Shouldn't have failed as withdrawal left more than MIN_USDX_IN_SP"
                );
            } else {
                revert(reason);
            }
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(msg.sender, v.usdxClaimed + v.withdrawn);
            _sweepColl(i, msg.sender, v.ethClaimed);
        }
    }
    ...
    function claimAllCollGains(uint256 i) external {
        ...
        TestDeployer.LiquityContractsDev memory c = branches[i];
        uint256 pendingInterest = c.activePool.calcPendingAggInterest();
        uint256 initialUsdxDeposit = c.stabilityPool.deposits(msg.sender);
        ...
        try c.stabilityPool.claimAllCollGains() {
            // Preconditions
            assertEqDecimal(initialUsdxDeposit, 0, 18, "Should have failed as user had a non-zero deposit");
            assertGtDecimal(ethStash, 0, 18, "Should have failed as ETH stash was zero");

            // Effects (deposit)
            assertEqDecimal(c.stabilityPool.getCompoundedUsdxDeposit(msg.sender), 0, 18, "Wrong deposit");
            ...
            if (reason.equals("StabilityPool: User must have no deposit")) {
                assertGtDecimal(initialUsdxDeposit, 0, 18, "Shouldn't have failed as user had no deposit");
    ...
    function lowerBatchManagementFee(uint256 i, uint256 newManagementFee) external {
        ...
        if (bytes(errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(msg.sender, batchManagementFee);
        }
    }
        ...
        external
    {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(v.newBatchManager, v.batchManagementFee);
        }
    }
    ...
    function removeFromBatch(uint256 i, uint256 newInterestRate, uint32 upperHintSeed, uint32 lowerHintSeed) external {
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(v.batchManager, v.batchManagementFee);
        }
    }
    ...
    function setBatchManagerAnnualInterestRate(
        ...
        if (bytes(v.errorString).length > 0) {
            ...
            // Cleanup (success)
            _sweepUsdx(msg.sender, v.b.accruedManagementFee);
        }
    }
    ...
    function _getBaseRateIncrease(uint256 usdxSupply, uint256 redeemed) internal pure returns (uint256) {
        return usdxSupply > 0 ? redeemed * DECIMAL_PRECISION / usdxSupply / REDEMPTION_BETA : 0;
    }
    ...
    function _mintYield(uint256 i, uint256 pendingInterest, uint256 upfrontFee) internal {
        ...
        uint256 mintedSPUsdxYield = mintedYield * SP_YIELD_SPLIT / DECIMAL_PRECISION;

        spUsdxYield[i] += mintedSPUsdxYield;
    ...
    function _dealUsdx(address to, uint256 amount) internal {
        usdxToken.transfer(to, amount);
        _handlerUsdx -= amount;
    }

    function _sweepUsdx(address from, uint256 amount) internal {
        vm.prank(from);
        usdxToken.transfer(address(this), amount);
        _handlerUsdx += amount;
    }
    ...
    function _aggregateLiquidation(uint256 i, LatestTroveData memory trove, LiquidationTotals storage t) internal {
        ...
        // Offset debt by SP
        uint256 spRemaining = spUsdxDeposits[i] - t.spOffset;
        uint256 spOffset = Math.min(trove.entireDebt, spRemaining > MIN_USDX_IN_SP ? spRemaining - MIN_USDX_IN_SP : 0);
        t.spOffset += spOffset;
    ...
    function _encodeActiveTroveAdjustment(
        ...
        if (prop == AdjustedTroveProperties.onlyDebt) {
            if (isDebtIncrease) {
                return abi.encodeCall(IBorrowerOperations.withdrawUsdx, (troveId, debtChange, maxUpfrontFee));
            } else {
                return abi.encodeCall(IBorrowerOperations.repayUsdx, (troveId, debtChange));
            }
        }
        ...
        returns (bytes4 selector, string memory errorString)
    {
        ...
        if (revertData.length == 4) {
            ...
            if (selector == BorrowerOperations.NotEnoughUsdxBalance.selector) {
                return (selector, "BorrowerOperations.NotEnoughUsdxBalance()");
            }
