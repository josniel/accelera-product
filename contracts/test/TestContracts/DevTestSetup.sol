...
contract DevTestSetup is BaseTest {
    ...
    function setUp() public virtual {
        ...
        (contracts, collateralRegistry, usdxToken, hintHelpers,, WETH, zappers) = deployer.deployAndConnectContracts();
    ...
    function _setupForSPDepositAdjustments() internal returns (ABCDEF memory troveIDs) {
        ...
        // D sends USDX to A and B so they have some to use in tests
        transferUsdx(D, A, usdxToken.balanceOf(D) / 2);
        transferUsdx(D, B, usdxToken.balanceOf(D));
        ...
        returns (uint256 coll, uint256 debtRequest, ABCDEF memory troveIDs)
    {
        ...
        // A, B, C, D transfer all their Usdx to E
        transferUsdx(A, E, usdxToken.balanceOf(A));
        transferUsdx(B, E, usdxToken.balanceOf(B));
        transferUsdx(C, E, usdxToken.balanceOf(C));
        transferUsdx(D, E, usdxToken.balanceOf(D));
    }
