...
contract MetadataDeployment is Script /* , StdAssertions */ {
    ...
    function _loadFiles() internal {
        ...
        bytes memory usdxFile = bytes(vm.readFile(string.concat(root, "usdx_logo.txt")));
        File memory usdx = File(usdxFile, offset, offset + usdxFile.length);

        offset += usdxFile.length;

        files[bytes4(keccak256("USDX"))] = usdx;
    ...
    function _storeFile() internal {
        bytes memory data = bytes.concat(
            files[bytes4(keccak256("USDX"))].data,
    ...
    function _deployFixedAssetReader(bytes32 _salt) internal {
        ...
        FixedAssets[0] = FixedAssetReader.Asset(
            uint128(files[bytes4(keccak256("USDX"))].start), uint128(files[bytes4(keccak256("USDX"))].end)
        );
