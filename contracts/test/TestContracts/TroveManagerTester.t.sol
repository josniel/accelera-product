...
contract TroveManagerTester is ITroveManagerTester, TroveManager {
    ...
    function getUsdxToken() external view returns (IUsdxToken) {
        return usdxToken;
    }
    ...
    function get_L_usdxDebt() external view returns (uint256) {
        return L_usdxDebt;
    }
    ...
    function get_lastUsdxDebtError_Redistribution() external view returns (uint256) {
        return lastUsdxDebtError_Redistribution;
    }
    ...
    */
    ...
    function getEffectiveRedemptionFeeInColl(uint256 _redeemAmount, uint256 _price) external view returns (uint256) {
        return collateralRegistry.getEffectiveRedemptionFeeInUsdx(_redeemAmount) * DECIMAL_PRECISION / _price;
    }
    ...
    // Get the borrower's pending accumulated Usdx reward, earned by their stake
    function getPendingUsdxDebtReward(uint256 _troveId) external view override returns (uint256 redistUsdxDebtGain) {
        LatestTroveData memory trove;
        _getLatestTroveData(_troveId, trove);
        return trove.redistUsdxDebtGain;
    }
        ...
        )
    {
        ...
        return
            (trove.entireDebt, trove.entireColl, trove.redistUsdxDebtGain, trove.redistCollGain, trove.accruedInterest);
    }
