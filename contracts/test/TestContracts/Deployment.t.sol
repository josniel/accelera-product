...
contract TestDeployer is MetadataDeployment {
        ...
        )
    {
        ...
        (contractsArray, collateralRegistry, usdxToken, hintHelpers, multiTroveGetter, WETH, zappersArray) =
        ...
        )
    {
        ...
        (contractsArray, collateralRegistry, usdxToken, hintHelpers, multiTroveGetter, zappersArray) =
        ...
        )
    {
        ...
        // Deploy Usdx
        vars.bytecode = abi.encodePacked(type(UsdxToken).creationCode, abi.encode(address(this)));
        vars.usdxTokenAddress = getAddress(address(this), vars.bytecode, SALT);
        usdxToken = new UsdxToken{salt: SALT}(address(this));
        assert(address(usdxToken) == vars.usdxTokenAddress);
        ...
        collateralRegistry = new CollateralRegistry(usdxToken, vars.collaterals, vars.troveManagers);
        ...
        usdxToken.setCollateralRegistry(address(collateralRegistry));
    }
        ...
        returns (DeploymentResultMainnet memory result)
    {
        ...
        vars.bytecode = abi.encodePacked(type(UsdxToken).creationCode, abi.encode(address(this)));
        vars.usdxTokenAddress = getAddress(address(this), vars.bytecode, SALT);
        result.usdxToken = new UsdxToken{salt: SALT}(address(this));
        assert(address(result.usdxToken) == vars.usdxTokenAddress);
        ...
        result.collateralRegistry = new CollateralRegistryTester(result.usdxToken, vars.collaterals, vars.troveManagers);
        ...
        ICurveStableswapNGPool usdcCurvePool = _deployCurveUsdxUsdcPool(result.usdxToken, true);
        ...
        for (vars.i = 0; vars.i < vars.numCollaterals; vars.i++) {
            ...
            params.usdxToken = result.usdxToken;
        ...
        result.usdxToken.setCollateralRegistry(address(result.collateralRegistry));
    }
        ...
        returns (LiquityContracts memory contracts, Zappers memory zappers)
    {
        ...
        // Connect contracts
        _params.usdxToken.setBranchAddresses(
            address(contracts.troveManager),
            address(contracts.stabilityPool),
            address(contracts.borrowerOperations),
            address(contracts.activePool)
    ...
    function _deployZappers(
        ...
        IExchange curveExchange = _deployCurveExchange(_collToken, _usdxToken, _priceFeed, _mainnet);
    ...
    function _deployCurveExchange(IERC20 _collToken, IUsdxToken _usdxToken, IPriceFeed _priceFeed, bool _mainnet)
        internal
        returns (IExchange)
    {
        if (!_mainnet) return new CurveExchange(_collToken, _usdxToken, ICurvePool(address(0)), 1, 0);
        ...
        IExchange curveExchange = new CurveExchange(_collToken, _usdxToken, curvePool, 1, 0);
    ...
    function _deployLeverageZappers(
        ...
        zappers.leverageZapperUniV3 =
            _deployUniV3LeverageZapper(_addressesRegistry, _collToken, _usdxToken, _priceFeed, _flashLoanProvider, _lst);
        zappers.leverageZapperHybrid = _deployHybridLeverageZapper(
            _addressesRegistry, _collToken, _usdxToken, _flashLoanProvider, _usdcCurvePool, _lst
        );
    }
    ...
    function _deployUniV3LeverageZapper(
        ...
        vars.uniV3Exchange = new UniV3Exchange(_collToken, _usdxToken, UNIV3_FEE, uniV3Router);
        ILeverageZapper leverageZapperUniV3;
        if (_lst) {
        ...
        if (address(_usdxToken) < address(_collToken)) {
            //console2.log("b < c");
            vars.tokens[0] = address(_usdxToken);
            vars.tokens[1] = address(_collToken);
        } else {
            //console2.log("c < b");
            vars.tokens[0] = address(_collToken);
            vars.tokens[1] = address(_usdxToken);
        }
        uniV3PositionManager.createAndInitializePoolIfNecessary(
            vars.tokens[0], // token0,
            vars.tokens[1], // token1,
            UNIV3_FEE, // fee,
            UniV3Exchange(address(vars.uniV3Exchange)).priceToSqrtPrice(_usdxToken, _collToken, vars.price) // sqrtPriceX96
        );
    ...
    function _deployCurveUsdxUsdcPool(IUsdxToken _usdxToken, bool _mainnet) internal returns (ICurveStableswapNGPool) {
