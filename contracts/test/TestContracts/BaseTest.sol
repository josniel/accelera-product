...
import "src/Interfaces/IUsdxToken.sol";
...
contract BaseTest is TestAccounts, Logging {
    ...
    function openTroveNoHints100pct(address _account, uint256 _coll, uint256 _usdxAmount, uint256 _annualInterestRate)
        public
        returns (uint256 troveId)
    {
        (troveId,) = openTroveHelper(_account, 0, _coll, _usdxAmount, _annualInterestRate);
    }
    ...
    function openTroveNoHints100pctWithIndex(
        ...
        (troveId,) = openTroveHelper(_account, _index, _coll, _usdxAmount, _annualInterestRate);
    }
    ...
    function openTroveHelper(
        ...
        upfrontFee = predictOpenTroveUpfrontFee(_usdxAmount, _annualInterestRate);
    ...
    function adjustTrove100pct(
        ...
        borrowerOperations.adjustTrove(
            ...
            predictAdjustTroveUpfrontFee(
                _troveId,
                _isDebtIncrease ? _usdxChange : 0 // debtIncrease
            )
        );
    ...
    function withdrawUsdx100pct(address _account, uint256 _troveId, uint256 _debtIncrease) public {
        vm.startPrank(_account);
        borrowerOperations.withdrawUsdx(_troveId, _debtIncrease, predictAdjustTroveUpfrontFee(_troveId, _debtIncrease));
        vm.stopPrank();
    }

    function repayUsdx(address _account, uint256 _troveId, uint256 _debtDecrease) public {
        vm.startPrank(_account);
        borrowerOperations.repayUsdx(_troveId, _debtDecrease);
        vm.stopPrank();
    }
    ...
    function transferUsdx(address _from, address _to, uint256 _amount) public {
        vm.startPrank(_from);
        usdxToken.transfer(_to, _amount);
        vm.stopPrank();
    }
    ...
    function getShareofSPReward(address _depositor, uint256 _reward) public view returns (uint256) {
        return _reward * stabilityPool.getCompoundedUsdxDeposit(_depositor) / stabilityPool.getTotalUsdxDeposits();
    }
    ...
    function logContractAddresses() public view {
        ...
        console.log("UsdxToken addr: ", address(usdxToken));
    }
