...
contract SPTest is DevTestSetup {
    ...
    struct PTestVars {
        ...
        uint256 spEthBal1;
        uint256 spEthBal2;
        uint256 initialUsdxGainA;
        uint256 initialUsdxGainB;
    }
    ...
    // --- provideToSP, doClaim == true, USDX gains ---

    function testProvideToSPWithClaim_WithOnlyCurrentUSDXGainsSendsTotalUSDXGainToDepositor() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 currentUsdxGain = stabilityPool.getDepositorYieldGain(A);
        assertGt(currentUsdxGain, 0);

        uint256 usdxBal_A = usdxToken.balanceOf(A);

        uint256 topUp = 1e18;
        makeSPDepositAndClaim(A, topUp);

        assertEq(usdxToken.balanceOf(A), usdxBal_A + currentUsdxGain - topUp);
    }

    function testProvideToSPWithClaim_WithCurrentUSDXGainsZerosCurrentUSDXGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        // Check A's currentUsdxGain reduced to 0
    ...
    // --- provideToSP, doClaim == false, USDX gains ---

    function testProvideToSPNoClaimAddsUSDXGainsToDeposit() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 depositBefore_A = stabilityPool.getCompoundedUsdxDeposit(A);

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertApproximatelyEqual(
            stabilityPool.getCompoundedUsdxDeposit(A), depositBefore_A + topUp + currentUsdxGain, 1e4
        );
    }

    function testProvideToSPNoClaimAddsUsdxGainsToTotalUsdxDeposits() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 totalUsdxDepositsBefore = stabilityPool.getTotalUsdxDeposits();

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertApproximatelyEqual(
            stabilityPool.getTotalUsdxDeposits(), totalUsdxDepositsBefore + topUp + currentUsdxGain, 1e4
        );
    }

    function testProvideToSPNoClaimZerosCurrentUsdxGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testProvideToSPNoClaimReducesDepositorUsdxBalanceByOnlyTheTopUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 usdxBalBefore_A = usdxToken.balanceOf(A);

        uint256 topUp = 1e18;
        makeSPDepositNoClaim(A, topUp);

        assertEq(usdxToken.balanceOf(A), usdxBalBefore_A - topUp);
    }
    ...
    // --- withdrawFromSP, doClaim == true, USDX gains ---
    function testWithdrawFromSPWithClaim_WithCurrentUSDXGainsSendsUSDXGainToDepositor() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertEq(usdxToken.balanceOf(A), usdxBal_A + withdrawal + currentUsdxGain);
    }

    function testWithdrawFromSPWithClaim_WithCurrentUSDXGainsZerosCurrentUsdxGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        // Check A's USDX gain reduced to 0
        assertEq(stabilityPool.getDepositorYieldGain(A), 0);
    }
    ...
    // --- withdrawFromSP, doClaim == false, USDX gains ---

    function testWithdrawFromSPNoClaimAddsUSDXGainsToDeposit() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertApproximatelyEqual(
            stabilityPool.getCompoundedUsdxDeposit(A), depositBefore_A - withdrawal + currentUsdxGain, 1e4
        );
    }

    function testWithdrawFromSPNoClaimAddsUsdxGainsToTotalUsdxDeposits() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertEq(stabilityPool.getTotalUsdxDeposits(), totalUsdxDepositsBefore - withdrawal + currentUsdxGain);
    }

    function testWithdrawFromSPNoClaimZerosCurrentUsdxGains() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testWithdrawFromSPNoClaimReducesDepositorUsdxBalanceByOnlyTheTopUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        assertEq(usdxToken.balanceOf(A), usdxBalBefore_A + withdrawal);
    }
    ...
    function testClaimAllCollGainsRevertsWhenUserHasNoDeposit() public {
        ...
        // A
        uint256 compoundedDeposit_A = stabilityPool.getCompoundedUsdxDeposit(A);
    ...
    function testClaimAllCollGainsRevertsWhenUserHasNoCollGains() public {
        ...
        // B
        uint256 compoundedDeposit_B = stabilityPool.getCompoundedUsdxDeposit(B);
        ...
        assertEq(stabilityPool.getCompoundedUsdxDeposit(B), 0);
    ...
    function testClaimAllCollGainsDoesNotChangeCompoundedDeposit() public {
        ...
        // A withdraws deposit
        uint256 deposit_A = stabilityPool.getCompoundedUsdxDeposit(A);
        ...
        assertEq(compoundedDeposit_A, stabilityPool.getCompoundedUsdxDeposit(A));
    }
    ...
    // --- Usdx reward sum 'B' tests ---

    function testUsdxRewardsSumDoesntChangeWhenSPIsEmpty() public {
        priceFeed.setPrice(2000e18);
        openTroveNoHints100pct(A, 3 ether, 2000e18, 25e16);

        // check SP is 0
        assertEq(stabilityPool.getTotalUsdxDeposits(), 0);
        ...
        uint256 usdxRewardSum_1 = stabilityPool.scaleToB(0);
        assertEq(usdxRewardSum_1, 0);

        openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);

        uint256 usdxRewardSum_2 = stabilityPool.scaleToB(0);
        assertEq(usdxRewardSum_2, usdxRewardSum_1);
    }

    function testUsdxRewardSumDoesntChangeWhenNoYieldMinted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 usdxRewardSum_1 = stabilityPool.scaleToB(0);
        assertGt(usdxRewardSum_1, 0, "USDX reward sum 1");

        // Adjust a Trove in a way that doesn't incur an upfront fee
        repayUsdx(B, troveIDs.B, 1_000 ether);

        uint256 usdxRewardSum_2 = stabilityPool.scaleToB(0);
        assertEq(usdxRewardSum_2, usdxRewardSum_1, "USDX reward sum 2");
    }

    function testUsdxRewardSumIncreasesWhenTroveOpened() public {
        _setupForSPDepositAdjustments();
        ...
        uint256 usdxRewardSum_1 = stabilityPool.scaleToB(0);
        assertGt(usdxRewardSum_1, 0);

        openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);

        uint256 usdxRewardSum_2 = stabilityPool.scaleToB(0);
        assertGt(usdxRewardSum_2, usdxRewardSum_1);
    }

    function testUsdxRewardSumIncreasesWhenTroveInterestRateAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardSumIncreasesWhenTroveClosed() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 usdxRewardSum_1 = stabilityPool.scaleToB(0);
        assertGt(usdxRewardSum_1, 0); // yield from upfront fee

        // F sends E his usdx so he can close
        vm.startPrank(F);
        usdxToken.transfer(E, usdxToken.balanceOf(F));
        vm.stopPrank();
    ...
    function testUsdxRewardSumIncreasesWhenTroveDebtAndCollAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardSumIncreasesWhenInterestAppliedPermissionlessly() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardSumIncreasesWhenTroveLiquidated() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardSumIncreasesWhenRedemptionOccurs() public {
        _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardSumIncreasesWhenNewDepositIsMade() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardSumIncreasesWhenDepositToppedUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardSumIncreasesWhenDepositWithdrawn() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardsOwedDoesntChangeWhenSPIsEmpty() public {
        priceFeed.setPrice(2000e18);
        openTroveNoHints100pct(A, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardsOwedOnlyIcreasesByUpfrontFeeYieldWhenNoInterestMinted() public {
        _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenTroveOpened() public {
        _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenTroveInterestRateAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenTroveClosed() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
        troveIDs.F = openTroveNoHints100pct(F, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardsOwedIncreasesWhenTroveDebtAndCollAdjusted() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenInterestAppliedPermissionlessly() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenTroveLiquidated() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenRedemptionOccurs() public {
        _setupForSPDepositAdjustments();
    ...
    function testUsdxRewardsOwedIncreasesWhenNewDepositIsMade() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardsOwedIncreasesWhenDepositToppedUp() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    function testUsdxRewardsOwedIncreasesWhenDepositWithdrawn() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        troveIDs.E = openTroveNoHints100pct(E, 3 ether, 2000e18, 25e16);
    ...
    // --- depositor USDX rewards tests ---

    function testGetDepositorUsdxGain_1SPDepositor1RewardEvent_EarnsAllSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();

        // B withdraws entirely
        makeSPWithdrawalAndClaim(B, stabilityPool.getCompoundedUsdxDeposit(B));

        // Check A has entirely of the non-zero SP deposits
        assertApproximatelyEqual(stabilityPool.getCompoundedUsdxDeposit(A), stabilityPool.getTotalUsdxDeposits(), 1e4);
        assertGt(stabilityPool.getTotalUsdxDeposits(), 0);
    ...
    function testGetDepositorUsdxGain_2SPDepositor1RewardEvent_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorUsdxGain_1SPDepositor2RewardEvent_EarnsAllSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorUsdxGain_2SPDepositor2RewardEvent_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
    ...
    function testGetDepositorUsdxGain_2SPDepositor1Liq1FreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsWithoutOwedYieldRewards();
        ...
        assertGt(expectedShareOfReward1_A, 0);
        assertGt(expectedShareOfReward1_B, 0);
        uint256 totalSPDeposits_1 = stabilityPool.getTotalUsdxDeposits();
        ...
        uint256 totalSPDeposits_2 = stabilityPool.getTotalUsdxDeposits();
    ...
    function testGetDepositorUsdxGain_2SPDepositor1LiqFreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustments();
        ...
        uint256 totalSPDeposits_0 = stabilityPool.getTotalUsdxDeposits();
        ...
        // A liquidates D
        liquidate(A, troveIDs.D);
        // Check SP has only 1e18 USDX now, and A and B have small remaining deposits
        assertEq(stabilityPool.getTotalUsdxDeposits(), 1e18, "SP total usdx deposits should be 1e18");
        assertLt(stabilityPool.getCompoundedUsdxDeposit(A), 1e18, "A should have <1e18 deposit");
        assertLt(stabilityPool.getCompoundedUsdxDeposit(B), 1e18, "B should have <1e18 deposit");
        assertLe(
            stabilityPool.getCompoundedUsdxDeposit(B) + stabilityPool.getCompoundedUsdxDeposit(A),
            1e18,
    ...
    function testGetDepositorUsdxGain_2SPDepositor1LiqScaleChangeFreshDeposit_EarnFairShareOfSPYield() public {
    ...
    function testGetDepositorUsdxGain_2SPDepositor2LiqScaleChangesFreshDeposit_EarnFairShareOfSPYield() public {
        ABCDEF memory troveIDs = _setupForSPDepositAdjustmentsBigTroves();
        ABCDEF[3] memory expectedShareOfReward;
        ...
        uint256 targetDebt_E = stabilityPool.getTotalUsdxDeposits() - 1e9;
    ...
    function testLiquidationWithLowPDoesNotDecreaseP_Cheat_Fuzz(uint256 _cheatP, uint256 _surplus) public {
        ...
        uint256 debtDelta = troveDebt - stabilityPool.getTotalUsdxDeposits();
    ...
    function testLiquidationsWithLowPAllowFurtherRewardsForAllFreshDepositors_Cheat_Fuzz(
        ...
        assertApproximatelyEqual(
            stabilityPool.getTotalUsdxDeposits(), stabilityPool.getCompoundedUsdxDeposit(A), 1e14, "A should own all SP"
        );
        ...
        // B transfers USDX to A so he can slightly top up the SP
        transferUsdx(B, A, usdxToken.balanceOf(B) / 2);
        ...
        // D sends some USDX to C
        uint256 freshDeposit = usdxToken.balanceOf(D) / 2;
        assertGt(freshDeposit, 0);

        transferUsdx(D, C, freshDeposit);
        ...
        makeSPWithdrawalAndClaim(A, stabilityPool.getCompoundedUsdxDeposit(A));
        assertEq(stabilityPool.getCompoundedUsdxDeposit(A), 0, "A deposit should be zero");
        ...
        testVars.totalDepositsBefore = stabilityPool.getTotalUsdxDeposits();
        ...
        testVars.usdxGainC = stabilityPool.getDepositorYieldGain(C);
        testVars.usdxGainD = stabilityPool.getDepositorYieldGain(D);
        assertApproximatelyEqual(testVars.expectedShareOfYield1_C, testVars.usdxGainC, 1e6, "C share of yield mismatch");
        assertApproximatelyEqual(testVars.expectedShareOfYield1_D, testVars.usdxGainD, 1e6, "D share of yield mismatch");
        ...
        transferUsdx(C, E, usdxToken.balanceOf(C));
        makeSPDepositAndClaim(E, usdxToken.balanceOf(E));
        ...
        // check all USDX and Coll gains are as expected
        uint256 usdxGainE = stabilityPool.getDepositorYieldGain(E);

        assertApproximatelyEqual(expectedShareOfYield2_E, usdxGainE, 1e9, "E share of yield mismatch");
    }
        ...
        public
    {
        ...
        ABCDEF memory troveIDs = _setupForPTests();
        testVars.initialUsdxGainA = stabilityPool.getDepositorYieldGain(A);
        testVars.initialUsdxGainB = stabilityPool.getDepositorYieldGain(B);

        uint256 troveDebt = troveManager.getTroveEntireDebt(troveIDs.D);
        uint256 debtDelta = troveDebt - stabilityPool.getTotalUsdxDeposits();
        // Make the surplus high enough  - in range [0.01, 1] USDX - to leave each depositor with non-zero deposit after the liq, yet still trigger a scale change
        ...
        testVars.totalSPBeforeLiq_C = stabilityPool.getTotalUsdxDeposits();
        testVars.deposit1_A = stabilityPool.getCompoundedUsdxDeposit(A);
        testVars.deposit1_B = stabilityPool.getCompoundedUsdxDeposit(B);

        uint256 spEthBalBefore = collToken.balanceOf(address(stabilityPool));
        liquidate(A, troveIDs.C);
        bool spTooLowAfterLiquidateA = stabilityPool.getTotalUsdxDeposits() < DECIMAL_PRECISION;
        ...
        assertGt(stabilityPool.getCompoundedUsdxDeposit(A), 0);
        assertGt(stabilityPool.getCompoundedUsdxDeposit(B), 0);

        testVars.deposit2_A = stabilityPool.getCompoundedUsdxDeposit(A);
        testVars.deposit2_B = stabilityPool.getCompoundedUsdxDeposit(B);
        ...
        transferUsdx(B, D, usdxToken.balanceOf(B));
        makeSPDepositAndClaim(D, usdxToken.balanceOf(D));
        testVars.totalSPBeforeLiq_D = stabilityPool.getTotalUsdxDeposits();
        ...
        assertApproximatelyEqual(testVars.expectedDeposit2_A, stabilityPool.getCompoundedUsdxDeposit(A), 1e18);
        assertApproximatelyEqual(testVars.expectedDeposit2_B, stabilityPool.getCompoundedUsdxDeposit(B), 1e18);
        assertGt(stabilityPool.getCompoundedUsdxDeposit(A), 0);
        assertGt(stabilityPool.getCompoundedUsdxDeposit(B), 0);
        ...
        // Check all USDX and Coll gains are as expected
        testVars.usdxGainA = stabilityPool.getDepositorYieldGain(A);
        testVars.usdxGainB = stabilityPool.getDepositorYieldGain(B);
        assertApproximatelyEqual(
            testVars.initialUsdxGainA + testVars.expectedShareOfYield1_A,
            testVars.usdxGainA,
            1e4,
            "A yield gain mismatch"
        );
        assertApproximatelyEqual(
            testVars.initialUsdxGainB + testVars.expectedShareOfYield1_B,
            testVars.usdxGainB,
            1e4,
    ...
    // --- Min. 1 USDX in SP tests ---

    // --- Liq debt x in range: (totalUsdxDeposits - 1e18) < x <= totalUsdxDeposits ---

    function testLiqDebtWithin1USDXOfTotalDepositsLeaves1USDXinSP_Fuzz(uint256 _toRedist) public {
        // The portion to redistribute is the portion of the debt that would otherwise eat into the
        // the last 1e18 USDX in the SP.
        ...
        // B's target debt should be: totalUsdxDeposits - 1e18 + _toRedist
        ...
        assertLt(stabilityPool.getTotalUsdxDeposits() - troveManager.getTroveEntireDebt(troveIdB), 1e18);
        ...
        assertEq(stabilityPool.getTotalUsdxDeposits(), 1e18);
        // A's deposit after liq should be <= totalUsdxDeposits, and very close to it, due to rounding error
        assertLe(stabilityPool.getCompoundedUsdxDeposit(A), stabilityPool.getTotalUsdxDeposits());
        assertApproximatelyEqual(stabilityPool.getCompoundedUsdxDeposit(A), stabilityPool.getTotalUsdxDeposits(), 1e3);
    }

    function testLiqDebtWithin1USDXOfTotalDepositsRedistributesRemainder_Fuzz(uint256 _toRedist) public {
        // The portion to redistribute is the portion of the debt that would otherwise eat into the
        // the last 1e18 USDX in the SP.
        // We bound this in range 0 < _toRedist <= 1e18.
        _toRedist = bound(_toRedist, 1, 1e18);
    ...
    function testLiqDebtWithin1UsdxOfTotalDepositsSplitsCollGainsCorrectly_Fuzz() public {
        ...
        // Confirm that B's entire debt is within 1e18 of SP total deposits
        assertLt(stabilityPool.getTotalUsdxDeposits() - vars.debtBBefore, 1e18);
    ...
    // --- Liq debt x in range: x < (totalUsdxDeposits - 1e18)

    function testLiqDebtLessThanSPMinus1USDXLeavesSurplusInSP_Fuzz(uint256 _surplusToLeaveInSP) public {
        ...
        // Confirm that B's entire debt is less than (totalUsdxDeposits - 1e18)
        assertLe(troveManager.getTroveEntireDebt(troveIdB), stabilityPool.getTotalUsdxDeposits() - 1e18);
        ...
        assertEq(stabilityPool.getTotalUsdxDeposits(), _surplusToLeaveInSP);
        // A's deposit after liq should be <= totalUsdxDeposits, and very close to it, due to rounding error
        assertLe(stabilityPool.getCompoundedUsdxDeposit(A), stabilityPool.getTotalUsdxDeposits());
        assertApproximatelyEqual(stabilityPool.getCompoundedUsdxDeposit(A), stabilityPool.getTotalUsdxDeposits(), 1e3);
    }

    function testLiqDebtLessThanSPMinus1USDXDoesNotRedistribute_Fuzz(uint256 _surplusToLeaveInSP) public {
        _surplusToLeaveInSP = bound(_surplusToLeaveInSP, 1e18, 2000e18);
    ...
    // --- Liq debt x for x > totalUsdxDeposits ---

    function testLiqDebtGreaterThanTotalSPLeaves1USDXInSP_Fuzz(uint256 _toRedist) public {
        ...
        // Confirm that B's entire debt is greater than totalUsdxDeposits
        assertGt(troveManager.getTroveEntireDebt(troveIdB), stabilityPool.getTotalUsdxDeposits());
    ...
    function testLiqDebtGreaterThanTotalSSplitsCollGainsCorrectly_Fuzz(uint256 _toRedist) public {
        ...
        uint256 spBefore = stabilityPool.getTotalUsdxDeposits();
        ...
        assertEq(stabilityPool.getTotalUsdxDeposits(), spBefore - liqDebtOffset);
    ...
    function testSingleDepositorCantWithdrawWhenLeavingLessThan1UsdxInPool_Fuzz(uint256 _withdrawal) public {
        priceFeed.setPrice(3000e18);
        ...
        assertEq(stabilityPool.getCompoundedUsdxDeposit(A), 2e18);
        assertEq(stabilityPool.getTotalUsdxDeposits(), 2e18);

        vm.startPrank(A);
        vm.expectRevert("Withdrawal must leave totalUsdxDeposits >= MIN_USDX_IN_SP");
        // Claim all gains here so they don't get added to deposit
        stabilityPool.withdrawFromSP(_withdrawal, true);
    }

    function testSingleDepositorCanWithdrawWhenLeavingAtLeast1UsdxInPool_Fuzz(uint256 _withdrawal) public {
        priceFeed.setPrice(3000e18);
        openTroveNoHints100pct(A, 5 ether, 2000e18, 5e16);
        ...
        uint256 ABalBefore = usdxToken.balanceOf(A);
        ...
        assertEq(stabilityPool.getCompoundedUsdxDeposit(A), 2e18 - _withdrawal);
        assertEq(usdxToken.balanceOf(A), ABalBefore + _withdrawal + accruedYieldGains);

        // Confirm SP has >= 1e18
        assertGe(stabilityPool.getTotalUsdxDeposits(), 1e18);
    }

    // Tests TODO:
    // - Collateral gains split correctly across mixed offset + redist
    // - Liquidations when SP.totalUsdxDeposits <1e18
    // - Depleted deposits can't withdraw if would leave <1e18 in SP
    // - When A can't withdraw, B can top up SP, then A can withdraw
    // - Usdx yield gains added to depsosit, allowing withdrawal
}
