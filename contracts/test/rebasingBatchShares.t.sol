...
contract RebasingBatchShares is DevTestSetup {
    ...
    // See: https://github.com/GalloDaSballo/usdx-review/issues/42
    ...
    function testBatchRebaseToSystemInsolvency() public {
        ...
        vm.startPrank(C);
        usdxToken.transfer(A, usdxToken.balanceOf(C));
        vm.stopPrank();
        ...
        uint256 balB4 = usdxToken.balanceOf(A);
        closeTrove(A, anotherATroveId);
        uint256 balAfter = usdxToken.balanceOf(A);
    ...
    function _addDebtAndEnsureItMintsShares(uint256 troveId, address caller, uint256 amt) internal {
        ...
        withdrawUsdx100pct(caller, troveId, amt);
    ...
    function testDeflateDebtLeavingSharesConstant() public {
        ...
        vm.startPrank(B);
        while (x++ < ITERATIONS) {
            borrowerOperations.repayUsdx(BTroveId, 1);
        }
        vm.stopPrank();
