...
contract ZapperLeverageMainnet is DevTestSetup {
    ...
    // Curve
    uint128 constant USDX_TOKEN_INDEX = 0;
    ...
    struct OpenTroveVars {
        ...
        uint256 expectedUsdxAmount;
        uint256 maxNetDebt;
        uint256 effectiveUsdxAmount;
        uint256 value;
        uint256 troveId;
    }
    ...
    function setUp() public override {
        ...
        hybridCurveUniV3ExchangeHelpers = new HybridCurveUniV3ExchangeHelpers(
            ...
            USDX_TOKEN_INDEX, // USDX Curve pool index
            UNIV3_FEE_USDC_WETH,
            UNIV3_FEE_WETH_COLL,
            uniV3Quoter
        );
    ...
    function fundCurveV2Pools(
        ...
        usdxAmount = usdcAmount * 1e12; // from 6 to 18 decimals
        deal(address(USDC), A, usdcAmount);
        deal(address(usdxToken), A, usdxAmount);
        vm.startPrank(A);
        // approve
        USDC.approve(address(usdcCurvePool), usdcAmount);
        usdxToken.approve(address(usdcCurvePool), usdxAmount);
        uint256[] memory amountsDynamic = new uint256[](2);
        amountsDynamic[0] = usdxAmount;
    ...
    function fundUniV3Pools(TestDeployer.LiquityContracts[] memory _contractsArray) internal {
        for (uint256 i = 0; i < NUM_COLLATERALS; i++) {
            ...
            // tokens and amounts
            uint256 collAmount = 1000 ether;
            uint256 usdxAmount = collAmount * price / DECIMAL_PRECISION;
            address[2] memory tokens;
            uint256[2] memory amounts;
            if (address(usdxToken) < address(_contractsArray[i].collToken)) {
                //console2.log("b < c");
                tokens[0] = address(usdxToken);
            ...
            usdxToken.approve(address(uniV3PositionManager), usdxAmount);

            // mint new position
            address uniV3PoolAddress =
                uniswapV3Factory.getPool(address(usdxToken), address(_contractsArray[i].collToken), UNIV3_FEE);
        ...
        returns (uint256, uint256)
    {
        ...
        vars.expectedUsdxAmount = vars.flashLoanAmount * vars.price / DECIMAL_PRECISION;
        vars.maxNetDebt = vars.expectedUsdxAmount * 105 / 100; // slippage
        vars.effectiveUsdxAmount = _getUsdxAmountToSwap(
            _inputParams.exchangeType,
            _inputParams.branch,
            vars.expectedUsdxAmount,
            vars.maxNetDebt,
            vars.flashLoanAmount,
        ...
        ILeverageZapper.OpenLeveragedTroveParams memory params = ILeverageZapper.OpenLeveragedTroveParams({
            ...
            usdxAmount: vars.effectiveUsdxAmount,
            upperHint: 0,
            lowerHint: 0,
        ...
        return (vars.troveId, vars.effectiveUsdxAmount);
    }
    ...
    function _setInitialBalances(ILeverageZapper _leverageZapper, uint256 _branch, TestVars memory vars)
        ...
        view
    {
        vars.usdxBalanceBeforeA = usdxToken.balanceOf(A);
        vars.ethBalanceBeforeA = A.balance;
        vars.collBalanceBeforeA = contractsArray[_branch].collToken.balanceOf(A);
        vars.usdxBalanceBeforeZapper = usdxToken.balanceOf(address(_leverageZapper));
        ...
        vars.usdxBalanceBeforeExchange = usdxToken.balanceOf(address(_leverageZapper.exchange()));
    ...
    function _testCanOpenTrove(
        ...
        // token balances
        assertEq(usdxToken.balanceOf(A), vars.usdxBalanceBeforeA, "USDX bal mismatch");
        assertEq(
            usdxToken.balanceOf(address(_leverageZapper)), vars.usdxBalanceBeforeZapper, "Zapper should not keep USDX"
        );
        assertEq(
            usdxToken.balanceOf(address(_leverageZapper.exchange())),
            vars.usdxBalanceBeforeExchange,
            "Exchange should not keep USDX"
        );
        assertEq(
    ...
    function _getLeverUpFlashLoanAndUsdxAmount(LeverUpParams memory _params) internal returns (uint256, uint256) {
        ...
        vars.expectedUsdxAmount = vars.flashLoanAmount * vars.price / DECIMAL_PRECISION;
        vars.maxNetDebtIncrease = vars.expectedUsdxAmount * 105 / 100; // slippage
        // The actual usdx we need, capped by the slippage above, to get flash loan amount
        vars.effectiveUsdxAmount = _getUsdxAmountToSwap(
    ...
    function leverUpTrove(LeverUpParams memory _params) internal returns (uint256, uint256) {
        ...
        return (flashLoanAmount, effectiveUsdxAmount);
    }
    ...
    function _testOnlyOwnerOrManagerCanLeverUpFromZapper(
        ...
        (uint256 flashLoanAmount, uint256 effectiveUsdxAmount) = _getLeverUpFlashLoanAndUsdxAmount(getterParams);
    ...
    function _getLeverDownFlashLoanAndUsdxAmount(
        ILeverageZapper _leverageZapper,
        uint256 _troveId,
        ...
        uint256 expectedUsdxAmount = flashLoanAmount * price / DECIMAL_PRECISION;
        uint256 minUsdxDebt = expectedUsdxAmount * 95 / 100; // slippage

        return (flashLoanAmount, minUsdxDebt);
    }
    ...
    function leverDownTrove(
        ...
        // This should be done in the frontend
        (uint256 flashLoanAmount, uint256 minUsdxDebt) =
            _getLeverDownFlashLoanAndUsdxAmount(_leverageZapper, _troveId, _leverageRatio, _troveManager, _priceFeed);

        ILeverageZapper.LeverDownTroveParams memory params = ILeverageZapper.LeverDownTroveParams({
            troveId: _troveId,
            flashLoanAmount: flashLoanAmount,
            minUsdxAmount: minUsdxDebt
        });
        vm.startPrank(A);
    ...
    function _testOnlyOwnerOrManagerCanLeverDownFromZapper(
        ...
        // B tries to lever up A’s trove
        (uint256 flashLoanAmount, uint256 minUsdxDebt) = _getLeverDownFlashLoanAndUsdxAmount(
            _leverageZapper,
            troveId,
    ...
    function _testCanCloseTrove(IZapper _zapper, uint256 _branch) internal {
        ...
        bool lst = _branch > 0;
        uint256 troveId = openTrove(_zapper, A, 0, collAmount, usdxAmount, lst);

        // open a 2nd trove so we can close the 1st one
        openTrove(_zapper, B, 0, 100 ether, 10000e18, lst);

        uint256 usdxBalanceBefore = usdxToken.balanceOf(A);
        ...
        assertGe(usdxToken.balanceOf(A), usdxBalanceBefore, "USDX bal should not decrease");
        assertLe(usdxToken.balanceOf(A), usdxBalanceBefore * 105 / 100, "USDX bal can only increase by slippage margin");
        if (lst) {
    ...
    // Helper to get the actual usdx we need, capped by a max value, to get flash loan amount
    function _getUsdxAmountToSwap(
        ExchangeType _exchangeType,
        uint256 _branch,
        uint256 _usdxAmount,
        ...
        if (_exchangeType == ExchangeType.Curve) {
            return _getUsdxAmountToSwapCurve(_branch, _usdxAmount, _maxUsdxAmount, _minCollAmount);
        }

        if (_exchangeType == ExchangeType.UniV3) {
            return _getUsdxAmountToSwapUniV3(_maxUsdxAmount, _minCollAmount, _collToken);
        }

        return _getUsdxAmountToSwapHybrid(_maxUsdxAmount, _minCollAmount, _collToken);
    }

    function _getUsdxAmountToSwapCurve(
        uint256 _branch,
        uint256 _usdxAmount,
        uint256 _maxUsdxAmount,
        uint256 _minCollAmount
        ...
        uint256 step = (_maxUsdxAmount - _usdxAmount) / 5; // In max 5 iterations we should reach the target, unless price is lower
        uint256 dy;
        // TODO: Optimizations: binary search, change the step depending on last dy, ...
        // Or check if there’s any helper implemented anywhere
        uint256 lastUsdxAmount = _maxUsdxAmount + step;
        do {
            lastUsdxAmount -= step;
            dy = curvePool.get_dy(USDX_TOKEN_INDEX, COLL_TOKEN_INDEX, lastUsdxAmount);
        } while (dy > _minCollAmount && lastUsdxAmount > step);

        uint256 usdxAmountToSwap = dy >= _minCollAmount ? lastUsdxAmount : lastUsdxAmount + step;
        require(usdxAmountToSwap <= _maxUsdxAmount, "Usdx amount required too high");

        return usdxAmountToSwap;
    }
    ...
    function _getUsdxAmountToSwapUniV3(uint256 _maxUsdxAmount, uint256 _minCollAmount, IERC20 _collToken)
        internal /* view */
        returns (uint256)
    {
        ...
        require(amountIn <= _maxUsdxAmount, "Price too high");

        return amountIn;
    }

    function _getUsdxAmountToSwapHybrid(uint256 _maxUsdxAmount, uint256 _minCollAmount, IERC20 _collToken)
        internal /* view */
        returns (uint256)
    {
        // Uniswap
        ...
        // Curve
        // USDC <- USDX
        uint256 usdxAmountToSwap = usdcCurvePool.get_dx(int128(USDX_TOKEN_INDEX), int128(USDC_INDEX), usdcAmount);
        require(usdxAmountToSwap <= _maxUsdxAmount, "Usdx amount required too high");

        usdxAmountToSwap = Math.min(usdxAmountToSwap * 101 / 100, _maxUsdxAmount); // TODO
    ...
    function _testHybridExchangeHelpers(
        ...
        (uint256 collAmount, uint256 slippage) =
            hybridCurveUniV3ExchangeHelpers.getCollFromUsdx(_usdxAmount, _collToken, _desiredCollAmount);
    ...
    function testHybridExchangeHelpersNoDeviation() public {
        ...
        (uint256 collAmount, uint256 slippage) =
            hybridCurveUniV3ExchangeHelpers.getCollFromUsdx(price, contractsArray[0].collToken, 0);
