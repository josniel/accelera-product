...
methods {
    ...
    // Not marked as view but it is almost one Updates a single state field lastUsdxLossError_Offset which is not relevant to these rules.
    function StabilityPool._computeCollRewardsPerUnitStaked(
        uint _collToAdd,
        uint _debtToOffset,
        uint _totalUsdxDeposits
    ) internal returns (uint, uint) => NONDET;

    // We ignore gas compensation as it is not related to these rules.
    function TroveManager._sendGasCompensation(address _activePool, address _liquidator, uint _usdx, uint _ETH) internal => NONDET;
...
// This is a helper method so that we can parameterize the rule
// for just the two calls related to collateral changes
function callDebtAdjustFunction(env e, method f, uint256 troveId, uint256 usdxAmount, uint256 maxUpfrontFee) {
    if(f.selector == sig:withdrawUsdx(uint256,uint256,uint256).selector) {
        withdrawUsdx(e, troveId, usdxAmount, maxUpfrontFee);
    }
    if(f.selector == sig:repayUsdx(uint256,uint256).selector) {
        repayUsdx(e, troveId, usdxAmount);
    }
}
...
rule sum_of_trove_debts {
    ...
    uint256 sum_trove_debt = require_uint256(
        troveDataX.entireDebt - troveDataX.redistUsdxDebtGain
        + troveDataY.entireDebt - troveDataY.redistUsdxDebtGain);
...
rule collateral_adjust_effects (method f) filtered {
    ...
    assert batchDataAfter.recordedDebt >= 
        ...
        // -Trove i’s pending redistribution debt gain is applied to the batch’s recorded debt
        troveDataBefore.redistUsdxDebtGain;
...
*/
// Related functions: withdrawUsdx, repayUsdx
// PASSING: https://prover.certora.com/output/65266/3edac4f9134b484bb112a585fa8268d6/?anonymousKey=041d059c08e8d9a08608bda2bd9411b75134ad10
rule debt_adjust_effects (method f) filtered {
    f -> f.selector == sig:withdrawUsdx(uint256,uint256,uint256).selector
    || f.selector == sig:repayUsdx(uint256,uint256).selector 
}{
    env e;
    uint256 troveId;
    uint256 usdxAmount;
    // Note: this is only used for withdrawUsdx
    uint256 maxUpfrontFee;
    ...
    callDebtAdjustFunction(e, f, troveId, usdxAmount, maxUpfrontFee);
    ...
    if(f.selector == sig:withdrawUsdx(uint256,uint256,uint256).selector) {
        assert batchDataAfter.recordedDebt >= 
            ...
            // adding the amount of usdx withdrawn
            batchDataBefore.recordedDebt +
            usdxAmount +
            // -All of the batch’s accrued management fee is applied to the batch’s recorded debt
            batchDataBefore.accruedManagementFee +
...
rule repay_debt_change {
    ...
    require troveDataBefore.entireDebt - usdxAmount >
        /* MIN_DEBT */
