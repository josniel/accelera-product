...
methods {
    ...
    function ActivePool._mintAggInterest(address _usdxToken, uint256 _upfrontFee) internal returns (uint256) => NONDET;
}
...
function troveBatchTroveEquivalent(env e, uint256 troveIdX, uint256 troveIdY) returns bool{
    ...
    return (troveManager.Troves[troveIdX].coll  == troveManager.Troves[troveIdY].coll   &&
            ...
            troveDataX.redistUsdxDebtGain       == troveDataY.redistUsdxDebtGain        &&
...
rule troveBatchTroveEquivalenceAddColl_recordedDebt(){
    ...
    // requiring this since redistUsdxDebtGain gets added to batch debt and then pro rated to batch trove
    require troveDataX_before.redistUsdxDebtGain == troveDataY_before.redistUsdxDebtGain * _batchDebtShares / _totalDebtShares;
...
rule troveBatchTroveEquivalenceAddColl_gainsColl(){
    ...
    assert troveDataX_after.redistUsdxDebtGain       == troveDataY_after.redistUsdxDebtGain,"debt gain should be the same";
...
rule troveBatchTroveEquivalenceWithdrawUsdx_recordedDebt(){
    env e;
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    ...
    withdrawUsdx(e, troveIdX, usdxAmount, maxUpfrontFeeX) at init;
    
    TroveManager.LatestTroveData troveDataX_after = troveManager.getLatestTroveData(e, troveIdX);
    
    uint256 maxUpfrontFeeY;
    withdrawUsdx(e, troveIdY, usdxAmount, maxUpfrontFeeY) at init;
...
rule troveBatchTroveEquivalenceWithdrawUsdx_gainsColl(){
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    require batchAddress != 0;
...
rule troveBatchTroveEquivalenceWithdrawUsdx_stake(){
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    require batchAddress != 0;
...
rule troveBatchTroveEquivalenceRepayUsdx_recordedDebt(){
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    require batchAddress != 0;
    ...
    storage init = lastStorage;
    uint256 usdxAmount;
    repayUsdx(e, troveIdX, usdxAmount) at init;
    
    TroveManager.LatestTroveData troveDataX_after = troveManager.getLatestTroveData(e, troveIdX);
    
    repayUsdx(e, troveIdY, usdxAmount) at init;
...
rule troveBatchTroveEquivalenceRepayUsdx_gainsColl(){
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    require batchAddress != 0;
...
rule troveBatchTroveEquivalenceRepayUsdx_stake(){
    uint256 troveIdX;
    uint256 troveIdY;
    address batchAddress;
    require batchAddress != 0;
