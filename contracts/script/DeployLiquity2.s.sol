...
contract DeployLiquity2Script is DeployGovernance, UniPriceConverter, StdCheats, MetadataDeployment, Logging {
    ...
    string constant DEPLOYMENT_MODE_USDX_ONLY = "usdx-only";
    string constant DEPLOYMENT_MODE_USE_EXISTING_USDX = "use-existing-usdx";
    ...
    // V1
    address ACCEL_ADDRESS = 0x6DEA81C8171D0bA574754EF6F8b412F2Ed88c54D;
    address ACCEL_STAKING_ADDRESS = 0x4f9Fbb3f1E99B56e0Fe2892e623Ed36A76Fc605d;
    address LUSD_ADDRESS = 0x5f98805A4E8be255a32880FDeC7F6728C6568bA0;

    address internal accel;
    ...
    function run() external {
        ...
        require(
            deploymentMode.eq(DEPLOYMENT_MODE_COMPLETE) || deploymentMode.eq(DEPLOYMENT_MODE_USDX_ONLY)
                || deploymentMode.eq(DEPLOYMENT_MODE_USE_EXISTING_USDX),
        ...
        // Deploy Usdx or pick up existing deployment
        bytes memory usdxBytecode = bytes.concat(type(UsdxToken).creationCode, abi.encode(deployer));
        address usdxAddress = vm.computeCreate2Address(SALT, keccak256(usdxBytecode));
        UsdxToken usdxToken;

        if (deploymentMode.eq(DEPLOYMENT_MODE_USE_EXISTING_USDX)) {
            require(usdxAddress.code.length > 0, string.concat("USDX not found at ", usdxAddress.toHexString()));
            usdxToken = UsdxToken(usdxAddress);

            // Check USDX is untouched
            require(usdxToken.totalSupply() == 0, "Some USDX has been minted!");
            require(usdxToken.collateralRegistryAddress() == address(0), "Collateral registry already set");
            require(usdxToken.owner() == deployer, "Not USDX owner");
        } else {
            usdxToken = new UsdxToken{salt: SALT}(deployer);
            assert(address(usdxToken) == usdxAddress);
        }

        if (deploymentMode.eq(DEPLOYMENT_MODE_USDX_ONLY)) {
            vm.writeFile("deployment-manifest.json", string.concat('{"usdxToken":"', usdxAddress.toHexString(), '"}'));
            return;
        }
        ...
        if (block.chainid == 1) {
            ...
            accel = ACCEL_ADDRESS;
            stakingV1 = ACCEL_STAKING_ADDRESS;
            lusd = LUSD_ADDRESS;
        } else {
            // sepolia, local
            ...
            accel = address(new ERC20Faucet("Liquity", "ACCEL", 100 ether, 1 days));
            lusd = address(new ERC20Faucet("Liquity USD", "LUSD", 100 ether, 1 days));
            stakingV1 = address(new MockStakingV1(IERC20_GOV(accel), IERC20_GOV(lusd)));

            // Let stakingV1 spend anyone's ACCEL without approval, like in the real ACCELStaking
            ERC20Faucet(accel).mock_setWildcardSpender(address(stakingV1), true);
        }
        ...
        if (block.chainid == 11155111) {
            ...
            _provideCurveLiquidity(deployed.usdxToken, deployed.contractsArray[0]);
        ...
        if (block.chainid == 1) {
            lusdCurvePool = _deployCurvePool(deployed.usdxToken, IERC20Metadata(LUSD_ADDRESS));
        }
    ...
    function _deployAndConnectContracts(
        ...
        r.usdxToken = UsdxToken(_deployGovernanceParams.usdx);

        // USDC and USDC-USDX pool
        r.usdcCurvePool = _deployCurvePool(r.usdxToken, USDC);
        ...
        r.collateralRegistry = new CollateralRegistry(r.usdxToken, vars.collaterals, vars.troveManagers);
        ...
        r.usdxToken.setCollateralRegistry(address(r.collateralRegistry));
    ...
    function _deployAndConnectCollateralContracts(
        ...
        (contracts.gasCompZapper, contracts.wethZapper, contracts.leverageZapper) =
            _deployZappers(contracts.addressesRegistry, contracts.collToken, _usdxToken, _usdcCurvePool);
    }
    ...
    function _deployCurvePool(IUsdxToken _usdxToken, IERC20Metadata _otherToken)
        internal
        returns (ICurveStableswapNGPool)
    {
        ...
        coins[USDX_TOKEN_INDEX] = address(_usdxToken);
        ...
        ICurveStableswapNGPool curvePool = curveStableswapFactory.deploy_plain_pool({
            name: string.concat("USDX/", _otherToken.symbol(), " Pool"),
            symbol: string.concat("USDX", _otherToken.symbol()),
            coins: coins,
            A: 100,
            fee: 4000000,
    ...
    function _mintUsdx(uint256 _usdxAmount, uint256 _price, LiquityContracts memory _contracts) internal {
        uint256 collAmount = _usdxAmount * 2 ether / _price; // CR of ~200%
    ...
    function _provideCurveLiquidity(IUsdxToken _usdxToken, LiquityContracts memory _contracts) internal {
        ICurveStableswapNGPool usdcCurvePool =
        ...
        returns (string memory)
    {
        ...
        return string.concat(
            ...
            string.concat(
                ...
                string.concat('"usdxToken":"', address(deployed.usdxToken).toHexString(), '",'),
