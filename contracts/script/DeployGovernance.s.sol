...
contract DeployGovernance is Script {
    ...
    ICurveStableSwapNG private curveUsdcUsdxPool;
    ILiquidityGaugeV6 private curveUsdcUsdxGauge;
    CurveV2GaugeRewards private curveUsdcUsdxInitiative;

    ICurveStableSwapNG private curveLusdUsdxPool;
    ILiquidityGaugeV6 private curveLusdUsdxGauge;
    CurveV2GaugeRewards private curveLusdUsdxInitiative;

    function deployGovernance(
        DeployGovernanceParams memory p,
        address _curveFactoryAddress,
        address _curveUsdcUsdxPoolAddress,
        address _curveLusdUsdxPoolAddress
        ...
        governance = new Governance{salt: p.salt}(
            p.accel, p.lusd, p.stakingV1, p.usdx, governanceConfiguration, p.deployer, initialInitiatives
        );

        assert(governanceAddress == address(governance));

        curveUsdcUsdxPool = ICurveStableSwapNG(_curveUsdcUsdxPoolAddress);
        curveLusdUsdxPool = ICurveStableSwapNG(_curveLusdUsdxPoolAddress);

        if (block.chainid == 1) {
            // mainnet
            (curveUsdcUsdxGauge, curveUsdcUsdxInitiative) = deployCurveV2GaugeRewards({
                _governance: governance,
                _usdx: p.usdx,
                _curveFactoryAddress: _curveFactoryAddress,
                _curvePool: curveUsdcUsdxPool
            });

            (curveLusdUsdxGauge, curveLusdUsdxInitiative) = deployCurveV2GaugeRewards({
                _governance: governance,
                _usdx: p.usdx,
                _curveFactoryAddress: _curveFactoryAddress,
                _curvePool: curveLusdUsdxPool
            });

            initialInitiatives.push(address(curveUsdcUsdxInitiative));
            initialInitiatives.push(address(curveLusdUsdxInitiative));
        ...
        returns (address, IGovernance.Configuration memory)
    {
        ...
        bytes memory bytecode = abi.encodePacked(
            ...
            abi.encode(p.accel, p.lusd, p.stakingV1, p.usdx, governanceConfiguration, p.deployer, new address[](0))
        );
    ...
    function deployCurveV2GaugeRewards(
        ...
        curveV2GaugeRewards = new CurveV2GaugeRewards(address(_governance), _usdx, CRV, address(gauge), DURATION);

        // add USDX as reward token
        gauge.add_reward(_usdx, address(curveV2GaugeRewards));
    ...
    function _getGovernanceManifestJson(DeployGovernanceParams memory p) internal view returns (string memory) {
        return string.concat(
            ...
            string.concat(
                ...
                string.concat('"curveUsdcUsdxPool":"', address(curveUsdcUsdxPool).toHexString(), '",'),
                string.concat('"curveUsdcUsdxGauge":"', address(curveUsdcUsdxGauge).toHexString(), '",'),
                string.concat('"curveUsdcUsdxInitiative":"', address(curveUsdcUsdxInitiative).toHexString(), '",'),
                string.concat('"curveLusdUsdxPool":"', address(curveLusdUsdxPool).toHexString(), '",'),
                string.concat('"curveLusdUsdxGauge":"', address(curveLusdUsdxGauge).toHexString(), '",'),
                string.concat('"curveLusdUsdxInitiative":"', address(curveLusdUsdxInitiative).toHexString(), '",')
            ),
            string.concat(
                ...
                string.concat('"ACCELToken":"', p.accel.toHexString(), '",'),
