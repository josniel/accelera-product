...
const accelToken = getContract({
  address: deploymentManifest.governance.ACCELToken,
  abi: abiERC20Faucet,
  client,
});

const usdxToken = getContract({
  address: deploymentManifest.usdxToken,
  abi: abiUsdxToken,
  client,
});
...
const mintUsdx = async (to: `0x${string}`, amount: bigint) => {
  ...
  await usdxToken.write.transfer([to, amount]).then(waitForSuccess);
};
...
const main = async () => {
  ...
  const accelAmount = BigInt(1_000e18);
  await accelToken.write.mint([governanceProxy.address, accelAmount]).then(waitForSuccess);
  await governanceProxy.write.depositACCEL([accelAmount]).then(waitForSuccess);
  await mintUsdx(governanceProxy.address, MIN_DEBT);
  ...
  const bribeInitiative = getContract({
    ...
    address: await client.deployContract({
      ...
      args: [deploymentManifest.governance.governance, usdxToken.address, accelToken.address],
    }).then(waitForContractAddress),
  });
  ...
  await governanceProxy.write.allocateACCEL([[], [bribeInitiative.address], [accelAmount], [0n]]).then(waitForSuccess);
};
