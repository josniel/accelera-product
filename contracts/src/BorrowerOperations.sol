...
contract BorrowerOperations is LiquityBase, AddRemoveManagers, IBorrowerOperations {
    ...
    */
    ...
    function withdrawUsdx(uint256 _troveId, uint256 _usdxAmount, uint256 _maxUpfrontFee) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);
    ...
    function repayUsdx(uint256 _troveId, uint256 _usdxAmount) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);
    ...
    function adjustTrove(
        ...
        TroveChange memory troveChange;
        _initTroveChange(troveChange, _collChange, _isCollIncrease, _usdxChange, _isDebtIncrease);
        _adjustTrove(troveManagerCached, _troveId, troveChange, _maxUpfrontFee);
    }
    ...
    */
    function _adjustTrove(
        ...
        if (_troveChange.debtDecrease > 0) {
            ...
            _requireSufficientUsdxBalance(vars.usdxToken, msg.sender, _troveChange.debtDecrease);
        }
        ...
        if (isTroveInBatch) {
            ...
            batchFutureDebt = batch.entireDebtWithoutRedistribution + vars.trove.redistUsdxDebtGain
        ...
        _moveTokensFromAdjustment(receiver, _troveChange, vars.usdxToken, vars.activePool);
    }
    ...
    function closeTrove(uint256 _troveId) external override {
        ...
        _requireSufficientUsdxBalance(usdxTokenCached, msg.sender, trove.entireDebt);
        ...
        if (batchManager != address(0)) {
            ...
            uint256 batchFutureDebt =
                batch.entireDebtWithoutRedistribution - (trove.entireDebt - trove.redistUsdxDebtGain);
    ...
    function applyPendingDebt(uint256 _troveId, uint256 _lowerHint, uint256 _upperHint) public {
        ...
        if (batchManager == address(0)) {
            ...
            change.newWeightedRecordedDebt =
                (batch.entireDebtWithoutRedistribution + trove.redistUsdxDebtGain) * batch.annualInterestRate;
            ...
            change.newWeightedRecordedBatchManagementFee =
                (batch.entireDebtWithoutRedistribution + trove.redistUsdxDebtGain) * batch.annualManagementFee;
        }
    ...
    function setInterestBatchManager(
        ...
        newBatchTroveChange.appliedRedistUsdxDebtGain = vars.trove.redistUsdxDebtGain;
    ...
    function removeFromBatch(
        ...
        uint256 batchFutureDebt =
            vars.batch.entireDebtWithoutRedistribution - (vars.trove.entireDebt - vars.trove.redistUsdxDebtGain);

        TroveChange memory batchChange;
        batchChange.appliedRedistUsdxDebtGain = vars.trove.redistUsdxDebtGain;
    ...
    function _requireSufficientUsdxBalance(IUsdxToken _usdxToken, address _borrower, uint256 _debtRepayment)
        internal
        view
    {
