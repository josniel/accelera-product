...
contract LeverageLSTZapper is GasCompZapper, ILeverageZapper {
        ...
        GasCompZapper(_addressesRegistry, _flashLoanProvider, _exchange)
    {
        ...
        // Approve Usdx to exchange module (Coll is approved in parent GasCompZapper)
        usdxToken.approve(address(_exchange), type(uint256).max);
    }
    ...
    function receiveFlashLoanOnOpenLeveragedTrove(
        ...
        // We compute usdxAmount off-chain for efficiency
        ...
        // Swap Usdx to Coll
        exchange.swapFromUsdx(_params.usdxAmount, _params.flashLoanAmount);
    ...
    function leverUpTrove(LeverUpTroveParams calldata _params) external {
        ...
        // Set initial balances to make sure there are not lefovers
        InitialBalances memory initialBalances;
        _setInitialTokensBalancesAndReceiver(collToken, usdxToken, initialBalances, receiver);
        ...
        override
    {
        ...
        // The frontend should calculate in advance the `_params.usdxAmount` needed for this to work
        exchange.swapFromUsdx(_params.usdxAmount, _params.flashLoanAmount);
        ...
        override
    {
        ...
        // Swap Coll from flash loan to Usdx, so we can repay and downsize trove
        // We swap the flash loan minus the flash loan fee
        // The frontend should calculate in advance the `_params.minUsdxAmount` to achieve the desired leverage ratio
        // (with some slippage tolerance)
        uint256 receivedUsdxAmount = exchange.swapToUsdx(_effectiveFlashLoanAmount, _params.minUsdxAmount);
