 ...
 * --- MIN USDX IN SP ---
 *
 * Once totalUsdxDeposits has become >= MIN_USDX_IN_SP, a liquidation may never fully empty the Pool - a minimum of 1 USDX remains in the SP at all times thereafter.
...
contract StabilityPool is LiquityBase, IStabilityPool, IStabilityPoolEvents {
    ...
    // Tracker for Usdx held in the pool. Changes when users deposit/withdraw, and when Trove debt is offset.
    uint256 internal totalUsdxDeposits;

    // Total remaining Usdx yield gains (from Trove interest mints) held by SP, and not yet paid out to depositors
    // From the contract's perspective, this is a write-only variable.
    uint256 internal yieldGainsOwed;
    // Total remaining Usdx yield gains (from Trove interest mints) held by SP, not yet paid out to depositors,
    ...
    struct Snapshots {
        ...
        uint256 P;
        uint256 B; // Usdx reward sum from minted interest
        uint256 scale;
    }
    ...
    * after a series of liquidations have occurred, each of which cancel some Usdx debt with the deposit.
    *
    ...
    */
    ...
    function getTotalUsdxDeposits() external view override returns (uint256) {
        return totalUsdxDeposits;
    }
    ...
    */
    function provideToSP(uint256 _topUp, bool _doClaim) external override {
        ...
        uint256 newDeposit = compoundedUsdxDeposit + _topUp + keptYieldGain;
        ...
        emit DepositOperation(
            ...
            initialDeposit - compoundedUsdxDeposit,
            int256(_topUp),
            currentYieldGain,
            yieldGainToSend,
            currentCollGain,
        ...
        usdxToken.sendToPool(msg.sender, address(this), _topUp);
        _updateTotalUsdxDeposits(_topUp + keptYieldGain, 0);
        _decreaseYieldGainsOwed(currentYieldGain);
        _sendUsdxtoDepositor(msg.sender, yieldGainToSend);
    ...
    */
    function withdrawFromSP(uint256 _amount, bool _doClaim) external override {
        ...
        uint256 currentYieldGain = getDepositorYieldGain(msg.sender);
        uint256 compoundedUsdxDeposit = getCompoundedUsdxDeposit(msg.sender);
        uint256 usdxToWithdraw = LiquityMath._min(_amount, compoundedUsdxDeposit);
        (uint256 keptYieldGain, uint256 yieldGainToSend) = _getYieldToKeepOrSend(currentYieldGain, _doClaim);
        uint256 newDeposit = compoundedUsdxDeposit - usdxToWithdraw + keptYieldGain;
        (uint256 newStashedColl, uint256 collToSend) =
            _getNewStashedCollAndCollToSend(msg.sender, currentCollGain, _doClaim);
        ...
        _updateDepositAndSnapshots(msg.sender, newDeposit, newStashedColl);
        _decreaseYieldGainsOwed(currentYieldGain);
        uint256 newTotalUsdxDeposits = _updateTotalUsdxDeposits(keptYieldGain, usdxToWithdraw);
        _sendUsdxtoDepositor(msg.sender, usdxToWithdraw + yieldGainToSend);
        _sendCollGainToDepositor(collToSend);

        require(newTotalUsdxDeposits >= MIN_USDX_IN_SP, "Withdrawal must leave totalUsdxDeposits >= MIN_USDX_IN_SP");
    }
    ...
    function _updateYieldRewardsSum(uint256 _newYield) internal {
        ...
        // When total deposits is very small, B is not updated. In this case, the USDX issued is held
        // until the total deposits reach 1 USDX (remains in the balance of the SP).
        if (totalUsdxDeposits < MIN_USDX_IN_SP) {
        ...
        scaleToB[currentScale] += P * accumulatedYieldGains / totalUsdxDeposits;
    ...
    */
    function offset(uint256 _debtToOffset, uint256 _collToAdd) external override {
        ...
        scaleToS[currentScale] += P * _collToAdd / totalUsdxDeposits;
        emit S_Updated(scaleToS[currentScale], currentScale);

        uint256 numerator = P * (totalUsdxDeposits - _debtToOffset);
        uint256 newP = numerator / totalUsdxDeposits;

        // For `P` to turn zero, `totalUsdxDeposits` has to be greater than `P * (totalUsdxDeposits - _debtToOffset)`.
        // - As the offset must leave at least 1 USDX in the SP (MIN_USDX_IN_SP),
        //   the minimum value of `totalUsdxDeposits - _debtToOffset` is `1e18`
        // - It can be shown that `P` is always in range (1e27, 1e36].
        // Thus, to turn `P` zero, `totalUsdxDeposits` has to be greater than `(1e27 + 1) * 1e18`,
        // and the offset has to be (near) maximal.
        // In other words, there needs to be octillions of USDX in the SP, which is unlikely to happen in practice.
        require(newP > 0, "P must never decrease to 0");

        // Overflow analyisis of scaling up P:
        // We know that the resulting P is <= 1e36, and it's the result of dividing numerator by totalUsdxDeposits.
        // Thus, numerator <= 1e36 * totalUsdxDeposits, so unless totalUsdxDeposits is septillions of USDX, it won’t overflow.
        // That holds on every iteration as an upper bound. We multiply numerator by SCALE_FACTOR,
        // but numerator is by definition smaller than 1e36 * totalUsdxDeposits / SCALE_FACTOR.
        while (newP < P_PRECISION / SCALE_FACTOR) {
            numerator *= SCALE_FACTOR;
            newP = numerator / totalUsdxDeposits;
            currentScale += 1;
    ...
    function _moveOffsetCollAndDebt(uint256 _collToAdd, uint256 _debtToOffset) internal {
        // Cancel the liquidated Usdx debt with the Usdx in the stability pool
        _updateTotalUsdxDeposits(0, _debtToOffset);

        // Burn the debt that was successfully offset
        usdxToken.burn(address(this), _debtToOffset);
    ...
    function _updateTotalUsdxDeposits(uint256 _depositIncrease, uint256 _depositDecrease) internal returns (uint256) {
        if (_depositIncrease == 0 && _depositDecrease == 0) return totalUsdxDeposits;
        uint256 newTotalUsdxDeposits = totalUsdxDeposits + _depositIncrease - _depositDecrease;
        totalUsdxDeposits = newTotalUsdxDeposits;

        emit StabilityPoolUsdxBalanceUpdated(newTotalUsdxDeposits);
        return newTotalUsdxDeposits;
    }
    ...
    function getDepositorYieldGainWithPending(address _depositor) external view override returns (uint256) {
        if (totalUsdxDeposits < MIN_USDX_IN_SP) return 0;
        ...
        if (currentScale <= snapshots.scale + SCALE_SPAN) {
            normalizedGains += P * pendingSPYield / totalUsdxDeposits / SCALE_FACTOR ** (currentScale - snapshots.scale);
        }
    ...
    function getCompoundedUsdxDeposit(address _depositor) public view override returns (uint256 compoundedDeposit) {
        uint256 initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) return 0;
    ...
    // --- Sender functions for Usdx deposit and Coll gains ---
    ...
    // Send Usdx to user and decrease Usdx in Pool
    function _sendUsdxtoDepositor(address _depositor, uint256 _usdxToSend) internal {
        if (_usdxToSend == 0) return;
        usdxToken.returnFromPool(address(this), _depositor, _usdxToSend);
    }
