...
contract TroveManager is LiquityBase, ITroveManager, ITroveEvents {
    ...
    **/
    ...
    function _liquidate(
        ...
        _movePendingTroveRewardsToActivePool(_defaultPool, trove.redistUsdxDebtGain, trove.redistCollGain);
        ...
        ) = _getOffsetAndRedistributionVals(trove.entireDebt, collToLiquidate, _usdxInSPForOffsets, _price);
        ...
        if (isTroveInBatch) {
            singleLiquidation.oldWeightedRecordedDebt =
                batch.weightedRecordedDebt + (trove.entireDebt - trove.redistUsdxDebtGain) * batch.annualInterestRate;
            ...
            troveChange.oldWeightedRecordedBatchManagementFee = batch.weightedRecordedBatchManagementFee
                + (trove.entireDebt - trove.redistUsdxDebtGain) * batch.annualManagementFee;
    ...
    */
        ...
        )
    {
        ...
        if (_usdxInSPForOffsets > 0) {
            debtToOffset = LiquityMath._min(_entireTroveDebt, _usdxInSPForOffsets);
    ...
    function batchLiquidateTroves(uint256[] memory _troveArray) public override {
        ...
        uint256 usdxToLeaveInSP = LiquityMath._min(MIN_USDX_IN_SP, totalUsdxDeposits);
        uint256 usdxInSPForOffsets = totalUsdxDeposits - usdxToLeaveInSP;

        // Perform the appropriate liquidation sequence - tally values and obtain their totals.
        _batchLiquidateTroves(defaultPoolCached, price, usdxInSPForOffsets, _troveArray, totals, troveChange);
    ...
    function _batchLiquidateTroves(
        ...
        for (uint256 i = 0; i < _troveArray.length; i++) {
            ...
            if (ICR < MCR) {
                ...
                _liquidate(_defaultPool, troveId, remainingUsdxInSPForOffsets, _price, trove, singleLiquidation);
    ...
    function _movePendingTroveRewardsToActivePool(IDefaultPool _defaultPool, uint256 _usdx, uint256 _coll) internal {
        if (_usdx > 0) {
    ...
    function _applySingleRedemption(
        ...
        uint256 newDebt = _singleRedemption.trove.entireDebt - _singleRedemption.usdxLot;
        uint256 newColl = _singleRedemption.trove.entireColl - _singleRedemption.collLot;

        _singleRedemption.appliedRedistUsdxDebtGain = _singleRedemption.trove.redistUsdxDebtGain;

        if (_isTroveInBatch) {
            _getLatestBatchData(_singleRedemption.batchAddress, _singleRedemption.batch);
            // We know usdxLot <= trove entire debt, so this subtraction is safe
            uint256 newAmountForWeightedDebt = _singleRedemption.batch.entireDebtWithoutRedistribution
                + _singleRedemption.trove.redistUsdxDebtGain - _singleRedemption.usdxLot;
            ...
            TroveChange memory troveChange;
            troveChange.debtDecrease = _singleRedemption.usdxLot;
            troveChange.collDecrease = _singleRedemption.collLot;
            troveChange.appliedRedistUsdxDebtGain = _singleRedemption.trove.redistUsdxDebtGain;
            ...
            // This call could revert due to BatchSharesRatioTooHigh if trove.redistCollGain > usdxLot
        ...
        _movePendingTroveRewardsToActivePool(
            _defaultPool, _singleRedemption.trove.redistUsdxDebtGain, _singleRedemption.trove.redistCollGain
        );
    ...
    function _redeemCollateralFromTrove(
        ...
        _singleRedemption.usdxLot = LiquityMath._min(_maxUsdxamount, _singleRedemption.trove.entireDebt);

        // Get the amount of Coll equal in USD value to the usdxLot redeemed
        uint256 correspondingColl = _singleRedemption.usdxLot * DECIMAL_PRECISION / _price;
    ...
    */
    function redeemCollateral(
        ...
        while (singleRedemption.troveId != 0 && remainingUsdx > 0 && _maxIterations > 0) {
            _maxIterations--;
            ...
            _redeemCollateralFromTrove(defaultPool, singleRedemption, remainingUsdx, _price, _redemptionRate);

            totalsTroveChange.collDecrease += singleRedemption.collLot;
            totalsTroveChange.debtDecrease += singleRedemption.usdxLot;
            totalsTroveChange.appliedRedistUsdxDebtGain += singleRedemption.appliedRedistUsdxDebtGain;
        ...
        emit Redemption(
            _usdxamount, totalsTroveChange.debtDecrease, totalsTroveChange.collDecrease, totalCollFee, _price
        );
    ...
    function _urgentRedeemCollateralFromTrove(
        ...
        _singleRedemption.collLot = _singleRedemption.usdxLot * (DECIMAL_PRECISION + URGENT_REDEMPTION_BONUS) / _price;
    ...
    function urgentRedemption(uint256 _usdxAmount, uint256[] calldata _troveIds, uint256 _minCollateral) external {
        _requireIsShutDown();
        ...
        for (uint256 i = 0; i < _troveIds.length; i++) {
            ...
            _urgentRedeemCollateralFromTrove(defaultPool, remainingUsdx, price, singleRedemption);
        ...
        emit Redemption(_usdxAmount, totalsTroveChange.debtDecrease, totalsTroveChange.collDecrease, 0, price);
    ...
    function _getLatestTroveData(uint256 _troveId, LatestTroveData memory trove) internal view {
        ...
        trove.redistUsdxDebtGain = stake * (L_usdxDebt - rewardSnapshots[_troveId].usdxDebt) / DECIMAL_PRECISION;
        ...
        trove.entireDebt = trove.recordedDebt + trove.redistUsdxDebtGain + trove.accruedInterest;
    ...
    function _getLatestTroveDataFromBatch(
        ...
        _latestTroveData.redistUsdxDebtGain =
            stake * (L_usdxDebt - rewardSnapshots[_troveId].usdxDebt) / DECIMAL_PRECISION;
        ...
        _latestTroveData.entireDebt = _latestTroveData.recordedDebt + _latestTroveData.redistUsdxDebtGain
    ...
    function _redistributeDebtAndColl(
        ...
        */
        ...
        uint256 usdxDebtNumerator = _debtToRedistribute * DECIMAL_PRECISION + lastUsdxDebtError_Redistribution;
        ...
        lastUsdxDebtError_Redistribution = usdxDebtNumerator - usdxDebtRewardPerUnitStaked * totalStakes;
    ...
    */
    ...
    function _requireUsdxBalanceCoversRedemption(IUsdxToken _usdxToken, address _redeemer, uint256 _amount)
        internal
        view
    {
    ...
    function onSetInterestBatchManager(OnSetInterestBatchManagerParams calldata _params) external {
        ...
        _troveChange.debtIncrease = _params.troveDebt - _troveChange.appliedRedistUsdxDebtGain - _troveChange.upfrontFee;
    ...
    function _updateBatchShares(
        ...
        uint256 debtIncrease =
            _troveChange.debtIncrease + _troveChange.upfrontFee + _troveChange.appliedRedistUsdxDebtGain;
    ...
    function _removeTroveSharesFromBatch(
        ...
        uint256 batchDebtDecrease = _newTroveDebt - _troveChange.upfrontFee - _troveChange.appliedRedistUsdxDebtGain;
