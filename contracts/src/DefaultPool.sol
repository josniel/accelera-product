...
/*
 * The Default Pool holds the Coll and Usdx debt (but not Usdx tokens) from liquidations that have been redistributed
...
contract DefaultPool is IDefaultPool {
    ...
    event DefaultPoolUsdxDebtUpdated(uint256 _usdxDebt);
    ...
    */
    ...
    function getUsdxDebt() external view override returns (uint256) {
        return UsdxDebt;
    }
    ...
    function increaseUsdxDebt(uint256 _amount) external override {
        _requireCallerIsTroveManager();
        UsdxDebt = UsdxDebt + _amount;
        emit DefaultPoolUsdxDebtUpdated(UsdxDebt);
    }

    function decreaseUsdxDebt(uint256 _amount) external override {
        _requireCallerIsTroveManager();
        UsdxDebt = UsdxDebt - _amount;
        emit DefaultPoolUsdxDebtUpdated(UsdxDebt);
    }
