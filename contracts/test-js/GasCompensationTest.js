...
contract("Gas compensation tests", async (accounts) => {
  ...
  it("getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves", async () => {
    ...
    // E opens with 4405.45 ETH, 32598.35 Usdx
  ...
  it("Gas compensation from pool-offset liquidations. All collateral paid as compensation", async () => {
    ...
    const { troveId: carolTroveId, totalDebt: C_totalDebt } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUsdxAmount: dec(300, 18),
      extraParams: { from: carol },
    });
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUsdxAmount: A_totalDebt,
      extraParams: { from: dennis },
    });
    await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUsdxAmount: B_totalDebt.add(C_totalDebt),
      extraParams: { from: erin },
    });

    // D, E each provide Usdx to SP
    ...
    const UsdxinSP_0 = await stabilityPool.getTotalUsdxDeposits();
    ...
    // Check SP Usdx has decreased due to the liquidation
    const UsdxinSP_A = await stabilityPool.getTotalUsdxDeposits();
    assert.isTrue(UsdxinSP_A.lte(UsdxinSP_0));
    ...
    // Check SP Usdx has decreased due to the liquidation of B
    const UsdxinSP_B = await stabilityPool.getTotalUsdxDeposits();
    assert.isTrue(UsdxinSP_B.lt(UsdxinSP_A));
    ...
    // Check SP Usdx has decreased due to the liquidation of C
    const UsdxinSP_C = await stabilityPool.getTotalUsdxDeposits();
    assert.isTrue(UsdxinSP_C.lt(UsdxinSP_B));
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder", async () => {
    ...
    const { troveId: bobTroveId } = await openTrove({
      ICR: toBN(dec(120, 16)),
      extraUsdxAmount: dec(5000, 18),
      extraParams: { from: bob },
    });
    await openTrove({
      ICR: toBN(dec(60, 18)),
      extraUsdxAmount: dec(600, 18),
      extraParams: { from: carol },
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
      extraUsdxAmount: dec(1, 23),
      extraParams: { from: dennis },
    });
    await openTrove({
      ICR: toBN(dec(80, 18)),
    ...
    // D, E each provide 10000 Usdx to SP
    ...
    // Check SP Usdx has decreased due to the liquidation of A
    const UsdxinSP_A = await stabilityPool.getTotalUsdxDeposits();
    assert.isTrue(UsdxinSP_A.lt(UsdxinSP_0));
  ...
  it("gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder", async () => {
    ...
    const { troveId: aliceTroveId } = await openTrove({
      ICR: toBN(dec(2, 18)),
      extraUsdxAmount: dec(2000, 18),
      extraParams: { from: alice },
    });
    const { troveId: bobTroveId } = await openTrove({
      ICR: toBN(dec(1875, 15)),
      extraUsdxAmount: dec(8000, 18),
      extraParams: { from: bob },
    });
    await openTrove({
      ICR: toBN(dec(2, 18)),
  ...
  it("Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR", async () => {
    ...
    let coll = 50;
    // create 20 troves, increasing collateral, constant debt = 100Usdx
