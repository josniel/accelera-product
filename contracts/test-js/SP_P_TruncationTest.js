...
const UsdxToken = artifacts.require("UsdxToken");
...
contract("StabilityPool Scale Factor issue tests", async (accounts) => {
  ...
  const getUsdxAmountForDesiredDebt = async (desiredDebt) => (await getOpenTroveUsdxAmount(dec(desiredDebt, 18)));
  ...
  describe("Scale Factor issue tests", async () => {
    it("1. Liquidation succeeds after 2 high-fraction liquidations", async () => {
      // Whale opens Trove with 100k ETH and sends 50k Usdx to A
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      });
      await usdxToken.transfer(A, dec(50000, 18), { from: whale });

      // Open 3 Troves with 2000 Usdx debt
      for (const account of [A, B, C]) {
        await th.openTroveWrapper(contracts, await getUsdxAmountForDesiredDebt(2000), account, account, 0, {
          from: account,
          value: dec(15, "ether"),
        });
    ...
    it("2. New deposits can be made after 2 high fraction liquidations", async () => {
      ...
      for (let i = 0; i < 3; i++) {
        await usdxToken.transfer(newDepositors[i], newDeposits[i], {
          from: whale,
        });
        ...
        assert.isTrue(
          (await stabilityPool.getCompoundedUsdxDeposit(newDepositors[i])).eq(
            newDeposits[i],
          ),
        );
      }
    ...
    it("3. Liquidation succeeds when P == 1e9 liquidation has newProductFactor == 1e9", async () => {
      ...
      const deposit_2 = deposit_0
        .sub(await stabilityPool.getTotalUsdxDeposits())
    ...
    it("5. Depositor have correct depleted stake after deposit at P == 1e9 and scale changing liq (with newProductFactor == 1e9)", async () => {
      ...
      // D makes deposit of 1000 Usdx
      const D_deposit = dec(1, 21);
      await usdxToken.transfer(D, dec(1, 21), { from: whale });
      ...
      const D_depletedDeposit = await stabilityPool.getCompoundedUsdxDeposit(D);
    ...
    it("7. Two consecutive liquidations when P == 1e9 and liquidations have newProductFactor > 1e9 prematurely deplete deposits with minimal (and capped) collateral reward", async () => {
      ...
      await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(100000, 18)),
        whale,
        whale,
        0,
      ...
      await usdxToken.transfer(A, dec(50000, 18), { from: whale });
      // Open 4 Troves with 2000 USDX debt
      for (account of [A, B, C, D]) {
        await th.openTroveWrapper(
          contracts,
          await getOpenTroveUsdxAmount(dec(2000, 18)),
          account,
          account,
          0,
      ...
      // Open 1 Trove with 16000 USDX debt
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(16001, 18)), E, E, 0, {
        from: E,
        value: dec(1215, 17),
      });
      ...
      console.log(scale); // 1
      // A re-fills SP back up to deposit 0 level, i.e. just enough to reduce P by 1e9 from a 2k debt liq.
      const deposit_1 = deposit_0.sub(await stabilityPool.getTotalUsdxDeposits());
      ...
      const deposit_2 = deposit_0.mul(th.toBN(10)).sub(await stabilityPool.getTotalUsdxDeposits());
      ...
      console.log(scale); // 4
      assert.equal(scale, "4");
      const A_Deposit_Value = await stabilityPool.getCompoundedUsdxDeposit(A);
      console.log("A's compounded USDX deposit:");
      ...
      // // Here we will cache the amount of raw ETH in the stability pool contract, the totalUSDXDeposits, and the epoch counter.
      ...
      const Total_USDX_Deposits = await stabilityPool.getTotalUsdxDeposits();
      console.log("Total Stability Pool USDX Deposits before complete liquidation LIQ4:");
      console.log(Total_USDX_Deposits.toString()); // 16000.000000000000020010
      ...
      // // The liquidation will go through because 81% of A's deposit is still a part of the variable 'totalUSDXDeposits'.
      ...
      await priceFeed.setPrice(dec(200, 18));
      // // Check: we can check that the full trove was indeed offset, by ensuring that the pool's USDX balance is now 0,
      ...
      const Total_USDX_Deposits_2 = await stabilityPool.getTotalUsdxDeposits();
      console.log("Total Stability Pool USDX Deposits after complete liquidation LIQ4:");
      console.log(Total_USDX_Deposits_2.toString()); // 0
      assert.equal(Total_USDX_Deposits_2, 0);
      ...
      const A_Deposit_Value_2 = await stabilityPool.getCompoundedUsdxDeposit(A);
      console.log("A's compounded USDX deposit after complete liquidation LIQ4:");
