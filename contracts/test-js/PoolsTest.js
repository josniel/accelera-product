...
contract("StabilityPool", async (accounts) => {
  ...
  it("getTotalUsdxDeposits(): gets the recorded USDX balance", async () => {
    const stabilityPool = await loadFixture(deployFixture);
    const recordedCollBalance = await stabilityPool.getTotalUsdxDeposits();
    assert.equal(recordedCollBalance, 0);
  });
});
...
contract("ActivePool", async (accounts) => {
  ...
  it("getUsdxDebt(): gets the total USDX debt balance", async () => {
    const recordedUsdxDebt = await activePool.getUsdxDebt();
    assert.equal(recordedUsdxDebt, 0);
  });
...
contract("DefaultPool", async (accounts) => {
  ...
  it("getCollBalance(): gets the recorded USDX balance", async () => {
    const recordedCollBalance = await defaultPool.getCollBalance();
    assert.equal(recordedCollBalance, 0);
  });

  it("getUsdxDebt(): gets the recorded USDX balance", async () => {
    const recordedUsdxDebt = await defaultPool.getUsdxDebt();
    assert.equal(recordedUsdxDebt, 0);
  });

  it("increaseUsdx(): increases the recorded USDX balance by the correct amount", async () => {
    const recordedUsdx_balanceBefore = await defaultPool.getUsdxDebt();
    assert.equal(recordedUsdx_balanceBefore, 0);

    // await defaultPool.increaseUsdxDebt(100, { from: mockTroveManagerAddress })
    const increaseUsdxDebtData = th.getTransactionData("increaseUsdxDebt(uint256)", ["0x64"]);
    const tx = await mockTroveManager.forward(defaultPool.address, increaseUsdxDebtData);
    assert.isTrue(tx.receipt.status);

    const recordedUsdx_balanceAfter = await defaultPool.getUsdxDebt();
    assert.equal(recordedUsdx_balanceAfter, 100);
  });

  it("decreaseUsdx(): decreases the recorded USDX balance by the correct amount", async () => {
    // start the pool on 100 wei
    // await defaultPool.increaseUsdxDebt(100, { from: mockTroveManagerAddress })
    const increaseUsdxDebtData = th.getTransactionData("increaseUsdxDebt(uint256)", ["0x64"]);
    const tx1 = await mockTroveManager.forward(defaultPool.address, increaseUsdxDebtData);
    assert.isTrue(tx1.receipt.status);

    const recordedUsdx_balanceBefore = await defaultPool.getUsdxDebt();
    assert.equal(recordedUsdx_balanceBefore, 100);

    // await defaultPool.decreaseUsdxDebt(100, { from: mockTroveManagerAddress })
    const decreaseUsdxDebtData = th.getTransactionData("decreaseUsdxDebt(uint256)", ["0x64"]);
    const tx2 = await mockTroveManager.forward(defaultPool.address, decreaseUsdxDebtData);
    assert.isTrue(tx2.receipt.status);

    const recordedUsdx_balanceAfter = await defaultPool.getUsdxDebt();
    assert.equal(recordedUsdx_balanceAfter, 0);
  });
