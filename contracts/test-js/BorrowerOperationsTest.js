  ...
  const testCorpus = (withBatchDelegation = false) => {
    ...
    describe(`${withBatchDelegation ? "With" : "Without"} delegation`, async () => {
      ...
      it("addColl(), active Trove: applies pending rewards and updates user's L_coll, L_usdxDebt snapshots", async () => {
        ...
        // price drops to 1ETH:100Usdx, reducing Carol's ICR below MCR
        ...
        const L_usdxDebt = await troveManager.get_L_usdxDebt();
        ...
        const alice_UsdxDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1];
        ...
        const bob_UsdxDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1];

        assert.equal(alice_ETHrewardSnapshot_Before, 0);
        assert.equal(alice_UsdxDebtRewardSnapshot_Before, 0);
        assert.equal(bob_ETHrewardSnapshot_Before, 0);
        assert.equal(bob_UsdxDebtRewardSnapshot_Before, 0);
        ...
        const alicePendingUsdxDebtReward = await troveManager.getPendingUsdxDebtReward(aliceTroveId);
        const bobPendingUsdxDebtReward = await troveManager.getPendingUsdxDebtReward(bobTroveId);
        for (
          const reward of [
            alicePendingETHReward,
            bobPendingETHReward,
            alicePendingUsdxDebtReward,
            bobPendingUsdxDebtReward,
          ]
        ) {
        ...
        assert.isTrue(
          aliceNewDebt.eq(aliceDebtBefore.add(alicePendingUsdxDebtReward)),
        );
        assert.isTrue(
          bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)),
        );
        assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingUsdxDebtReward)));

        /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
           to the latest values of L_coll and L_usdxDebt */
        ...
        const alice_UsdxDebtRewardSnapshot_After = alice_rewardSnapshot_After[1];
        ...
        const bob_UsdxDebtRewardSnapshot_After = bob_rewardSnapshot_After[1];
        ...
        assert.isAtMost(
          th.getDifference(alice_UsdxDebtRewardSnapshot_After, L_usdxDebt),
          100,
        );
        assert.isAtMost(
        ...
        assert.isAtMost(
          th.getDifference(bob_UsdxDebtRewardSnapshot_After, L_usdxDebt),
          100,
        );
      });
      ...
      //   // --- SETUP ---
      //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 Usdx
      ...
      //   // close Carol's Trove, liquidating her 5 ether and 900Usdx.
      ...
      it("withdrawColl(): reverts if system is below CT", async () => {
        ...
        // price drops to 1ETH:150Usdx, reducing TCR below 150%
      ...
      it("withdrawColl(): applies pending rewards and updates user's L_coll, L_usdxDebt snapshots", async () => {
        // --- SETUP ---
        ...
        // close Carol's Trove, liquidating her 1 ether and 180Usdx.
        ...
        const pendingDebtReward_A = await troveManager.getPendingUsdxDebtReward(aliceTroveId);
        const pendingCollReward_B = await troveManager.getPendingCollReward(bobTroveId);
        const pendingDebtReward_B = await troveManager.getPendingUsdxDebtReward(bobTroveId);
        for (
          const reward of [
            pendingCollReward_A,
            pendingDebtReward_A,
      ...
      it("withdrawUsdx(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
        // alice creates a Trove and adds first collateral
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        const Usdxwithdrawal = 1; // withdraw 1 wei Usdx

        await assertRevert(
          borrowerOperations.withdrawUsdx(
            aliceTroveId,
            Usdxwithdrawal,
      ...
      it("withdrawUsdx(): reverts when calling address does not have active trove", async () => {
        await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        const { troveId: bobTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });

        // Bob successfully withdraws Usdx
        const txBob = await borrowerOperations.withdrawUsdx(
          bobTroveId,
          dec(100, 18),
          th.MAX_UINT256, // _maxUpfrontFee
          { from: bob },
        ...
        // Carol with no active trove attempts to withdraw Usdx
        try {
          const txCarol = await borrowerOperations.withdrawUsdx(
            th.addressToTroveId(carol),
            dec(100, 18),
            th.MAX_UINT256, // _maxUpfrontFee
            { from: carol },
      ...
      it("withdrawUsdx(): reverts when requested withdrawal amount is zero Usdx", async () => {
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        const { troveId: bobTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });

        // Bob successfully withdraws 1e-18 Usdx
        const txBob = await borrowerOperations.withdrawUsdx(
          bobTroveId,
          1,
          th.MAX_UINT256, // _maxUpfrontFee
        ...
        // Alice attempts to withdraw 0 Usdx
        try {
          const txAlice = await borrowerOperations.withdrawUsdx(
            aliceTroveId,
            0,
            th.MAX_UINT256, // _maxUpfrontFee
            { from: alice },
      ...
      it("withdrawUsdx(): reverts when system is below CT", async () => {
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        const { troveId: bobTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        // Check Usdx withdrawal impossible when below CT
        try {
          const txBob = await borrowerOperations.withdrawUsdx(
            bobTroveId,
            1,
      ...
      it("withdrawUsdx(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
        await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        // Bob tries to withdraw Usdx that would bring his ICR < MCR
        try {
          const txBob = await borrowerOperations.withdrawUsdx(
            bobTroveId,
            1,
      ...
      it("withdrawUsdx(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
        await priceFeed.setPrice(dec(100, 18));
        const price = await priceFeed.getPrice();
        ...
        // Bob attempts to withdraw 1 Usdx.
      ...
      it("withdrawUsdx(): reverts if system is below CT", async () => {
        // --- SETUP ---
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        try {
          const txData = await borrowerOperations.withdrawUsdx(
            aliceTroveId,
            "200",
            th.MAX_UINT256, // _maxUpfrontFee
            { from: alice },
      ...
      it("withdrawUsdx(): increases user UsdxToken balance by correct amount", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraParams: { value: toBN(dec(100, "ether")), from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });

        // check before
        const alice_UsdxTokenBalance_Before = await usdxToken.balanceOf(alice);
        assert.isTrue(alice_UsdxTokenBalance_Before.gt(toBN("0")));

        await borrowerOperations.withdrawUsdx(
          aliceTroveId,
          dec(10000, 18),
          th.MAX_UINT256,
          { from: alice },
        ...
        // check after
        const alice_UsdxTokenBalance_After = await usdxToken.balanceOf(alice);
        assert.isTrue(
          alice_UsdxTokenBalance_After.eq(
            alice_UsdxTokenBalance_Before.add(toBN(dec(10000, 18))),
          ),
        );
      });

      // --- repayUsdx() ---
      it("repayUsdx(): reverts when repayment would leave trove with ICR < MCR", async () => {
        // alice creates a Trove and adds first collateral
        const { troveId: aliceTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        const UsdxRepayment = 1; // 1 wei repayment

        await assertRevert(
          borrowerOperations.repayUsdx(aliceTroveId, UsdxRepayment, {
            from: alice,
          }),
          "BorrowerOps: An operation that would result in ICR < MCR is not permitted",
        );
      ...
      it("repayUsdx(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
        // Make the Usdx request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
        ...
        const repayTxA = await borrowerOperations.repayUsdx(ATroveId, 1, { from: A });
        ...
        const repayTxB = await borrowerOperations.repayUsdx(BTroveId, dec(19, 25), {
          from: B,
        });
        assert.isTrue(repayTxB.receipt.status);
      });

      it("repayUsdx(): reverts when it would leave trove with net debt < minimum net debt", async () => {
        ...
        const repayTxAPromise = borrowerOperations.repayUsdx(ATroveId, 2, {
          from: A,
        });
        await assertRevert(
          repayTxAPromise,
      ...
      it("repayUsdx(): reverts when calling address does not have active trove", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        // Bob successfully repays some Usdx
        const txBob = await borrowerOperations.repayUsdx(bobTroveId, dec(10, 18), {
          from: bob,
        });
        assert.isTrue(txBob.receipt.status);

        // Carol with no active trove attempts to repayUsdx
        try {
          const txCarol = await borrowerOperations.repayUsdx(
            th.addressToTroveId(carol),
            dec(10, 18),
            { from: carol },
          );
      ...
      it("repayUsdx(): reverts when attempted repayment is > the debt of the trove", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        try {
          const txAlice = await borrowerOperations.repayUsdx(
            aliceTroveId,
      ...
      // repayUsdx: reduces Usdx debt in Trove
      it("repayUsdx(): reduces the Trove's Usdx debt by the correct amount", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        await borrowerOperations.repayUsdx(
          aliceTroveId,
      ...
      it("repayUsdx(): decreases Usdx debt in ActivePool by correct amount", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        // Check before
        const activePool_Usdx_Before = await activePool.getUsdxDebt();
        assert.isTrue(activePool_Usdx_Before.gt(toBN("0")));
        ...
        // check after
        const activePool_Usdx_After = await activePool.getUsdxDebt();
        th.assertIsApproximatelyEqual(
          activePool_Usdx_After,
          activePool_Usdx_Before.sub(aliceDebtBefore.div(toBN(10))),
        );
      });

      it("repayUsdx(): decreases user UsdxToken balance by correct amount", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        const alice_UsdxTokenBalance_After = await usdxToken.balanceOf(alice);
        th.assertIsApproximatelyEqual(
          alice_UsdxTokenBalance_After,
          alice_UsdxTokenBalance_Before.sub(aliceDebtBefore.div(toBN(10))),
        );
      });

      it("repayUsdx(): can repay debt below CT", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        const tx = await borrowerOperations.repayUsdx(
          aliceTroveId,
          aliceDebtBefore.div(toBN(10)),
          { from: alice },
        );
      ...
      it("repayUsdx(): Reverts if borrower has insufficient Usdx balance to cover his debt repayment", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        const bobBalBefore = await usdxToken.balanceOf(B);
        assert.isTrue(bobBalBefore.gt(toBN("0")));

        // Bob transfers all but 5 of his Usdx to Carol
        await usdxToken.transfer(C, bobBalBefore.sub(toBN(dec(5, 18))), {
          from: B,
        });

        // Confirm B's Usdx balance has decreased to 5 Usdx
        const bobBalAfter = await usdxToken.balanceOf(B);

        assert.isTrue(bobBalAfter.eq(toBN(dec(5, 18))));

        // Bob tries to repay 6 Usdx
        const repayUsdxPromise_B = borrowerOperations.repayUsdx(BTroveId, toBN(dec(6, 18)), { from: B });

        await assertRevert(
          repayUsdxPromise_B,
          "Caller doesnt have enough Usdx to make repayment",
        );
      });
      ...
      it("adjustTrove(): reverts when calling address has no active trove", async () => {
        ...
        await openTrove({
          extraUsdxAmount: toBN(dec(20000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        // Alice coll and debt increase(+1 ETH, +50Usdx)
      ...
      it("adjustTrove(): reverts when Usdx repaid is > debt of the trove", async () => {
        await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        const { troveId: bobTroveId } = await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) } });
        ...
        // Alice transfers 1 Usdx to bob
        await usdxToken.transfer(bob, th.toBN(dec(1, 18)), { from: alice });
      ...
      it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
        ...
        // Bob attempts to increase debt by 100 Usdx and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
      ...
      it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
        ...
        // Alice adjusts trove. No coll change, and a debt increase (+50Usdx)
        await borrowerOperations.adjustTrove(
          aliceTroveId,
          0,
          false,
      ...
      it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
        ...
        const aliceDebtBefore = await getTroveEntireDebt(aliceTroveId);
        const activePoolDebtBefore = await activePool.getUsdxDebt();
        ...
        const aliceDebtAfter = await getTroveEntireDebt(aliceTroveId);
        const activePoolDebtAfter = await activePool.getUsdxDebt();
      ...
      it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
        ...
        // Alice adjusts trove. Coll and debt increase(+1 ETH, +50Usdx)
        // approve ERC20 ETH
        await contracts.WETH.approve(borrowerOperations.address, dec(1, 24), { from: alice });
        await borrowerOperations.adjustTrove(
          aliceTroveId,
      ...
      it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
        ...
        // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50Usdx)
        await borrowerOperations.adjustTrove(
          aliceTroveId,
          dec(500, "finney"),
          false,
      ...
      it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
        ...
        // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50Usdx)
        // approve ERC20 ETH
        await contracts.WETH.approve(borrowerOperations.address, dec(1, 24), { from: alice });
        await borrowerOperations.adjustTrove(
          aliceTroveId,
      ...
      it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
        ...
        // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10Usdx)
        await borrowerOperations.adjustTrove(
          aliceTroveId,
          dec(1, 17),
          false,
      ...
      it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
        ...
        // Alice adjusts trove - coll and debt increase (+1 ETH, +50 Usdx)
        // approve ERC20 ETH
        await contracts.WETH.approve(borrowerOperations.address, dec(1, 24), { from: alice });
        await borrowerOperations.adjustTrove(
          aliceTroveId,
      ...
      it("adjustTrove(): changes UsdxToken balance by the requested decrease", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        assert.isTrue(
          alice_UsdxTokenBalance_After.eq(
            alice_UsdxTokenBalance_Before.sub(toBN(dec(10, 18))),
          ),
        );
      });

      it("adjustTrove(): changes UsdxToken balance by the requested increase", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        assert.isTrue(
          alice_UsdxTokenBalance_After.eq(
            alice_UsdxTokenBalance_Before.add(toBN(dec(100, 18))),
          ),
        );
      });
      ...
      it("adjustTrove(): Changes the Usdx debt in ActivePool by requested decrease", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        const activePool_UsdxDebt_Before = await activePool.getUsdxDebt();
        assert.isTrue(activePool_UsdxDebt_Before.gt(toBN("0")));
        ...
        const activePool_UsdxDebt_After = await activePool.getUsdxDebt();
        assert.isTrue(
          activePool_UsdxDebt_After.eq(
            activePool_UsdxDebt_Before.sub(toBN(dec(30, 18))),
          ),
        );
      });

      it("adjustTrove(): Changes the Usdx debt in ActivePool by requested increase", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        th.assertIsApproximatelyEqual(
          activePool_UsdxDebt_After,
          activePool_UsdxDebt_Before.add(toBN(dec(100, 18))),
        );
      });
      ...
      it("adjustTrove(): Reverts if borrower has insufficient Usdx balance to cover his debt repayment", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        // Bob transfers some Usdx to carol
        await usdxToken.transfer(C, dec(10, 18), { from: B });

        // Confirm B's Usdx balance is less than 50 Usdx
        const B_UsdxBal = await usdxToken.balanceOf(B);
        assert.isTrue(B_UsdxBal.lt(bobDebt));

        const repayUsdxPromise_B = borrowerOperations.adjustTrove(
          BTroveId,
          0,
          false,
          bobDebt,
        ...
        await assertRevert(repayUsdxPromise_B, "revert");
      });
      ...
      it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
        ...
        await usdxToken.transfer(alice, dec(300, 18), { from: bob });
      ...
      it("closeTrove(): reverts when system is below CT", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(100000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: bobTroveId } = await openTrove({
        ...
        // Alice transfers her Usdx to Bob and Carol so they can cover fees
        const aliceBal = await usdxToken.balanceOf(alice);
        await usdxToken.transfer(bob, aliceBal.div(toBN(2)), { from: alice });
        await usdxToken.transfer(carol, aliceBal.div(toBN(2)), { from: alice });
      ...
      it("closeTrove(): reduces a Trove's collateral to zero", async () => {
        ...
        const aliceCollBefore = await getTroveEntireColl(aliceTroveId);
        const dennisUsdx = await usdxToken.balanceOf(dennis);
        assert.isTrue(aliceCollBefore.gt(toBN("0")));
        assert.isTrue(dennisUsdx.gt(toBN("0")));

        // To compensate borrowing fees
        await usdxToken.transfer(alice, dennisUsdx.div(toBN(2)), {
          from: dennis,
        });
      ...
      it("closeTrove(): zero's the troves reward snapshots", async () => {
        ...
        const L_usdxDebt_Snapshot_A_AfterLiquidation = (
          await troveManager.rewardSnapshots(aliceTroveId)
        )[1];

        assert.isTrue(L_coll_Snapshot_A_AfterLiquidation.gt(toBN("0")));
        assert.isTrue(L_usdxDebt_Snapshot_A_AfterLiquidation.gt(toBN("0")));

        // to compensate borrowing fees
        await usdxToken.transfer(alice, await usdxToken.balanceOf(dennis), {
          from: dennis,
        });
        ...
        const L_usdxDebt_Snapshot_A_afterAliceCloses = (
          await troveManager.rewardSnapshots(aliceTroveId)
        )[1];

        assert.equal(L_coll_Snapshot_A_afterAliceCloses, "0");
        assert.equal(L_usdxDebt_Snapshot_A_afterAliceCloses, "0");
      });
      ...
      it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
        ...
        // Check after
        const activePool_Debt_After = (await activePool.getUsdxDebt()).toString();
      ...
      it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's UsdxToken balance", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(10000, 18)),
          ICR: toBN(dec(2, 18)),
          extraParams: { from: dennis, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        const alice_UsdxBalance_Before = await usdxToken.balanceOf(alice);
        assert.isTrue(alice_UsdxBalance_Before.gt(toBN("0")));

        // close trove
        await borrowerOperations.closeTrove(aliceTroveId, { from: alice });

        // check alice Usdx balance after
        const alice_UsdxBalance_After = await usdxToken.balanceOf(alice);
        th.assertIsApproximatelyEqual(
          alice_UsdxBalance_After,
          alice_UsdxBalance_Before.sub(aliceDebt),
        );
      });
      ...
      it("closeTrove(): applies pending rewards", async () => {
        ...
        const { troveId: whaleTroveId } = await openTrove({
          extraUsdxAmount: toBN(dec(1000000, 18)),
          ICR: toBN(dec(10, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        const defaultPool_UsdxDebt = await defaultPool.getUsdxDebt();

        // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
        assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100);
        assert.isAtMost(
          th.getDifference(defaultPool_UsdxDebt, liquidatedDebt_C),
          100,
        );
        ...
        const defaultPool_UsdxDebt_afterAliceCloses = await defaultPool.getUsdxDebt();
        ...
        assert.isAtMost(
          th.getDifference(
            defaultPool_UsdxDebt_afterAliceCloses,
            defaultPool_UsdxDebt.sub(pendingDebtReward_A),
          ),
          1000,
        );
        ...
        const defaultPool_UsdxDebt_afterBobCloses = await defaultPool.getUsdxDebt();
        ...
        assert.isAtMost(
          th.getDifference(defaultPool_UsdxDebt_afterBobCloses, 0),
          100000,
        );
      });

      it("closeTrove(): reverts if borrower has insufficient Usdx balance to repay his entire debt", async () => {
        await openTrove({
          extraUsdxAmount: toBN(dec(15000, 18)),
          ICR: toBN(dec(2, 18)),
        ...
        // B sends away 1 Usdx to A
        await usdxToken.transfer(A, th.toBN(dec(1, 18)), { from: B });

        // Confirm B's Usdx balance is less than his trove debt
        const B_UsdxBal = await usdxToken.balanceOf(B);
        const B_troveDebt = await getTroveEntireDebt(BTroveId);

        assert.isTrue(B_UsdxBal.lt(B_troveDebt));
        ...
        await assertRevert(
          closeTrovePromise_B,
          "BorrowerOps: Caller doesnt have enough Usdx to make repayment",
        );
      });
      ...
      it("openTrove(): Cannot open a trove with ICR == CCR when system is below CT", async () => {
        ...
        // price drops to 1ETH:100Usdx, reducing TCR below 150%
        await priceFeed.setPrice("100000000000000000000");
        const price = await priceFeed.getPrice();
      ...
      it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
        ...
        */
        ...
        const UsdxRequest = MIN_DEBT;
        const aliceTroveId = await th.openTroveWrapper(contracts, MIN_DEBT, carol, carol, 0, {
          from: alice,
          value: dec(100, "ether"),
          batchManager: getBatchManager(withBatchDelegation, dennis) });

        // Get the expected debt based on the Usdx request (adding fee and liq. reserve on top)
        const expectedDebt = UsdxRequest;
      ...
      it("openTrove(): records up-to-date initial snapshots of L_coll and L_usdxDebt", async () => {
        // --- SETUP ---
        ...
        /* with total stakes = 10 ether, after liquidation, L_coll should equal 1/10 ether per-ether-staked,
           and L_Usdx should equal 18 Usdx per-ether-staked. */

        const L_coll = await troveManager.get_L_coll();
        const L_Usdx = await troveManager.get_L_usdxDebt();

        assert.isTrue(L_coll.gt(toBN("0")));
        assert.isTrue(L_Usdx.gt(toBN("0")));
      ...
      it("openTrove(): increases the Trove's Usdx debt by the correct amount", async () => {
        // check before
      ...
      it("openTrove(): increases Usdx debt in ActivePool by the debt of the trove", async () => {
        const activePool_UsdxDebt_Before = await activePool.getUsdxDebt();
        assert.equal(activePool_UsdxDebt_Before, 0);
        ...
        const activePool_UsdxDebt_After = await activePool.getUsdxDebt();
        assert.isTrue(activePool_UsdxDebt_After.eq(aliceDebt));
      });

      it("openTrove(): increases user UsdxToken balance by correct amount", async () => {
        // check before
        const alice_UsdxTokenBalance_Before = await usdxToken.balanceOf(alice);
        assert.equal(alice_UsdxTokenBalance_Before, 0);
        ...
        // check after
        const alice_UsdxTokenBalance_After = await usdxToken.balanceOf(alice);
        assert.equal(alice_UsdxTokenBalance_After, dec(10000, 18));
      });
      ...
      describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
        ...
        it("collChange = 0, debtChange = 0", async () => {
          ...
          const bobUsdxAmount = await getOpenTroveUsdxAmount(bobTotalDebt);
          const whaleUsdxAmount = await getOpenTroveUsdxAmount(whaleTotalDebt);

          await th.openTroveWrapper(contracts, whaleUsdxAmount, whale, whale, 0, {
            from: whale,
            value: whaleColl,
            batchManager: getBatchManager(withBatchDelegation, dennis) });
          const bobTroveId = await th.openTroveWrapper(contracts, bobUsdxAmount, bob, bob, 0, {
            from: bob,
            value: bobColl,
            batchManager: getBatchManager(withBatchDelegation, dennis) });
...
/* TODO:
   ...
   changes with addColl, withdrawColl, withdrawUsdx, repayUsdx, etc. Can split them up and put them with
