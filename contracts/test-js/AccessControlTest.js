...
contract(
  ...
  async (accounts) => {
    ...
    describe("DefaultPool", async (accounts) => {
      ...
      // increaseUsdx
      it("increaseUsdxDebt(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.increaseUsdxDebt(100, {
            from: alice,
          });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      // decreaseUsdx
      it("decreaseUsdx(): reverts when called by an account that is not TroveManager", async () => {
        // Attempt call from alice
        try {
          const txAlice = await defaultPool.decreaseUsdxDebt(100, {
            from: alice,
          });
        } catch (err) {
          assert.include(err.message, "revert");
    ...
    describe("UsdxToken", async (accounts) => {
      //    mint
      it("mint(): reverts when called by an account that is not BorrowerOperations", async () => {
        // Attempt call from alice
        const txAlice = usdxToken.mint(bob, 100, { from: alice });
      ...
      it("burn(): reverts when called by an account that is not BO nor TroveM nor SP", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdxToken.burn(bob, 100, { from: alice });
        } catch (err) {
          assert.include(err.message, "revert");
      ...
      it("sendToPool(): reverts when called by an account that is not StabilityPool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdxToken.sendToPool(
            bob,
            activePool.address,
            100,
            { from: alice },
      ...
      it("returnFromPool(): reverts when called by an account that is not TroveManager nor StabilityPool", async () => {
        // Attempt call from alice
        try {
          const txAlice = await usdxToken.returnFromPool(
            activePool.address,
            bob,
            100,
            { from: alice },
