...
contract("TroveManager - Redistribution reward calculations", async (accounts) => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    */
    ...
    assert.equal((await usdxToken.balanceOf(owner)).toString(), "0");
  });
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
    ...
    */
    ...
    assert.equal(
      (await usdxToken.balanceOf(owner)).toString(),
      "0",
    );
  });
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and UsdxDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 Usdx (10 each for gas compensation)
    const bob_Coll = (await troveManager.Troves(bobTroveId))[1]
      .add(await troveManager.getPendingCollReward(bobTroveId))
      .toString();

    const bob_UsdxDebt = (await troveManager.Troves(bobTroveId))[0]
      .add(await troveManager.getPendingUsdxDebtReward(bobTroveId))
      .toString();
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
       L1: 1/2*0.995 ETH, 55 Usdx
       L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 UsdxDebt

       coll: 3.1195 ETH
       debt: 233.77 UsdxDebt

       Alice rewards:
       L1 1/2*0.995 ETH, 55 Usdx
       L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 UsdxDebt

       coll: 1.8705 ETH
       debt: 146.23 UsdxDebt

       totalColl: 4.99 ETH
       totalDebt 380 Usdx (includes 50 each for gas compensation)
    */
    const bob_Coll = (await troveManager.Troves(bobTroveId))[1]
      .add(await troveManager.getPendingCollReward(bobTroveId))
      .toString();
    ...
    const alice_UsdxDebt = (await troveManager.Troves(aliceTroveId))[0]
      .add(await troveManager.getPendingUsdxDebtReward(aliceTroveId))
      .toString();
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000);
    assert.isAtMost(th.getDifference(bob_UsdxDebt, expected_B_debt), 10000);
    ...
    assert.isAtMost(th.getDifference(alice_UsdxDebt, expected_A_debt), 10000);
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and UsdxDebt in the system: 2.5 Ether and 300 Usdx
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    const entireSystemDebt = (await activePool.getUsdxDebt()).add(
