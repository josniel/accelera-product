...
contract("TroveManager", async (accounts) => {
  ...
  const getOpenTroveTotalDebt = async (usdxAmount) => th.getOpenTroveTotalDebt(contracts, usdxAmount);
  ...
  const withdrawUsdx = async (params) => th.withdrawUsdx(contracts, params);
  ...
  const testCorpus = (withBatchDelegation = false) => {
    ...
    describe(`${withBatchDelegation ? "With" : "Without"} delegation`, async () => {
      it("liquidate(): closes a Trove that has ICR < MCR", async () => {
        ...
        // Alice increases debt to 180 Usdx, lowering her ICR to 1.11
        const A_UsdxWithdrawal = await getNetBorrowingAmount(dec(130, 18));

        const targetICR = toBN("1111111111111111111");
        await withdrawUsdx({ ICR: targetICR, extraParams: { from: alice } });
        ...
        // price drops to 1ETH:100Usdx, reducing Alice's ICR below MCR
      ...
      it("liquidate(): decreases ActivePool ETH and UsdxDebt by correct amounts", async () => {
        // --- SETUP ---
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(4, 18)),
        ...
        // check ActivePool Coll and Usdx debt before
        ...
        const activePool_UsdxDebt_Before = (
          await activePool.getUsdxDebt()
        ).toString();
        ...
        th.assertIsApproximatelyEqual(
          activePool_UsdxDebt_Before,
          A_totalDebt.add(B_totalDebt),
        );

        // price drops to 1ETH:100Usdx, reducing Bob's ICR below MCR
        await priceFeed.setPrice("100000000000000000000");

        // Confirm system is not below CT
        assert.isFalse(await th.checkBelowCriticalThreshold(contracts));

        /* close Bob's Trove. Should liquidate his ether and Usdx,
           leaving Alice’s ether and Usdx debt in the ActivePool. */
        await troveManager.liquidate(bobTroveId, { from: owner });

        // check ActivePool ETH and Usdx debt
        const activePool_coll_After = (await activePool.getCollBalance()).toString();
        const activePool_RawEther_After = (
          await contracts.WETH.balanceOf(activePool.address)
        ).toString();
        const activePool_UsdxDebt_After = (
          await activePool.getUsdxDebt()
        ).toString();

        assert.equal(activePool_coll_After, A_collateral);
        assert.equal(activePool_RawEther_After, A_collateral);
        th.assertIsApproximatelyEqual(activePool_UsdxDebt_After, A_totalDebt, 1e13);
      });

      it("liquidate(): increases DefaultPool ETH and Usdx debt by correct amounts", async () => {
        // --- SETUP ---
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(4, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        ...
        // check DefaultPool ETH and Usdx debt before
        ...
        const defaultPool_UsdxDebt_Before = (
          await defaultPool.getUsdxDebt()
        ).toString();

        assert.equal(defaultPool_coll_Before, "0");
        assert.equal(defaultPool_RawEther_Before, "0");
        assert.equal(defaultPool_UsdxDebt_Before, "0");
        ...
        const defaultPool_UsdxDebt_After = (
          await defaultPool.getUsdxDebt()
        ).toString();
        ...
        th.assertIsApproximatelyEqual(defaultPool_UsdxDebt_After, B_totalDebt, 1e13);
      });
      ...
      it("liquidate(): updates the snapshots of total stakes and total collateral", async () => {
        ...
        // close Bob's Trove.  His ether*0.995 and Usdx should be added to the DefaultPool.
        await troveManager.liquidate(bobTroveId, { from: owner });
      ...
      it("liquidate(): updates the L_coll and L_usdxDebt reward-per-unit-staked totals", async () => {
        // --- SETUP ---
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(8, 18)),
          extraParams: {
        ...
        // price drops to 1ETH:100Usdx, reducing Carols's ICR below MCR
        await priceFeed.setPrice("100000000000000000000");
        ...
        // Carol's ether*0.995 and Usdx should be added to the DefaultPool.
        const L_coll_AfterCarolLiquidated = await troveManager.get_L_coll();
        const L_usdxDebt_AfterCarolLiquidated = await troveManager.get_L_usdxDebt();

        const L_coll_expected_1 = th
          .applyLiquidationFee(C_collateral)
          .mul(mv._1e18BN)
          .div(A_collateral.add(B_collateral));
        const L_usdxDebt_expected_1 = C_totalDebt.add(carolInterest).add(carolFee).mul(mv._1e18BN).div(
        ...
        assert.isAtMost(
          th.getDifference(L_usdxDebt_AfterCarolLiquidated, L_usdxDebt_expected_1),
          100,
        );
        ...
        // Bob now withdraws Usdx, bringing his ICR to 1.11
        ...
        // price drops to 1ETH:50Usdx, reducing Bob's ICR below MCR
        ...
        /* Alice now has all the active stake. totalStakes in the system is now 10 ether.
           ...
           L_usdxDebt = (180 / 20) + (890 / 10) = 98 Usdx */
        const L_coll_AfterBobLiquidated = await troveManager.get_L_coll();
        const L_usdxDebt_AfterBobLiquidated = await troveManager.get_L_usdxDebt();
        ...
        const L_usdxDebt_expected_2 = L_usdxDebt_expected_1.add(
          B_totalDebt.add(B_increasedTotalDebt).add(bobInterest1).add(bobFee1).add(bobInterest2).add(bobFee2)
            .add(B_collateral.mul(L_usdxDebt_expected_1).div(mv._1e18BN))
            .mul(mv._1e18BN)
            .div(A_collateral),
        );
        assert.isAtMost(
        ...
        assert.isAtMost(
          th.getDifference(L_usdxDebt_AfterBobLiquidated, L_usdxDebt_expected_2),
          1e11,
        );
      });
      ...
      it("liquidate(): Liquidates undercollateralized trove if there are two troves in the system", async () => {
        ...
        // Alice creates a single trove with 0.7 ETH and a debt of 70 Usdx, and provides 10 Usdx to SP
        const { troveId: aliceTroveId, collateral: A_collateral, totalDebt: A_totalDebt } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, janet) },
        });

        // Alice proves 10 Usdx to SP
      ...
      it("liquidate(): Given the same price and no other trove changes, complete Pool offsets restore the TCR to its value prior to the defaulters opening troves", async () => {
        // Whale provides Usdx to SP
        const spDeposit = toBN(dec(100, 24));
        await openTrove({
          ICR: toBN(dec(4, 18)),
          extraUsdxAmount: spDeposit,
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
      ...
      it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
        ...
        const { troveId: carolTroveId, C_totalDebt, C_collateral } = await openTrove({
          ICR: toBN(dec(218, 16)),
          extraUsdxAmount: toBN(dec(100, 18)),
          extraParams: { from: carol, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });

        // Bob sends tokens to Dennis, who has no trove
        await usdxToken.transfer(dennis, spDeposit, { from: bob });

        // Dennis provides Usdx to SP
        ...
        const dennis_Deposit_Before = (
          await stabilityPool.getCompoundedUsdxDeposit(dennis)
        ).toString();
      ...
      it("liquidate(): does not liquidate a SP depositor's trove with ICR > 110%, and does not affect their SP deposit or ETH gain", async () => {
        ...
        const bob_Deposit_Before = (
          await stabilityPool.getCompoundedUsdxDeposit(bob)
        ).toString();
      ...
      it("liquidate(): liquidates a SP depositor's trove with ICR < 110%, and the liquidation correctly impacts their SP deposit and ETH gain", async () => {
        ...
        // Check Bob' SP deposit has absorbed Carol's debt, and he has received her liquidated ETH
        const bob_Deposit_Before = await stabilityPool.getCompoundedUsdxDeposit(
          bob,
        );
        ...
        /* Alice's Usdx Loss = (300 / 400) * 200 = 150 Usdx
           Alice's ETH gain = (300 / 400) * 2*0.995 = 1.4925 ETH

           Bob's UsdxLoss = (100 / 400) * 200 = 50 Usdx
           Bob's ETH gain = (100 / 400) * 2*0.995 = 0.4975 ETH

           Check Bob' SP deposit has been reduced to 50 Usdx, and his ETH gain has increased to 1.5 ETH. */
        const alice_Deposit_After = (
          await stabilityPool.getCompoundedUsdxDeposit(alice)
        ).toString();
        ...
        const bob_Deposit_After = await stabilityPool.getCompoundedUsdxDeposit(bob);
      ...
      it("liquidate(): does not alter the liquidated user's token balance", async () => {
        ...
        const { troveId: aliceTroveId, usdxAmount: A_usdxAmount } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: toBN(dec(300, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: bobTroveId, usdxAmount: B_usdxAmount } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: toBN(dec(200, 18)),
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: carolTroveId, usdxAmount: C_usdxAmount } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: toBN(dec(100, 18)),
          extraParams: { from: carol, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        // Liquidate A, B and C
        const activeUsdxDebt_0 = await activePool.getUsdxDebt();
        const defaultUsdxDebt_0 = await defaultPool.getUsdxDebt();

        await troveManager.liquidate(aliceTroveId);
        const activeUsdxDebt_A = await activePool.getUsdxDebt();
        const defaultUsdxDebt_A = await defaultPool.getUsdxDebt();

        await troveManager.liquidate(bobTroveId);
        const activeUsdxDebt_B = await activePool.getUsdxDebt();
        const defaultUsdxDebt_B = await defaultPool.getUsdxDebt();
        ...
        assert.equal((await usdxToken.balanceOf(alice)).toString(), A_usdxAmount);
        assert.equal((await usdxToken.balanceOf(bob)).toString(), B_usdxAmount);
        assert.equal((await usdxToken.balanceOf(carol)).toString(), C_usdxAmount);
      });
      ...
      it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
        ...
        // Defaulter opens with 60 Usdx, 0.6 ETH
        const { troveId: defaulter_1_TroveId } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraParams: { from: defaulter_1, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        /* Liquidate defaulter. 30 Usdx and 0.3 ETH is distributed between A, B and C.

           A receives (30 * 2/4) = 15 Usdx, and (0.3*2/4) = 0.15 ETH
           B receives (30 * 1/4) = 7.5 Usdx, and (0.3*1/4) = 0.075 ETH
           C receives (30 * 1/4) = 7.5 Usdx, and (0.3*1/4) = 0.075 ETH
        */
        await troveManager.liquidate(defaulter_1_TroveId);
      ...
      it("batchLiquidateTroves(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
        ...
        // Liquidate defaulter. 30 Usdx and 0.3 ETH is distributed uniformly between A, B and C. Each receive 10 Usdx, 0.1 ETH
        await troveManager.liquidate(defaulter_1_TroveId);
      ...
      it("batchLiquidateTroves(): does not affect the liquidated user's token balances", async () => {
        ...
        const D_balanceBefore = await usdxToken.balanceOf(dennis);
        const E_balanceBefore = await usdxToken.balanceOf(erin);
        const F_balanceBefore = await usdxToken.balanceOf(flyn);
        ...
        assert.equal(
          (await usdxToken.balanceOf(dennis)).toString(),
          D_balanceBefore,
        );
        assert.equal((await usdxToken.balanceOf(erin)).toString(), E_balanceBefore);
        assert.equal((await usdxToken.balanceOf(flyn)).toString(), F_balanceBefore);
      });

      it("batchLiquidateTroves(): A liquidation sequence containing Pool offsets increases the TCR", async () => {
        // Whale provides 500 Usdx to SP
        const { troveId: whaleTroveId } = await openTrove({
          ICR: toBN(dec(100, 18)),
          extraUsdxAmount: toBN(dec(500, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        // Check pool has 500 Usdx
        assert.equal(
          (await stabilityPool.getTotalUsdxDeposits()).toString(),
          dec(500, 18),
        );
        ...
        assert.equal((await stabilityPool.getTotalUsdxDeposits()).toString(), "0");
      ...
      it("batchLiquidateTroves(): Liquidating troves with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
        // Whale provides 400 Usdx to the SP
        const whaleDeposit = toBN(dec(40000, 18));
        const { troveId: whaleTroveId } = await openTrove({
          ICR: toBN(dec(100, 18)),
          extraUsdxAmount: whaleDeposit,
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: A_deposit,
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: bobTroveId } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: B_deposit,
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: carolTroveId } = await openTrove({
          ICR: toBN(dec(2, 18)),
        ...
        /* Prior to liquidation, SP deposits were:
           ...
           Total Usdx in Pool: 800 Usdx

           Then, liquidation hits A,B,C:

           Total liquidated debt = 150 + 350 + 150 = 650 Usdx
           Total liquidated ETH = 1.1 + 3.1 + 1.1 = 5.3 ETH

           whale usdx loss: 650 * (400/800) = 325 usdx
           alice usdx loss:  650 *(100/800) = 81.25 usdx
           bob usdx loss: 650 * (300/800) = 243.75 usdx

           whale remaining deposit: (400 - 325) = 75 usdx
           alice remaining deposit: (100 - 81.25) = 18.75 usdx
           bob remaining deposit: (300 - 243.75) = 56.25 usdx
           ...
           Total remaining deposits: 150 Usdx
           Total ETH gain: 4.975 ETH */

        // Check remaining Usdx Deposits and ETH gain, for whale and depositors whose troves were liquidated
        const whale_Deposit_After = await stabilityPool.getCompoundedUsdxDeposit(
          whale,
        );
        const alice_Deposit_After = await stabilityPool.getCompoundedUsdxDeposit(
          alice,
        );
        const bob_Deposit_After = await stabilityPool.getCompoundedUsdxDeposit(bob);
        ...
        const total_UsdxinSP = (
          await stabilityPool.getTotalUsdxDeposits()
        ).toString();
        const total_ETHinSP = (await stabilityPool.getCollBalance()).toString();

        assert.isAtMost(
          th.getDifference(total_UsdxinSP, totalDeposits.sub(liquidatedDebt)),
          1e14,
        );
        assert.isAtMost(
      ...
      it("batchLiquidateTroves(): liquidates a Trove that a) was skipped in a previous liquidation and b) has pending rewards", async () => {
        ...
        // A adds 10 Usdx to the SP, but less than C's debt
        ...
        await borrowerOperations.repayUsdx(DTroveId, dec(1, 18), { from: D });
        await borrowerOperations.repayUsdx(ETroveId, dec(1, 18), { from: E });
        ...
        const pendingUsdxDebt_C = await troveManager.getPendingUsdxDebtReward(CTroveId);
        const defaultPoolETH = await defaultPool.getCollBalance();
        const defaultPoolUsdxDebt = await defaultPool.getUsdxDebt();
        assert.isTrue(pendingETH_C.lte(defaultPoolETH));
        assert.isTrue(pendingUsdxDebt_C.lte(defaultPoolUsdxDebt));
        // Check only difference is dust
        assert.isAtMost(th.getDifference(pendingETH_C, defaultPoolETH), 1000);
        assert.isAtMost(
          th.getDifference(pendingUsdxDebt_C, defaultPoolUsdxDebt),
          1000,
        );
      ...
      it("batchLiquidateTroves(): closes every trove with ICR < MCR in the given array", async () => {
        ...
        // Price drops to 1ETH:100Usdx, reducing A, B, C ICR below MCR
        await priceFeed.setPrice(dec(100, 18));
        const price = await priceFeed.getPrice();
      ...
      it("batchLiquidateTroves(): skips if a trove has been closed", async () => {
        ...
        await usdxToken.transfer(carol, dec(100, 18), { from: whale });
      ...
      it("redeemCollateral(): cancels the provided Usdx with debt from Troves with the lowest ICRs and sends an equivalent amount of Ether", async () => {
        // --- SETUP ---
        const { troveId: aliceTroveId, totalDebt: A_totalDebt } = await openTrove({
          ICR: toBN(dec(310, 16)),
          extraUsdxAmount: dec(10, 18),
          extraParams: {
            from: alice,
            annualInterestRate: dec(5, 16),
            batchManager: getBatchManager(withBatchDelegation, dennis),
        ...
        await openTrove({
          ICR: toBN(dec(100, 18)),
          extraUsdxAmount: redemptionAmount,
          extraParams: {
            from: dennis,
        ...
        const dennis_UsdxBalance_Before = await usdxToken.balanceOf(dennis);
        ...
        /* check that Dennis' redeemed 20 Usdx has been cancelled with debt from Bobs's Trove (8) and Carol's Trove (10).
           The remaining lot (2) is sent to Alice's Trove, who had the best ICR.
           It leaves her with (3) Usdx debt + 50 for gas compensation. */
        th.assertIsApproximatelyEqual(
          alice_debt_After,
        ...
        const expectedTotalETHDrawn = redemptionAmount.div(toBN(200)); // convert redemptionAmount Usdx to ETH, at ETH:USD price 200
        ...
        const dennis_UsdxBalance_After = (
          await usdxToken.balanceOf(dennis)
        ).toString();
        assert.equal(
          dennis_UsdxBalance_After,
          dennis_UsdxBalance_Before.sub(redemptionAmount),
        );
      });
      ...
      it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
        ...
        // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 Usdx), 20000%
        const { usdxAmount: F_usdxAmount } = await openTrove({
          ICR: toBN(dec(200, 18)),
          extraUsdxAmount: A_debt.add(B_debt).add(C_debt).mul(toBN(2)),
          extraParams: {
            from: flyn,
            annualInterestRate: dec(50, 16),
            batchManager: getBatchManager(withBatchDelegation, flyn),
        ...
        // Check Flyn's redemption has reduced his balance from 100 to (100-60) = 40 Usdx
        const flynBalance = await usdxToken.balanceOf(flyn);
        th.assertIsApproximatelyEqual(
          flynBalance,
          F_usdxAmount.sub(redemptionAmount),
        );
        ...
        // (Erin has a small impact, ~600e-18 Usdx, which makes it update the interest)
      ...
      it("redeemCollateral(): ends the redemption sequence when the token redemption request has been filled", async () => {
        ...
        // open trove from redeemer.  Redeemer has highest ICR (100ETH, 100 Usdx), 20000%
        const { troveId: flynTroveId, usdxAmount: F_usdxAmount } = await openTrove({
          ICR: toBN(dec(200, 18)),
          extraUsdxAmount: A_debt.add(B_debt).add(C_debt).mul(toBN(2)),
          extraParams: {
            from: flyn,
      ...
      it("redeemCollateral(): ends the redemption sequence when max iterations have been reached", async () => {
        ...
        const { troveId: flynTroveId, usdxAmount: F_usdxAmount } = await openTrove({
          ICR: toBN(dec(200, 18)),
          extraUsdxAmount: C_debt.add(B_debt).mul(toBN(2)),
          extraParams: {
            from: flyn,
      ...
      it("redeemCollateral(): doesn’t perform partial redemption if resultant debt is > minimum net debt", async () => {
        ...
        await usdxToken.transfer(B, await usdxToken.balanceOf(A), { from: A });
        await usdxToken.transfer(B, await usdxToken.balanceOf(C), { from: C });
        ...
        // Usdx redemption is 55000
        const UsdxRedemption = dec(55000, 18);
      ...
      it("redeemCollateral(): can redeem if there is zero active debt but non-zero debt in DefaultPool", async () => {
        ...
        const amount = await getOpenTroveUsdxAmount(dec(2110, 18));
        await openTrove({
          ICR: toBN(dec(20, 18)),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: bobTroveId } = await openTrove({
          ICR: toBN(dec(133, 16)),
          extraUsdxAmount: amount,
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });

        await usdxToken.transfer(carol, amount, { from: bob });
        ...
        const expectedTotalETHDrawn = toBN(amount).div(toBN(100)); // convert 100 Usdx to ETH at ETH:USD price of 100
        const expectedReceivedETH = expectedTotalETHDrawn.sub(ETHFee);

        const receivedETH = carol_ETHBalance_After.sub(carol_ETHBalance_Before);
        assert.isTrue(expectedReceivedETH.eq(receivedETH));

        const carol_UsdxBalance_After = (
          await usdxToken.balanceOf(carol)
        ).toString();
        assert.equal(carol_UsdxBalance_After, "0");
      });
      ...
      it("redeemCollateral(): doesn’t redeem Troves with ICR < 100%", async () => {
        ...
        const { troveId: bobTroveId, usdxAmount: B_usdxAmount, totalDebt: B_totalDebt } = await openTrove({
          ICR: toBN(dec(133, 16)),
          extraUsdxAmount: A_debt,
          extraParams: {
            from: bob,
            annualInterestRate: dec(4, 16),
            batchManager: getBatchManager(withBatchDelegation, carol),
        ...
        await usdxToken.transfer(carol, B_usdxAmount, { from: bob });
      ...
      it("redeemCollateral(): reverts when argument _amount is 0", async () => {
        ...
        // Alice opens trove and transfers 500Usdx to Erin, the would-be redeemer
        await openTrove({
          ICR: toBN(dec(200, 16)),
          extraUsdxAmount: dec(500, 18),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        await usdxToken.transfer(erin, dec(500, 18), { from: alice });
      ...
      it("redeemCollateral(): reverts if max fee > 100%", async () => {
        ...
        await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(30, 18),
          extraParams: { from: C, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(40, 18),
          extraParams: { from: D, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
      ...
      it("redeemCollateral(): reverts if fee exceeds max fee percentage", async () => {
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(80, 18),
          extraParams: { from: A, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: bobTroveId } = await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(90, 18),
          extraParams: { from: B, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: carolTroveId } = await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(100, 18),
          extraParams: { from: C, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        ...
        // Usdx redemption is 27 USD: a redemption that incurs a fee of 27/(270 * 2) = 5%
        const attemptedUsdxRedemption = expectedTotalSupply.div(toBN(10));
      ...
      it("redeemCollateral(): doesn't affect the Stability Pool deposits or ETH gain of redeemed-from troves", async () => {
        ...
        const redemptionAmount = B_totalDebt.add(C_totalDebt)
          .add(D_totalDebt)
          .add(F_totalDebt);
        // Alice opens trove and transfers Usdx to Erin, the would-be redeemer
        await openTrove({
          ICR: toBN(dec(300, 16)),
          extraUsdxAmount: redemptionAmount,
          extraParams: {
        ...
        await usdxToken.transfer(erin, redemptionAmount, { from: alice });
        ...
        const carol_SPDeposit_before = (
          await stabilityPool.getCompoundedUsdxDeposit(carol)
        ).toString();
        ...
        // Check the remaining Usdx and ETH in Stability Pool after liquidation is non-zero
        const UsdxinSP = await stabilityPool.getTotalUsdxDeposits();
        const ETHinSP = await stabilityPool.getCollBalance();
        assert.isTrue(UsdxinSP.gte(mv._zeroBN));
      ...
      it("redeemCollateral(): caller can redeem their entire UsdxToken balance", async () => {
        const { troveId: whaleTroveId } = await openTrove({
          ICR: toBN(dec(20, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });

        // Alice opens trove and transfers 400 Usdx to Erin, the would-be redeemer
        const { troveId: aliceTroveId } = await openTrove({
          ICR: toBN(dec(300, 16)),
          extraUsdxAmount: dec(400, 18),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        await usdxToken.transfer(erin, dec(400, 18), { from: alice });

        // Check Erin's balance before
        const erin_balance_before = await usdxToken.balanceOf(erin);
        assert.equal(erin_balance_before, dec(400, 18));

        // B, C, D open trove
        const { troveId: bobTroveId } = await openTrove({
          ICR: toBN(dec(300, 16)),
          extraUsdxAmount: dec(590, 18),
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: carolTroveId } = await openTrove({
          ICR: toBN(dec(300, 16)),
          extraUsdxAmount: dec(1990, 18),
          extraParams: { from: carol, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: dennisTroveId } = await openTrove({
          ICR: toBN(dec(500, 16)),
        ...
        const activePool_debt_before = await activePool.getUsdxDebt();
        ...
        // Erin attempts to redeem 400 Usdx
        ...
        // Check activePool debt reduced by  400 Usdx
        const activePool_debt_after = await activePool.getUsdxDebt();
        th.assertIsApproximatelyEqual(
        ...
        const erin_balance_after = (await usdxToken.balanceOf(erin)).toString();
        assert.equal(erin_balance_after, "0");
      });

      it("redeemCollateral(): reverts when requested redemption amount exceeds caller's Usdx token balance", async () => {
        ...
        // Erin tries to redeem 1000 Usdx
        try {
          const redemptionTx = await th.redeemCollateralAndGetTxObject(
            erin,
            contracts,
          ...
          assert.include(
            error.message,
            "Requested redemption amount must be <= user's Usdx token balance",
          );
        }

        // Erin tries to redeem 401 Usdx
        try {
          const redemptionTx = await th.redeemCollateralAndGetTxObject(
            erin,
            contracts,
        ...
        // Erin tries to redeem 239482309 Usdx
        try {
          const redemptionTx = await th.redeemCollateralAndGetTxObject(
            erin,
            contracts,
        ...
        // Erin tries to redeem 2^256 - 1 Usdx
      ...
      it("redeemCollateral(): value of issued ETH == face value of redeemed Usdx (assuming 1 Usdx has value of $1)", async () => {
        const { collateral: W_coll } = await openTrove({
          ICR: toBN(dec(20, 18)),
          extraParams: { from: whale, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });

        // Alice opens trove and transfers 1000 Usdx each to Erin, Flyn, Graham
        const { troveId: aliceTroveId, collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({
          ICR: toBN(dec(400, 16)),
          extraUsdxAmount: dec(4990, 18),
          extraParams: { from: alice, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        await usdxToken.transfer(erin, dec(1000, 18), { from: alice });
        await usdxToken.transfer(flyn, dec(1000, 18), { from: alice });
        await usdxToken.transfer(graham, dec(1000, 18), { from: alice });

        // B, C, D open trove
        const { troveId: bobTroveId, collateral: B_coll } = await openTrove({
          ICR: toBN(dec(300, 16)),
          extraUsdxAmount: dec(1590, 18),
          extraParams: { from: bob, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: carolTroveId, collateral: C_coll } = await openTrove({
          ICR: toBN(dec(600, 16)),
          extraUsdxAmount: dec(1090, 18),
          extraParams: { from: carol, batchManager: getBatchManager(withBatchDelegation, dennis) },
        });
        const { troveId: dennisTroveId, collateral: D_coll } = await openTrove({
          ICR: toBN(dec(800, 16)),
        ...
        const _120_Usdx = "120000000000000000000";
        const _373_Usdx = "373000000000000000000";
        const _950_Usdx = "950000000000000000000";
        ...
        const ETHFee_1 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_120_Usdx, price);
        ...
        /* 120 Usdx redeemed.  Expect $120 worth of ETH removed. At ETH:USD price of $200,
        ...
        assert.equal(
          ...
          activeETH_0.sub(toBN(_120_Usdx).mul(mv._1e18BN).div(price)).toString(),
        );

        const ETHFee_2 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_373_Usdx, price);
        // Flyn redeems 373 Usdx
        ...
        /* 373 Usdx redeemed.  Expect $373 worth of ETH removed. At ETH:USD price of $200,
        ...
        assert.equal(
          ...
          activeETH_1.sub(toBN(_373_Usdx).mul(mv._1e18BN).div(price)).toString(),
        );

        const ETHFee_3 = await contracts.troveManager.getEffectiveRedemptionFeeInColl(_950_Usdx, price);
        // Graham redeems 950 Usdx
        ...
        /* 950 Usdx redeemed.  Expect $950 worth of ETH removed. At ETH:USD price of $200,
        ...
        assert.equal(
          ...
          activeETH_2.sub(toBN(_950_Usdx).mul(mv._1e18BN).div(price)).toString(),
        );
      });
      ...
      it("redeemCollateral(): reverts if there is zero outstanding system debt", async () => {
        // --- SETUP --- illegally mint Usdx to Bob
        await usdxToken.unprotectedMint(bob, dec(100, 18));

        assert.equal(await usdxToken.balanceOf(bob), dec(100, 18));

        const price = await priceFeed.getPrice();

        // Bob tries to redeem his illegally obtained Usdx
        try {
          const redemptionTx = await th.redeemCollateralAndGetTxObject(
            bob,
            contracts,
      ...
      it("redeemCollateral(): reverts if caller's tries to redeem more than the outstanding system debt", async () => {
        // --- SETUP --- illegally mint Usdx to Bob
        await usdxToken.unprotectedMint(bob, "101000000000000000000");

        assert.equal(await usdxToken.balanceOf(bob), "101000000000000000000");
        ...
        th.assertIsApproximatelyEqual(
          (await activePool.getUsdxDebt()).toString(),
          totalDebt,
        );
        ...
        // Bob attempts to redeem his ill-gotten 101 Usdx, from a system that has 100 Usdx outstanding debt
        try {
          const redemptionTx = await th.redeemCollateralAndGetTxObject(
            bob,
            contracts,
      ...
      it("redeemCollateral(): a redemption sends the ETH remainder (ETHDrawn - gas) to the redeemer", async () => {
        // time fast-forwards 1 year, and multisig stakes 1 ACCEL
        ...
        // Check total Usdx supply
        const activeUsdx = await activePool.getUsdxDebt();
        const defaultUsdx = await defaultPool.getUsdxDebt();

        const totalUsdxSupply = activeUsdx.add(defaultUsdx);
        th.assertIsApproximatelyEqual(totalUsdxSupply, totalDebt, 1e5);
      ...
      it("redeemCollateral(): a full redemption (leaving trove with 0 debt), closes the trove", async () => {
        ...
        // whale redeems 360 Usdx.  Expect this to fully redeem A, B, C, and partially redeem D.
      ...
      const redeemCollateral3Full1Partial = async (close = false) => {
        ...
        // whale redeems Usdx.  Expect this to fully redeem A, B, C, and partially redeem D.
        await th.redeemCollateral(whale, contracts, redemptionAmount, th._100pct, GAS_PRICE);
      ...
      it("getPendingUsdxDebtReward(): Returns 0 if there is no pending UsdxDebt reward", async () => {
        // Make some troves
        const { troveId: defaulter_1_TroveId } = await openTrove({
          ICR: toBN(dec(2, 18)),
          extraUsdxAmount: dec(100, 18),
        ...
        const current_L_usdxDebt = await troveManager.get_L_usdxDebt();
        assert.equal(current_L_usdxDebt.toString(), "0");

        const carolSnapshot_L_usdxDebt = (
          await troveManager.rewardSnapshots(carolTroveId)
        )[1];
        assert.equal(carolSnapshot_L_usdxDebt, 0);

        const carol_PendingUsdxDebtReward = await troveManager.getPendingUsdxDebtReward(carolTroveId);
        assert.equal(carol_PendingUsdxDebtReward, 0);
      });
      ...
      it("computeICR(): Returns 2^256-1 for ETH:USD = 100, coll = 1 ETH, debt = 100 Usdx", async () => {
        const price = dec(100, 18);
        const coll = dec(1, "ether");
        const debt = dec(100, 18);
      ...
      it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 200 ETH, debt = 30 Usdx", async () => {
        const price = dec(100, 18);
      ...
      it("computeICR(): returns correct ICR for ETH:USD = 250, coll = 1350 ETH, debt = 127 Usdx", async () => {
      ...
      it("computeICR(): returns correct ICR for ETH:USD = 100, coll = 1 ETH, debt = 54321 Usdx", async () => {
        const price = dec(100, 18);
        const coll = dec(1, "ether");
      ...
      it("getTroveStatus(): Returns status", async () => {
        ...
        const { troveId: ATroveId } = await openTrove({
          ICR: toBN(dec(150, 16)),
          extraUsdxAmount: B_totalDebt,
          extraParams: { from: A },
        });

        // to be able to repay:
        await usdxToken.transfer(B, B_totalDebt, { from: A });
        await borrowerOperations.closeTrove(BTroveId, { from: B });
