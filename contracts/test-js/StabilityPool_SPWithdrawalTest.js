...
contract("StabilityPool - Withdrawal of stability deposit - Reward calculations", async (accounts) => {
  ...
  const getOpenTroveUsdxAmount = async (totalDebt) => th.getOpenTroveUsdxAmount(contracts, totalDebt);
  ...
  describe("Stability Pool Withdrawal", async () => {
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      });

      // Whale transfers 10k Usdx to A, B and C who then deposit it to the SP
      const depositors = [alice, bob, carol];
      for (const account of depositors) {
        await usdxToken.transfer(account, dec(10000, 18), { from: whale });
        await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: account });
      }

      // Defaulter opens trove with 200% ICR and 10k Usdx net debt
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(10000, 18)),
        defaulter_1,
        defaulter_1,
        0,
      ...
      // Check depositors' compounded deposit is 6666.66 Usdx and ETH Gain is 33.16 ETH
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "6666666666666666666666"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "6666666666666666666666"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "6666666666666666666666"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 3333.33 Usdx and ETH Gain is 66.33 ETH
      const txA = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: alice });
      const txB = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: bob });
      const txC = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: carol });
      // Grab the ETH gain from the emitted event in the tx log
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "3333333333333333333333"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "3333333333333333333333"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "3333333333333333333333"), 10000);
    ...
    it("withdrawFromSP():  Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      // Check depositors' compounded deposit is 0 Usdx and ETH Gain is 99.5 ETH
      const txA = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: alice });
      const txB = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: bob });
      const txC = await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: carol });
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "0"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after two liquidations of increasing Usdx", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(5000, 18)),
        defaulter_1,
        defaulter_1,
        0,
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(7000, 18)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "6000000000000000000000"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "6000000000000000000000"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "6000000000000000000000"), 10000);
    ...
    it("withdrawFromSP(): Depositors with equal initial deposit withdraw correct compounded deposit and ETH Gain after three liquidations of increasing Usdx", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(6000, 18)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "4000000000000000000000"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "4000000000000000000000"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "4000000000000000000000"), 10000);
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after two identical liquidations", async () => {
      ...
      // Whale transfers 10k, 20k, 30k Usdx to A, B and C respectively who then deposit it to the SP
      await usdxToken.transfer(alice, dec(10000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: alice });
      await usdxToken.transfer(bob, dec(20000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: bob });
      await usdxToken.transfer(carol, dec(30000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "6666666666666666666666"),
        100000,
      );
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "13333333333333333333333"), 100000);
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "20000000000000000000000"),
        100000,
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three identical liquidations", async () => {
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "5000000000000000000000"),
        100000,
      );
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "10000000000000000000000"), 100000);
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "15000000000000000000000"),
        100000,
      );
    ...
    it("withdrawFromSP(): Depositors with varying deposits withdraw correct compounded deposit and ETH Gain after three varying liquidations", async () => {
      ...
      await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(1000000, 18)),
        whale,
        whale,
        0,
      ...
      /* Depositors provide:-
         ...
         Bob:  456000 Usdx
         Carol: 13100 Usdx */
      // Whale transfers Usdx to  A, B and C respectively who then deposit it to the SP
      await usdxToken.transfer(alice, dec(2000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(2000, 18), { from: alice });
      await usdxToken.transfer(bob, dec(456000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(456000, 18), { from: bob });
      await usdxToken.transfer(carol, dec(13100, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(13100, 18), { from: carol });

      /* Defaulters open troves

         Defaulter 1: 207000 Usdx & 2160 ETH
         Defaulter 2: 5000 Usdx & 50 ETH
         Defaulter 3: 46700 Usdx & 500 ETH
      */
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("207000000000000000000000"),
        defaulter_1,
        defaulter_1,
        0,
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(5, 21)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      const defaulter_3_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("46700000000000000000000"),
        defaulter_3,
        defaulter_3,
        0,
      ...
      // ()
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "901719380174061000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(bob)).toString(), "205592018679686000000000"),
        10000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "5906261940140100000000"),
        10000000000,
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 1 liquidation. All deposits and liquidations = 100 Usdx.  A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      await usdxToken.transfer(dennis, dec(10000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "1666666666666666666666"),
        100000,
      );
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "1666666666666666666666"), 100000);
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "1666666666666666666666"),
        100000,
      );

      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
        100000,
      );
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. All deposits and liquidations = 100 Usdx.  A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "0"), 100000);
    ...
    it("withdrawFromSP(): A, B, C Deposit -> 2 liquidations -> D deposits -> 2 liquidations. Various deposit and liquidation vals.  A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 1m ETH
      await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(1000000, 18)),
      ...
      */
      // Whale transfers Usdx to  A, B and C respectively who then deposit it to the SP
      await usdxToken.transfer(alice, dec(60000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(60000, 18), { from: alice });
      await usdxToken.transfer(bob, dec(20000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: bob });
      await usdxToken.transfer(carol, dec(15000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(15000, 18), { from: carol });

      /* Defaulters open troves:
         Defaulter 1:  10000 Usdx, 100 ETH
         Defaulter 2:  25000 Usdx, 250 ETH
         Defaulter 3:  5000 Usdx, 50 ETH
         Defaulter 4:  40000 Usdx, 400 ETH
      */
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(10000, 18)),
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(25000, 18)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      const defaulter_4_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(40000, 18)),
        defaulter_4,
        defaulter_4,
        0,
      ...
      // Dennis provides 25000 Usdx
      await usdxToken.transfer(dennis, dec(25000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "17832817337461300000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(bob)).toString(), "5944272445820430000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "4458204334365320000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "11764705882352900000000"),
        100000000000,
      );
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. All deposits and liquidations = 100 Usdx.  A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "0"), 1000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "0"), 1000);
    ...
    it("withdrawFromSP(): A, B, C, D deposit -> 2 liquidations -> D withdraws -> 2 liquidations. Various deposit and liquidation vals. A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      /* Initial deposits:
         ...
         Dennis: 40000 Usdx
      */
      // Whale transfers Usdx to  A, B,C and D respectively who then deposit it to the SP
      await usdxToken.transfer(alice, dec(20000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: alice });
      await usdxToken.transfer(bob, dec(25000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(25000, 18), { from: bob });
      await usdxToken.transfer(carol, dec(12500, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(12500, 18), { from: carol });
      await usdxToken.transfer(dennis, dec(40000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(40000, 18), { from: dennis });

      /* Defaulters open troves:
         Defaulter 1: 10000 Usdx
         Defaulter 2: 20000 Usdx
         Defaulter 3: 30000 Usdx
         Defaulter 4: 5000 Usdx
      */
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(10000, 18)),
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(20000, 18)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      const defaulter_3_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(30000, 18)),
        defaulter_3,
        defaulter_3,
        0,
      ...
      const dennis_ETHWithdrawn = getETHGainWithdrawn(txD).toString();
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "27692307692307700000000"),
        100000000000,
      );
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(alice)).toString(), "1672240802675590000000"),
        10000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(bob)).toString(), "2090301003344480000000"),
        100000000000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "1045150501672240000000"),
        100000000000,
      );
    ...
    it("withdrawFromSP(): A, B, D deposit -> 2 liquidations -> C makes deposit -> 1 liquidation -> D withdraws -> 1 liquidation. All deposits: 100 Usdx. Liquidations: 100,100,100,50.  A, B, C, D withdraw correct Usdx deposit and ETH Gain", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      // Whale transfers 10k Usdx to A, B and D who then deposit it to the SP
      ...
      // Carol makes deposit
      await usdxToken.transfer(carol, dec(10000, 18), { from: whale });
      ...
      const dennis_ETHWithdrawn = getETHGainWithdrawn(txD).toString();
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "1666666666666666666666"),
        100000,
      );
      ...
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "666666666666666666666"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "666666666666666666666"), 100000);
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "2000000000000000000000"),
        100000,
      );
    ...
    // A, B withdraw 0Usdx & 100e
    // C, D withdraw 5000Usdx  & 500e
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Whale transfers 10k Usdx to A, B who then deposit it to the SP
      ...
      // Defaulter 1 liquidated. 20000 Usdx fully offset with pool.
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner });

      // Carol, Dennis each deposit 10000 Usdx
      ...
      // Defaulter 2 liquidated. 10000 Usdx offset
      await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
      ...
      // Expect Alice And Bob's compounded deposit to be 0 Usdx
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "0"), 10000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "0"), 10000);
      ...
      // Expect Carol And Dennis' compounded deposit to be 50 Usdx
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "5000000000000000000000"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "5000000000000000000000"),
    ...
    it("withdrawFromSP(): Pool-emptying liquidation increases epoch by one, resets scaleFactor to 0, and resets P to 1e18", async () => {
      ...
      // Defaulter 1 liquidated. 10--0 Usdx fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
      ...
      // Defaulter 2 liquidated. 1--00 Usdx, empties pool
      await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
      ...
      // Defaulter 3 liquidated. 10000 Usdx fully offset, Pool remains non-zero
      await troveManager.liquidate(defaulter_3_TroveId, { from: owner });
      ...
      // Defaulter 4 liquidated. 10000 Usdx, empties pool
      await troveManager.liquidate(defaulter_4_TroveId, { from: owner });
    ...
    // A, B withdraw 0 Usdx & 100e
    // C, D withdraw 5000 Usdx  & 50e
    it("withdrawFromSP(): Depositors withdraw correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Carol, Dennis, Erin each deposit 10000, 20000, 30000 Usdx respectively
      await usdxToken.transfer(carol, dec(10000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: carol });

      await usdxToken.transfer(dennis, dec(20000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: dennis });

      await usdxToken.transfer(erin, dec(30000, 18), { from: whale });
      ...
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(carol)).toString(), "8333333333333333333333"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "16666666666666666666666"),
        100000,
      );
      assert.isAtMost(
        th.getDifference((await usdxToken.balanceOf(erin)).toString(), "25000000000000000000000"),
        100000,
      );
    ...
    // A deposits 10000
    // L1, L2, L3 liquidated with 10000 Usdx each
    // A withdraws all
    ...
    it("withdrawFromSP(): single deposit fully offset. After subsequent liquidations, depositor withdraws 0 deposit and *only* the ETH Gain from one liquidation", async () => {
      ...
      // Defaulter 1,2,3 withdraw 10000 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(10000, 18)),
        defaulter_1,
    ...
    // A,B deposit 10000 Usdx
    // L1 cancels 20000 Usdx, 2E
    // B,C deposits 10000 Usdx
    // L2 cancels 20000 Usdx, 2E
    // E,F deposit 10000 Usdx
    // L3 cancels 20000, 200E
    // G,H deposits 10000
    // L4 cancels 20000, 200E

    // Expect all depositors withdraw 0 Usdx and 100 ETH
    ...
    it("withdrawFromSP(): Depositor withdraws correct compounded deposit after liquidation empties the pool", async () => {
      ...
      // Alice, Bob each deposit 10k Usdx
      ...
      // Defaulter 1 liquidated. 20k Usdx fully offset with pool.
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner });
      ...
      // Erin, Flyn each deposit 10000 Usdx
      ...
      // Defaulter 3 liquidated. 10000 Usdx offset
      await troveManager.liquidate(defaulter_3_TroveId, { from: owner });

      // Graham, Harriet each deposit 10000 Usdx
      ...
      // Defaulter 4 liquidated. 10k Usdx offset
      await troveManager.liquidate(defaulter_4_TroveId, { from: owner });
      ...
      // Expect all deposits to be 0 Usdx
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(alice)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(dennis)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(erin)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(flyn)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(graham)).toString(), "0"), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(harriet)).toString(), "0"), 100000);
    ...
    // A withdraws all
    // B deposits 10000
    // L2 of 9900 Usdx, should bring P slightly past boundary i.e. 1e-9 -> 1e-10
    ...
    // in helper functon getOpenTroveUsdxAmount due to now-zero borrow fees. Double-check this when we write new SP arithmetic tests and fix the "P" issue.
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10000 Usdx:  9999.99991 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("9999999910000000000000"),
        defaulter_1,
        defaulter_1,
        0,
        { from: defaulter_1, value: dec(100, "ether") },
      ...
      // Defaulter 2 withdraws 9900 Usdx
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(9900, 18)),
        defaulter_2,
        defaulter_2,
        0,
      ...
      // usdxLossPerUnitStaked = usdxLossNumerator / _totalUsdxDeposits + 1;
      ...
      await usdxToken.transfer(bob, dec(10000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: bob });

      // Defaulter 2 liquidated.  9900 Usdx liquidated. P altered by a factor of 1-(9900/10000) = 0.01.  Scale changed.
      await troveManager.liquidate(defaulter_2_TroveId, { from: owner });
      ...
      // Expect Bob to withdraw 1% of initial deposit (100 Usdx) and all the liquidated ETH (60 ether)
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), "100000000000000000000"), 100000);
      assert.isAtMost(th.getDifference(bob_ETHWithdrawn, "59700000000000000000"), 100000);
    });

    // A deposits 10000
    // L1 brings P close to boundary, i.e. 9e-9: liquidate 9999.99991 Usdx
    // A withdraws all
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraw correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 withdraws 'almost' 10k Usdx.
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("9999999910000000000000"),
        defaulter_1,
      ...
      // Defaulter 2 withdraws 59400 Usdx
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("59400000000000000000000"),
        defaulter_2,
        defaulter_2,
        0,
      ...
      await usdxToken.transfer(carol, dec(20000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(20000, 18), { from: carol });

      await usdxToken.transfer(dennis, dec(30000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(30000, 18), { from: dennis });

      // 54000 Usdx liquidated.  P altered by a factor of 1-(59400/60000) = 0.01. Scale changed.
      ...
      /* Expect depositors to withdraw 1% of their initial deposit, and an ETH gain
         ...
         Bob:  1000 Usdx, 55 Ether
         Carol:  2000 Usdx, 110 Ether
         Dennis:  3000 Usdx, 165 Ether

         Total: 6000 Usdx, 300 Ether
      */
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), dec(100, 18)), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), dec(200, 18)), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(dennis)).toString(), dec(300, 18)), 100000);
    ...
    // A make deposit 10000 Usdx
    // L1 brings P to 1e-5*P. L1:  9999.9000000000000000 Usdx
    // A withdraws
    // B makes deposit 10000 Usdx
    // L2 decreases P again by 1e-5, over the scale boundary: 9999.9000000000000000 (near to the 10000 Usdx total deposits)
    // B withdraws
    ...
    it("withdrawFromSP(): deposit spans one scale factor change: Single depositor withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 each withdraw 9999.999999999 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(99999, 17)),
        defaulter_1,
        defaulter_1,
        0,
        { from: defaulter_1, value: dec(100, "ether") },
      ...
      // Bob should withdraw 1e-5 of initial deposit: 0.1 Usdx and the full ETH gain of 100 ether
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), dec(1, 17)), 100000);
    ...
    // L2 decreases P again by 1e-5, over boundary. L2: 59999.4000000000000000  (near to the 60000 Usdx total deposits)
    // B withdraws
    // expect d(B) = d0(B) * 1e-5
    // expect B gets entire ETH gain from L2
    //
    ...
    it("withdrawFromSP(): Several deposits of varying amounts span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      ...
      // Defaulter 1 and default 2 withdraw up to debt of 9999.9 Usdx and 59999.4 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("9999900000000000000000"),
        defaulter_1,
        defaulter_1,
        0,
        { from: defaulter_1, value: dec(100, "ether") },
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("59999400000000000000000"),
        defaulter_2,
        defaulter_2,
        0,
      ...
      // B, C, D deposit 10000, 20000, 30000 Usdx
      await usdxToken.transfer(bob, dec(10000, 18), { from: whale });
      await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: bob });
      ...
      // {B, C, D} should have a compounded deposit of {0.1, 0.2, 0.3} Usdx
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(bob)).toString(), dec(1, 17)), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(carol)).toString(), dec(2, 17)), 100000);
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(dennis)).toString(), dec(3, 17)), 100000);
    ...
    // A make deposit 10000 Usdx
    // L1 brings P to (~1e-10)*P. L1: 9999.9999999000000000 Usdx
    ...
    it("withdrawFromSP(): Deposit that decreases to less than 1e-9 of it's original value is reduced to 0", async () => {
      ...
      // Defaulters 1 withdraws 9999.9999999 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("9999999999900000000000"),
        defaulter_1,
        defaulter_1,
        0,
        { from: defaulter_1, value: dec(100, "ether") },
      ...
      const aliceDeposit = (await stabilityPool.getCompoundedUsdxDeposit(alice)).toString();
    ...
    /* A make deposit 10000 Usdx
       L1 brings P to 0.0001P. L1:  9999.900000000000000000 Usdx, 1 ETH
       B makes deposit 9999.9, brings SP to 10k
       L2 decreases P by(~1e-5)P. L2:  9999.900000000000000000 Usdx, 1 ETH
       C makes deposit 9999.9, brings SP to 10k
       L3 decreases P by(~1e-5)P. L3:  9999.900000000000000000 Usdx, 1 ETH
       D makes deposit 9999.9, brings SP to 10k
       L4 decreases P by(~1e-5)P. L4:  9999.900000000000000000 Usdx, 1 ETH
       expect A, B, C, D each withdraw ~100 Ether
    */
    // TODO: Changes since v1 have introduced very slight precision error in this test. Potentially due to slightly different rounding
    // in helper functon getOpenTroveUsdxAmount due to now-zero borrow fees. Double-check this when we write new SP arithmetic tests and fix the "P" issue.
    it("withdrawFromSP(): Several deposits of 10000 Usdx span one scale factor change. Depositors withdraws correct compounded deposit and ETH Gain after one liquidation", async () => {
      // Whale opens Trove with 100k ETH
      await th.openTroveWrapper(contracts, await getOpenTroveUsdxAmount(dec(100000, 18)), whale, whale, 0, {
        from: whale,
        value: dec(100000, "ether"),
      ...
      // Defaulters 1-4 each withdraw 9999.9 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount("9999900000000000000000"),
        defaulter_1,
      ...
      // B deposits 9999.9 Usdx
      await usdxToken.transfer(bob, dec(99999, 17), { from: whale });
      ...
      // C deposits 9999.9 Usdx
      await usdxToken.transfer(carol, dec(99999, 17), { from: whale });
      ...
      // D deposits 9999.9 Usdx
      await usdxToken.transfer(dennis, dec(99999, 17), { from: whale });
      ...
      assert.equal(await usdxToken.balanceOf(alice), "0");
      assert.equal(await usdxToken.balanceOf(alice), "0");
      assert.equal(await usdxToken.balanceOf(alice), "0");
      // D should withdraw around 0.9999 Usdx, since his deposit of 9999.9 was reduced by a factor of 1e-5
      assert.isAtMost(th.getDifference((await usdxToken.balanceOf(dennis)).toString(), dec(99999, 12)), 100000);
    ...
    it("withdrawFromSP(): 2 depositors can withdraw after each receiving half of a pool-emptying liquidation", async () => {
      ...
      // Defaulters 1-3 each withdraw 24100, 24300, 24500 Usdx (inc gas comp)
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(24100, 18)),
        defaulter_1,
        defaulter_1,
        0,
        { from: defaulter_1, value: dec(200, "ether") },
      ...
      const defaulter_2_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(24300, 18)),
        defaulter_2,
        defaulter_2,
        0,
        { from: defaulter_2, value: dec(200, "ether") },
      ...
      const defaulter_3_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(24500, 18)),
        defaulter_3,
        defaulter_3,
        0,
        { from: defaulter_3, value: dec(200, "ether") },
      ...
      // A, B provide 10k Usdx
      await usdxToken.transfer(A, dec(10000, 18), { from: whale });
      await usdxToken.transfer(B, dec(10000, 18), { from: whale });
      ...
      const A_deposit = await stabilityPool.getCompoundedUsdxDeposit(A);
      const B_deposit = await stabilityPool.getCompoundedUsdxDeposit(B);
      ...
      const UsdxinSP_1 = await stabilityPool.getTotalUsdxDeposits();
      // console.log(`UsdxinSP_1: ${UsdxinSP_1}`)
      assert.equal(UsdxinSP_1, "0");

      // Check SP Usdx balance is zero
      const SPUsdxBalance_1 = await usdxToken.balanceOf(stabilityPool.address);
      // console.log(`SPUsdxBalance_1: ${SPUsdxBalance_1}`)
      assert.equal(SPUsdxBalance_1, "0");
      ...
      // C, D provide 10k Usdx
      await usdxToken.transfer(C, dec(10000, 18), { from: whale });
      await usdxToken.transfer(D, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const C_deposit = await stabilityPool.getCompoundedUsdxDeposit(C);
      const D_deposit = await stabilityPool.getCompoundedUsdxDeposit(D);
      ...
      // Check SP tracker is zero
      const UsdxinSP_2 = await stabilityPool.getTotalUsdxDeposits();
      // console.log(`UsdxinSP_2: ${UsdxinSP_2}`)
      assert.equal(UsdxinSP_2, "0");

      // Check SP Usdx balance is zero
      const SPUsdxBalance_2 = await usdxToken.balanceOf(stabilityPool.address);
      // console.log(`SPUsdxBalance_2: ${SPUsdxBalance_2}`)
      assert.equal(SPUsdxBalance_2, "0");
      ...
      // E, F provide 10k Usdx
      await usdxToken.transfer(E, dec(10000, 18), { from: whale });
      await usdxToken.transfer(F, dec(10000, 18), { from: whale });
      ...
      // Check compounded deposits
      const E_deposit = await stabilityPool.getCompoundedUsdxDeposit(E);
      const F_deposit = await stabilityPool.getCompoundedUsdxDeposit(F);
      ...
      // Check SP tracker is zero
      const UsdxinSP_3 = await stabilityPool.getTotalUsdxDeposits();
      assert.equal(UsdxinSP_3, "0");

      // Check SP Usdx balance is zero
      const SPUsdxBalance_3 = await usdxToken.balanceOf(stabilityPool.address);
      // console.log(`SPUsdxBalance_3: ${SPUsdxBalance_3}`)
      assert.equal(SPUsdxBalance_3, "0");
    ...
    it("withdrawFromSP(): Depositor's ETH gain stops increasing after two scale changes", async () => {
      ...
      // Defaulters 1-5 each withdraw up to debt of 9999.9999999 Usdx
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(99999, 17)),
        defaulter_1,
      ...
      // E deposits 9999.9 Usdx
      await usdxToken.transfer(erin, dec(99999, 17), { from: whale });
    ...
    it("withdrawFromSP(): Large liquidated coll/debt, deposits and ETH price", async () => {
      ...
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(1, 36)),
        defaulter_1,
        defaulter_1,
        0,
      ...
      // Check Usdx balances
      const aliceUsdxBalance = await usdxToken.balanceOf(alice);
      const aliceExpectedUsdxBalance = web3.utils.toBN(dec(5, 35));
      const aliceUsdxBalDiff = aliceUsdxBalance.sub(aliceExpectedUsdxBalance).abs();

      assert.isTrue(aliceUsdxBalDiff.lte(toBN(dec(1, 18)))); // error tolerance of 1e18

      const bobUsdxBalance = await usdxToken.balanceOf(bob);
      const bobExpectedUsdxBalance = toBN(dec(5, 35));
      const bobUsdxBalDiff = bobUsdxBalance.sub(bobExpectedUsdxBalance).abs();

      assert.isTrue(bobUsdxBalDiff.lte(toBN(dec(1, 18))));
    ...
    // TODO: Changes since v1 have made the error margin in this test  i.e. aliceUsdxBalanceDiff increase 100x (but still low relative to the huge values used in test).
    // Potentially due to slightly different rounding in helper getOpenTroveUsdxAmount due to now-zero borrow fees.
    ...
    it("withdrawFromSP(): Small liquidated coll/debt, large deposits and ETH price", async () => {
      ...
      // Defaulter opens trove with 50e-7 ETH and  5000 Usdx. 200% ICR
      const defaulter_1_TroveId = await th.openTroveWrapper(
        contracts,
        await getOpenTroveUsdxAmount(dec(5000, 18)),
        defaulter_1,
      ...
      const aliceUsdxBalance = await usdxToken.balanceOf(alice);
      const aliceExpectedUsdxBalance = toBN("99999999999999997500000000000000000000");
      const aliceUsdxBalDiff = aliceUsdxBalance.sub(aliceExpectedUsdxBalance).abs();

      // th.logBN("aliceUsdxBalDiff", aliceUsdxBalDiff);
      ...
      assert.isTrue(aliceUsdxBalDiff.lte(toBN(dec(1, 20))));

      const bobUsdxBalance = await usdxToken.balanceOf(bob);
      const bobExpectedUsdxBalance = toBN("99999999999999997500000000000000000000");
      const bobUsdxBalDiff = bobUsdxBalance.sub(bobExpectedUsdxBalance).abs();

      assert.isTrue(bobUsdxBalDiff.lte(toBN("100000000000000000000")));
