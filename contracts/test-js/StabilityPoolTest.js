...
contract("StabilityPool", async (accounts) => {
  ...
  describe("Stability Pool Mechanisms", async () => {
    ...
    // --- provideToSP() ---
    // increases recorded Usdx at Stability Pool
    it("provideToSP(): increases the Stability Pool Usdx balance", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraUsdxAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check Usdx balances after
      const stabilityPool_Usdx_After = await stabilityPool.getTotalUsdxDeposits();
      assert.equal(stabilityPool_Usdx_After, 200);
    });
    ...
    it("provideToSP(): reduces the user's Usdx balance by the correct amount", async () => {
      // --- SETUP --- Give Alice a least 200
      await openTrove({
        extraUsdxAmount: toBN(200),
        ICR: toBN(dec(2, 18)),
      ...
      // check user's Usdx balance change
      const alice_UsdxBalance_After = await usdxToken.balanceOf(alice);
      assert.equal(
        alice_UsdxBalance_Before.sub(alice_UsdxBalance_After),
        "200",
      );
    });

    it("provideToSP(): increases totalUsdxDeposits by correct amount", async () => {
      // --- SETUP ---

      // Whale opens Trove with 50 ETH, adds 2000 Usdx to StabilityPool
      await openTrove({
        extraUsdxAmount: toBN(dec(2000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      const totalUsdxDeposits = await stabilityPool.getTotalUsdxDeposits();
      assert.equal(totalUsdxDeposits, dec(2000, 18));
    });
    ...
    it("provideToSP(): Correctly updates user snapshots of accumulated rewards per unit staked", async () => {
      ...
      const whaleUsdx = await usdxToken.balanceOf(whale);
      await th.provideToSPAndClaim(contracts, whaleUsdx, { from: whale });
      ...
      // Alice makes Trove and withdraws 100 Usdx
      await openTrove({
      ...
      const SPUsdx_Before = await stabilityPool.getTotalUsdxDeposits();
      ...
      const SPUsdx_After = await stabilityPool.getTotalUsdxDeposits();
      assert.isTrue(SPUsdx_After.lt(SPUsdx_Before));
    ...
    it("provideToSP(), multiple deposits: updates user's deposit and snapshots", async () => {
      ...
      // 3 Troves opened. Two users withdraw 160 Usdx each
      const { troveId: defaulter_1_TroveId } = await openTrove({
        extraUsdxAmount: 0,
        ICR: toBN(dec(2, 18)),
      ...
      // Alice makes deposit #1: 150 Usdx
      await openTrove({
        extraUsdxAmount: toBN(dec(250, 18)),
        ICR: toBN(dec(3, 18)),
        extraParams: { from: alice },
      });
      ...
      // 2 users with Trove with 180 Usdx drawn are closed
      await troveManager.liquidate(defaulter_1_TroveId, { from: owner }); // 180 Usdx closed
      await troveManager.liquidate(defaulter_2_TroveId, { from: owner }); // 180 Usdx closed

      const alice_compoundedDeposit_1 = await stabilityPool.getCompoundedUsdxDeposit(alice);
      ...
      await openTrove({
        extraUsdxAmount: toBN(dec(1000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      const alice_compoundedDeposit_2 = await stabilityPool.getCompoundedUsdxDeposit(alice);
    ...
    it("provideToSP(): reverts if user tries to provide more than their Usdx balance", async () => {
      await openTrove({
        extraUsdxAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") },
      ...
      const aliceUsdxbal = await usdxToken.balanceOf(alice);
      const bobUsdxbal = await usdxToken.balanceOf(bob);

      // Alice, attempts to deposit 1 wei more than her balance

      const aliceTxPromise = stabilityPool.provideToSP(
        aliceUsdxbal.add(toBN(1)),
        { from: alice },
      );
      ...
      const bobTxPromise = stabilityPool.provideToSP(
        bobUsdxbal.add(toBN(dec(235534, 18))),
        { from: bob },
      );
      await assertRevert(bobTxPromise, "revert");
    });

    it("provideToSP(): reverts if user tries to provide 2^256-1 Usdx, which exceeds their balance", async () => {
      await openTrove({
        extraUsdxAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: whale, value: dec(50, "ether") },
      ...
      // Alice attempts to deposit 2^256-1 Usdx
      try {
    ...
    it("provideToSP(): doesn't impact other users' deposits or ETH gains", async () => {
      ...
      await openTrove({
        extraUsdxAmount: toBN(dec(3000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      assert.equal(
        (await stabilityPool.getCompoundedUsdxDeposit(dennis)).toString(),
        dec(1000, 18),
      );

      const alice_UsdxDeposit_After = (
        await stabilityPool.getCompoundedUsdxDeposit(alice)
      ).toString();
      const bob_UsdxDeposit_After = (
        await stabilityPool.getCompoundedUsdxDeposit(bob)
      ).toString();
      const carol_UsdxDeposit_After = (
        await stabilityPool.getCompoundedUsdxDeposit(carol)
      ).toString();
      ...
      assert.equal(alice_UsdxDeposit_Before, alice_UsdxDeposit_After);
      assert.equal(bob_UsdxDeposit_Before, bob_UsdxDeposit_After);
      assert.equal(carol_UsdxDeposit_Before, carol_UsdxDeposit_After);
    ...
    it("provideToSP(): doesn't impact system debt, collateral or TCR", async () => {
      ...
      const activeDebt_Before = (await activePool.getUsdxDebt()).toString();
      const defaultedDebt_Before = (await defaultPool.getUsdxDebt()).toString();
      ...
      const activeDebt_After = (await activePool.getUsdxDebt()).toString();
      const defaultedDebt_After = (await defaultPool.getUsdxDebt()).toString();
    ...
    it("provideToSP(): doesn't protect the depositor's trove from liquidation", async () => {
      ...
      assert.equal(
        (await stabilityPool.getCompoundedUsdxDeposit(bob)).toString(),
        dec(1000, 18),
      );
    ...
    it("provideToSP(): providing 0 Usdx reverts", async () => {
      // --- SETUP ---
      await openTrove({
        extraUsdxAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(2, 18)),
      ...
      // A, B, C provides 100, 50, 30 Usdx to SP
      await th.provideToSPAndClaim(contracts, dec(100, 18), {
        from: alice,
      });
      ...
      const UsdxinSP_Before = (
        await stabilityPool.getTotalUsdxDeposits()
      ).toString();

      assert.equal(UsdxinSP_Before, dec(180, 18));

      // Bob provides 0 Usdx to the Stability Pool
    ...
    it("provideToSP(), new deposit: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers Usdx to A, B
      await usdxToken.transfer(A, dec(100, 18), { from: whale });
      await usdxToken.transfer(B, dec(200, 18), { from: whale });
    ...
    it("provideToSP(), new deposit after past full withdrawal: depositor does not receive ETH gains", async () => {
      ...
      // Whale transfers Usdx to A, B
      await usdxToken.transfer(A, dec(1000, 18), { from: whale });
      await usdxToken.transfer(B, dec(1000, 18), { from: whale });

      // C, D open troves
      await openTrove({
        extraUsdxAmount: toBN(dec(4000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: C },
      });
      await openTrove({
    ...
    it("provideToSP(): reverts when amount is zero", async () => {
      ...
      // Whale transfers Usdx to C, D
      await usdxToken.transfer(C, dec(100, 18), { from: whale });
      await usdxToken.transfer(D, dec(100, 18), { from: whale });
    ...
    it("withdrawFromSP(): partial retrieval - retrieves correct Usdx amount and the entire ETH Gain, and updates deposit", async () => {
      // --- SETUP ---
      // Whale deposits 185000 Usdx in StabilityPool
      await openTrove({
        extraUsdxAmount: toBN(dec(1, 24)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
      });
      ...
      // Alice makes deposit #1: 15000 Usdx
      await openTrove({
      ...
      // 2 users with Trove with 170 Usdx drawn are closed
      ...
      // Alice UsdxLoss is ((15000/200000) * liquidatedDebt), for each liquidation
      const expectedUsdxLoss_A = liquidatedDebt_1
      ...
      const expectedCompoundedUsdxDeposit_A = toBN(dec(15000, 18)).sub(
        expectedUsdxLoss_A,
      );
      const compoundedUsdxDeposit_A = await stabilityPool.getCompoundedUsdxDeposit(alice);

      assert.isAtMost(
        th.getDifference(
          expectedCompoundedUsdxDeposit_A,
          compoundedUsdxDeposit_A,
        ),
        100000,
      );

      // Alice retrieves part of her entitled Usdx: 9000 Usdx
      await th.withdrawFromSPAndClaim(contracts, dec(9000, 18), { from: alice });

      const expectedNewDeposit_A = compoundedUsdxDeposit_A.sub(
        toBN(dec(9000, 18)),
      );
    ...
    it("withdrawFromSP(): partial retrieval - leaves the correct amount of Usdx in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 Usdx in StabilityPool
      await openTrove({
        extraUsdxAmount: toBN(dec(1, 24)),
      ...
      const SP_Usdx_Before = await stabilityPool.getTotalUsdxDeposits();
      assert.equal(SP_Usdx_Before, dec(200000, 18));
      ...
      /* Check SP has reduced from 2 liquidations and Alice's withdrawal
         Expect Usdx in SP = (200000 - liquidatedDebt_1 - liquidatedDebt_2 - 9000) */
      const expectedSPUsdx = toBN(dec(200000, 18))
      ...
      th.assertIsApproximatelyEqual(SP_Usdx_After, expectedSPUsdx);
    });

    it("withdrawFromSP(): full retrieval - leaves the correct amount of Usdx in the Stability Pool", async () => {
      // --- SETUP ---
      // Whale deposits 185000 Usdx in StabilityPool
      await openTrove({
      ...
      const UsdxinSPBefore = await stabilityPool.getTotalUsdxDeposits();

      // Alice retrieves all of her entitled Usdx:
      await th.withdrawFromSPAndClaim(contracts, dec(15000, 18), { from: alice });

      const expectedUsdxinSPAfter = UsdxinSPBefore.sub(compoundedUsdxDeposit_A);

      const UsdxinSPAfter = await stabilityPool.getTotalUsdxDeposits();
      assert.isAtMost(
        th.getDifference(expectedUsdxinSPAfter, UsdxinSPAfter),
        100000,
      );
    });
    ...
    it("withdrawFromSP(): Subsequent deposit and withdrawal attempt from same account, with no intermediate liquidations, withdraws zero ETH", async () => {
      // --- SETUP ---
      // Whale deposits 1850 Usdx in StabilityPool
      await openTrove({
        extraUsdxAmount: toBN(dec(1000000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: whale },
    ...
    it("withdrawFromSP(): it correctly updates the user's Usdx and ETH snapshots of entitled reward per unit staked", async () => {
      // --- SETUP ---
      // Whale deposits 185000 Usdx in StabilityPool
      await openTrove({
        extraUsdxAmount: toBN(dec(1000000, 18)),
    ...
    it("withdrawFromSP(): increases depositor's Usdx token balance by the expected amount", async () => {
      // Whale opens trove
      await openTrove({
      ...
      const aliceBalBefore = await usdxToken.balanceOf(alice);
      const bobBalBefore = await usdxToken.balanceOf(bob);

      /* From an offset of 10000 Usdx, each depositor receives
         UsdxLoss = 1666.6666666666666666 Usdx

         and thus with a deposit of 10000 Usdx, each should withdraw 8333.3333333333333333 Usdx (in practice, slightly less due to rounding error)
      */

      // Price bounces back to $200 per ETH
      await priceFeed.setPrice(dec(200, 18));

      // Bob issues a further 5000 Usdx from his trove
      await borrowerOperations.withdrawUsdx(th.addressToTroveId(bob), dec(5000, 18), th.MAX_UINT256, { from: bob });

      // Expect Alice's Usdx balance increase be very close to 8333.3333333333333333 Usdx
      await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: alice });
      const aliceBalance = await usdxToken.balanceOf(alice);
      ...
      // expect Bob's Usdx balance increase to be very close to  13333.33333333333333333 Usdx
      await th.withdrawFromSPAndClaim(contracts, dec(10000, 18), { from: bob });
      const bobBalance = await usdxToken.balanceOf(bob);
      assert.isAtMost(
        th.getDifference(
    ...
    it("withdrawFromSP(): doesn't impact other users Stability deposits or ETH gains", async () => {
      ...
      await openTrove({
        extraUsdxAmount: toBN(dec(30000, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: carol },
      });
    ...
    it("withdrawFromSP(): withdrawing 0 Usdx doesn't alter the caller's deposit or the total Usdx in the Stability Pool", async () => {
      // --- SETUP ---
      await openTrove({
        extraUsdxAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // Bob withdraws 0 Usdx from the Stability Pool
      await th.withdrawFromSPAndClaim(contracts, 0, { from: bob });

      // check Bob's deposit and total Usdx in Stability Pool has not changed
      const bob_Deposit_After = (
        await stabilityPool.getCompoundedUsdxDeposit(bob)
      ).toString();
      const UsdxinSP_After = (
        await stabilityPool.getTotalUsdxDeposits()
      ).toString();

      assert.equal(bob_Deposit_Before, bob_Deposit_After);
      assert.equal(UsdxinSP_Before, UsdxinSP_After);
    });
    ...
    it("withdrawFromSP(): Request to withdraw > caller's deposit only withdraws the caller's compounded deposit", async () => {
      ...
      const alice_Usdx_Balance_Before = await usdxToken.balanceOf(alice);
      const bob_Usdx_Balance_Before = await usdxToken.balanceOf(bob);

      const alice_Deposit_Before = await stabilityPool.getCompoundedUsdxDeposit(
        alice,
      );
      ...
      const UsdxinSP_Before = await stabilityPool.getTotalUsdxDeposits();
      ...
      // Check Bob's Usdx balance has risen by only the value of his compounded deposit
      const bob_expectedUsdxBalance = bob_Usdx_Balance_Before
        .add(bob_Deposit_Before)
        .toString();
      const bob_Usdx_Balance_After = (
        await usdxToken.balanceOf(bob)
      ).toString();
      assert.equal(bob_Usdx_Balance_After, bob_expectedUsdxBalance);

      // Alice attempts to withdraws 2309842309.000000000000000000 Usdx from the Stability Pool
      await th.withdrawFromSPAndClaim(contracts, "2309842309000000000000000000", {
        from: alice,
      });

      // Check Alice's Usdx balance has risen by only the value of her compounded deposit
      const alice_expectedUsdxBalance = alice_Usdx_Balance_Before
        .add(alice_Deposit_Before)
        .toString();
      const alice_Usdx_Balance_After = (
        await usdxToken.balanceOf(alice)
      ).toString();
      assert.equal(alice_Usdx_Balance_After, alice_expectedUsdxBalance);

      // Check Usdx in Stability Pool has been reduced by only Alice's compounded deposit and Bob's compounded deposit
      const expectedUsdxinSP = UsdxinSP_Before.sub(alice_Deposit_Before)
        .sub(bob_Deposit_Before)
        .toString();
      const UsdxinSP_After = (
        await stabilityPool.getTotalUsdxDeposits()
      ).toString();
      assert.equal(UsdxinSP_After, expectedUsdxinSP);
    });

    it("withdrawFromSP(): Request to withdraw 2^256-1 Usdx only withdraws the caller's compounded deposit", async () => {
      // --- SETUP ---
      await openTrove({
        extraUsdxAmount: toBN(dec(10000, 18)),
        ICR: toBN(dec(10, 18)),
      ...
      // Bob attempts to withdraws 2^256 - 1 Usdx from the Stability Pool
      ...
      // Check Usdx in Stability Pool has been reduced by only  Bob's compounded deposit
      const expectedUsdxinSP = UsdxinSP_Before.sub(bob_Deposit_Before).toString();
      const UsdxinSP_After = (
        await stabilityPool.getTotalUsdxDeposits()
      ).toString();
      assert.equal(UsdxinSP_After, expectedUsdxinSP);
    ...
    it("withdrawFromSP(): caller can withdraw full deposit and ETH gain while below CT", async () => {
      ...
      // A, B, C provides 10000, 5000, 3000 Usdx to SP
      const A_GAS_Used = th.gasUsed(
        await th.provideToSPAndClaim(contracts, dec(10000, 18), {
          from: alice,
          gasPrice: GAS_PRICE,
      ...
      const alice_Usdx_Balance_Before = await usdxToken.balanceOf(alice);
      const bob_Usdx_Balance_Before = await usdxToken.balanceOf(bob);
      const carol_Usdx_Balance_Before = await usdxToken.balanceOf(carol);
      ...
      const carol_Deposit_Before = await stabilityPool.getCompoundedUsdxDeposit(
        carol,
      );
      ...
      // Check Usdx balances of A, B, C have risen by the value of their compounded deposits, respectively
      const alice_expectedUsdxBalance = alice_Usdx_Balance_Before
        .add(alice_Deposit_Before)
        .toString();
      ...
      const bob_expectedUsdxBalance = bob_Usdx_Balance_Before
        .add(bob_Deposit_Before)
        .toString();
      const carol_expectedUsdxBalance = carol_Usdx_Balance_Before
      ...
      const carol_Usdx_Balance_After = (
        await usdxToken.balanceOf(carol)
      ).toString();

      assert.equal(alice_Usdx_Balance_After, alice_expectedUsdxBalance);
      assert.equal(bob_Usdx_Balance_After, bob_expectedUsdxBalance);
      assert.equal(carol_Usdx_Balance_After, carol_expectedUsdxBalance);
      ...
      // Check Usdx in Stability Pool has been reduced by A, B and C's compounded deposit
      const expectedUsdxinSP = UsdxinSP_Before.sub(alice_Deposit_Before)
        .sub(bob_Deposit_Before)
    ...
    it("getDepositorCollGain(): depositor does not earn further ETH gains from liquidations while their compounded deposit == 0: ", async () => {
      ...
      // A, B, provide 10000, 5000 Usdx to SP
      await th.provideToSPAndClaim(contracts, dec(10000, 18), {
        from: alice,
      });
      ...
      const UsdxinSP = (await stabilityPool.getTotalUsdxDeposits()).toString();
      assert.equal(UsdxinSP, "0");
      ...
      // Whale deposits 10000 Usdx to Stability Pool
    ...
    it("withdrawFromSP(), full withdrawal: zero's depositor's snapshots", async () => {
      ...
      await usdxToken.transfer(B, dec(20000, 18), { from: whale });
      ...
      await openTrove({
        extraUsdxAmount: toBN(dec(40000, 18)),
        ICR: toBN(dec(10, 18)),
        extraParams: { from: D },
      });
    ...
    it("withdrawFromSP(), reverts when initial deposit value is 0", async () => {
      ...
      await openTrove({
        extraUsdxAmount: toBN(dec(10100, 18)),
        ICR: toBN(dec(2, 18)),
        extraParams: { from: A },
      });
      await th.provideToSPAndClaim(contracts, dec(10000, 18), { from: A });
