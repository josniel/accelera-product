...
contract("Deployment script - Sets correct contract addresses dependencies after deployment", async (accounts) => {
  ...
  before(async () => {
    ...
    usdxToken = coreContracts.usdxToken;
  ...
  it("Sets the correct UsdxToken address in TroveManager", async () => {
    const usdxTokenAddress = usdxToken.address;

    const recordedClvTokenAddress = await troveManager.getUsdxToken();

    assert.equal(usdxTokenAddress, recordedClvTokenAddress);
  });
  ...
  it("Sets the correct UsdxToken address in StabilityPool", async () => {
    const usdxTokenAddress = usdxToken.address;

    const recordedClvTokenAddress = await stabilityPool.usdxToken();
