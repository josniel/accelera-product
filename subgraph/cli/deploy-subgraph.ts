...
export async function main() {
  ...
  if (isLocal) {
    await updateNetworksWithLocalUsdxToken();
  }
...
async function updateNetworksWithLocalUsdxToken() {
  const networksJson = JSON.parse(await fs.readFile(NETWORKS_JSON_PATH, "utf8"));
  const latestDeploymentContext = getLatestDeploymentContext();

  const deployedAddress = latestDeploymentContext?.protocolContracts.UsdxToken;
  if (!deployedAddress || (networksJson.local.UsdxToken.address === deployedAddress)) {
    return;
  }

  const answer = await question(
    `\nNew UsdxToken detected (${deployedAddress}) for local network. Update networks.json? [Y/n] `,
  );
  ...
  networksJson.local.UsdxToken.address = deployedAddress;
  await fs.writeFile(NETWORKS_JSON_PATH, JSON.stringify(networksJson, null, 2));

  console.log("");
  console.log("networks.json updated with local UsdxToken:", deployedAddress);
}
