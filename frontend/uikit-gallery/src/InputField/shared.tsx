...
export function InputFieldFixture({
  ...
  const value_ = match(fixture)
    ...
    .with("borrow", () => (
      (focused || !parsedValue) ? value : `${dn.format(parsedValue)} USDX`
    ))
    .with("slider", () => (
...
function Token({ name }: { name: "ETH" | "USDX" }) {
  return (
    <Action
      icon={match(name)
        .with("ETH", () => <IconEth />)
