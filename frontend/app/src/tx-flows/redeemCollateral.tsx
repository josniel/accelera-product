...
export const redeemCollateral: FlowDeclaration<RedeemCollateralRequest> = {
  ...
  Details(ctx) {
    ...
    const usdxChange = estimatedGains.data?.find(({ symbol }) => symbol === "USDX")?.change;
    const collChanges = estimatedGains.data?.filter(({ symbol }) => symbol !== "USDX");
    return (
      <>
        ...
        <TransactionDetailsRow
          ...
          value={[
            ...
            />,
            <Fragment key="end">
              Estimated USDX that will be redeemed.
            </Fragment>,
          ]}
        />
        {branches.map(({ symbol }) => {
  ...
  steps: {
    approve: {
      ...
      async commit({ request, writeContract }) {
        ...
        const UsdxToken = getProtocolContract("UsdxToken");
  ...
  async getSteps(ctx) {
    ...
    // check for allowance
    const usdxAllowance = await ctx.readContract({
      ...getProtocolContract("UsdxToken"),
    ...
    if (dn.gt(ctx.request.amount, dnum18(usdxAllowance))) {
      steps.push("approve");
    }
...
export function useSimulatedBalancesChange({
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const usdxBalanceCall = {
        to: UsdxToken.address,
        abi: UsdxToken.abi,
        functionName: "balanceOf" as const,
        args: [account],
      } as const;
      ...
      const getBalancesFromSimulated = (position: number) => {
        return simulation.results
          ...
          .map((result, index) => {
            const symbol = index === 0 ? "USDX" : branches[index - 1]?.symbol;
            return {
              symbol,
