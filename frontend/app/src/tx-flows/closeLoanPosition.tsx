...
export const closeLoanPosition: FlowDeclaration<CloseLoanPositionRequest> = {
          ...
          <TransactionDetailsRow
            ...
            value={[
              <Amount
                key="start"
                value={amountToRepay}
                suffix={` ${repayWithCollateral ? collateral.symbol : "USDX"}`}
              />,
            ]}
          />
        )}
  ...
  steps: {
    ...
    // Close a loan position, repaying with USDX or with the collateral
    closeLoanPosition: {
      name: () => "Close loan",
  ...
  async getSteps(ctx) {
    ...
    const [{ entireDebt }, usdxAllowance] = await readContracts(ctx.wagmiConfig, {
      contracts: [{
        ...branch.contracts.TroveManager,
        functionName: "getLatestTroveData",
        args: [BigInt(loan.troveId)],
    ...
    const isUsdxApproved = ctx.request.repayWithCollateral || (
      entireDebt <= usdxAllowance
    );

    const steps: string[] = [];

    if (!isUsdxApproved) {
      steps.push("approveUsdx");
    }
