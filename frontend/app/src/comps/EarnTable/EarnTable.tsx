...
type EarnTableProps<Cols extends readonly ReactNode[]> = {
  // columns: Cols;
  // icon: ReactNode;
  loading?: ReactNode;
...
interface PositionEarn {
  collateral: string;
  avgRate: string;
  maxLTV: string;
}

const positions: PositionEarn[] = [
  {
    collateral: "ETH",
    avgRate: "5.2%",
    maxLTV: "75%",
...
export function EarnTable<Cols extends readonly ReactNode[]>({
  // columns,
  // icon,
  loading,
  // placeholder,
  ...
  const TableHeaderCells: TableHeaderCell[] = [
    { text: "Pool", className: css({ w: "15%" }) },
    { text: "Current APR", className: css({ w: "30%", textAlign: "center" }), sortBy: "balance" },
    { text: "Pool Size", className: css({ w: "25%", textAlign: "center" }), sortBy: "points" },
  ...
  const [sortBy, setSortBy] = useState<keyof PositionEarn | null>(null);
  //
  ...
  function poolsSort(
    a: PositionEarn,
    b: PositionEarn,
    defaultSort: (a: PositionEarn, b: PositionEarn) => number,
  ): number {
    return defaultSort(a, b);
  }

  function toggleSort(sortBy: string | null, sort: SortTypes | null): void {
    setSortBy(sortBy as keyof PositionEarn);
    setSort(sort);
  }
  return (
    <section
        ...
        >
          <TableBody>
            {loading
              ...
              ? <div></div>
              : (
                getBranches().map(({ symbol }, index) => <EarnRow index={index} key={index} symbol={symbol} />)
              )}
          </TableBody>
        </div>
      </div>
