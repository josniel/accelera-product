...
export async function getLeverUpTroveParams(
  ...
  const expectedUsdxAmount = flashLoanAmount * price / DECIMAL_PRECISION;
  const maxNetDebtIncrease = expectedUsdxAmount * 105n / 100n; // 5% slippage

  return {
    flashLoanAmount,
    effectiveUsdxAmount: maxNetDebtIncrease,
  };
}
...
export async function getLeverDownTroveParams(
  ...
  const expectedUsdxAmount = flashLoanAmount * price / DECIMAL_PRECISION;
  const minUsdxDebt = expectedUsdxAmount * 95n / 100n; // 5% slippage
...
export async function getOpenLeveragedTroveParams(
  ...
  const expectedUsdxAmount = flashLoanAmount * price / DECIMAL_PRECISION;
  const maxNetDebt = expectedUsdxAmount * 105n / 100n; // 5% slippage

  return {
    effectiveUsdxAmount: maxNetDebt,
    expectedUsdxAmount,
    flashLoanAmount,
    maxNetDebt,
    price,
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const [_, slippage] = await readContract(wagmiConfig, {
        ...
        functionName: "getCollFromUsdx",
        args: [
          params.expectedUsdxAmount,
