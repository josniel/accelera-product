...
function LoanPositionsTable() {
  ...
  const usdxBalance = dnum18(legacyPositions.data?.usdxBalance ?? 0n);
  return (
    <HomeTable
      ...
      rows={troves.map((trove) => {
        ...
        const hasEnoughUsdx = dn.lte(debt, usdxBalance);
        return (
          <tr key={trove.collToken.symbol}>
            ...
            <td>
              ...
              >
                {!hasEnoughUsdx && (
                  <InfoTooltip level="warning" heading="Insufficient Legacy USDX">
                    Your current Legacy USDX balance ({fmtnum(
                      usdxBalance,
                      { digits: 2 },
                    )} USDX) is too low to close this loan. You need at least {fmtnum(debt, { digits: 2 })}{" "}
                    USDX to close this loan.
                  </InfoTooltip>
                )}
                <Button
                  disabled={!hasEnoughUsdx}
                  mode="primary"
                  size="sm"
                  label="Close loan"
                  onClick={() => {
...
function StakingPositionsTable() {
  ...
  return (
    <HomeTable
      ...
      // icon={<IconStake />}
      columns={["Staked ACCEL", null] as const}
                ...
                onClick={() => {
                  if (stakeDeposit) {
                    txFlow.start({
                      ...
                      backLink: ["/legacy", "Back to legacy positions"],
                      successLink: ["/legacy", "Go to legacy positions"],
                      successMessage: "You have successfully unstaked your ACCEL.",
                      accelAmount: stakeDeposit,
                    });
                  }
                }}
              />
...
function RedeemSection() {
  ...
  const txFlow = useTransactionFlow();
  const legacyPositions = useLegacyPositions(account.address ?? null);
  const usdxBalance = legacyPositions.data ? dnum18(legacyPositions.data.usdxBalance) : null;
  ...
  const allowSubmit = account.isConnected
    ...
    && maxFee.parsed
    && usdxBalance
    && dn.gte(usdxBalance, amount.parsed);

  return (
    <HomeTable
      title="Legacy Redemption"
      subtitle="Redeem your Legacy USDX for ETH and LSTs."
      // icon={<IconEarn />}
      columns={[]}
      rows={[
        <tr
          key="redeem-legacy-usdx"
          className={css({
            "& th, & td": {
              fontWeight: "initial",
              whiteSpace: "initial",
                ...
                field={
                  <InputField
                    ...
                    drawer={amount.isFocused
                      ...
                      : usdxBalance
                          && amount.parsed
                          && dn.gt(amount.parsed, usdxBalance)
                      ? {
                        mode: "error",
                        message: `Insufficient USDX balance. You have ${fmtnum(usdxBalance)} USDX.`,
                      }
                      : null}
                    label="Redeeming"
                    placeholder="0.00"
                    secondary={{
                      start: `$${
                        ...
                        usdxBalance && dn.gt(usdxBalance, 0) && (
                          <TextButton
                            label={`Max ${fmtnum(usdxBalance)} Legacy USDX`}
                            onClick={() => {
                              amount.setValue(dn.toString(usdxBalance));
                            }}
                          />
                        )
                      ),
...
function TokenAmount({
  ...
  digits?: number | null;
  symbol: TokenSymbol | "LEGACY_USDX";
  value?: Dnum | null;
}) {
  const token = TOKENS_BY_SYMBOL[symbol === "LEGACY_USDX" ? "USDX" : symbol];
  return (
    <div
      title={fmtnum(value, {
        digits: digits ?? undefined,
        suffix: " " + (symbol === "LEGACY_USDX" ? "Legacy USDX" : token.name),
      })}
      className={css({
        display: "flex",
        alignItems: "center",
    ...
    >
      ...
      <div
        ...
        style={{
          opacity: symbol === "LEGACY_USDX" ? 0.5 : 1,
        }}
      >
        <TokenIcon
          size="mini"
          symbol={symbol === "LEGACY_USDX" ? "USDX" : symbol}
        />
      </div>
    </div>
  );
