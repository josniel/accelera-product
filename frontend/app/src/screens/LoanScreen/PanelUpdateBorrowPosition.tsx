...
export function PanelUpdateBorrowPosition({
  ...
  const usdxPriceUsd = usePrice("USDX") ?? dnum18(0);
  ...
  const usdxMax = debtMode === "remove" && usdxBalance.data
    ? dnumMin(
      usdxBalance.data,
      loan.borrowed,
    )
  ...
  if (!collPrice.data || !usdxPriceUsd.data) {
    return null;
  }

  const debtChangeUsd = debtChange.parsed && dn.mul(debtChange.parsed, usdxPriceUsd.data);
  ...
  const insufficientUsdx = debtMode === "remove"
    && debtChange.parsed
    && !debtChange.isEmpty
    && usdxBalance.data
    && dn.gt(debtChange.parsed, usdxBalance.data);
  ...
  const allowSubmit = account.isConnected
    ...
    // the account must have enough USDX
    && !insufficientUsdx
          ...
          field={
            <InputField
              ...
              drawer={!debtChange.isFocused && isBelowMinDebt
                ? { mode: "error", message: `You must borrow at least ${fmtnum(MIN_DEBT, 2)} USDX.` }
                : insufficientUsdx
                ? { mode: "error", message: "Insufficient USDX balance." }
                : null}
              label={{
              ...
              secondary={{
                start: <Amount value={debtChangeUsd ?? 0} suffix="$" />,
                  ...
                  usdxMax && (
                    <TextButton
                      label={`Max ${fmtnum(usdxMax)} USDX`}
                      onClick={() => {
                        debtChange.setValue(dn.toString(usdxMax));
                      }}
                    />
                  )
                ),
                ...
                value={
                  <HFlex alignItems="center" gap={8}>
                    ...
                    <InfoTooltip heading="Debt update">
                      <div>
                        Before: <Amount value={loanDetails.debt} suffix=" USDX" />
                      </div>
                      <div>
                        After: <Amount value={newLoanDetails.debt} suffix=" USDX" />
                      </div>
                    </InfoTooltip>
                  </HFlex>
                }
