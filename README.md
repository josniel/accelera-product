...
[![Coverage Status](https://coveralls.io/repos/github/liquity/usdx/badge.svg?branch=main&t=yZSfc8)](https://coveralls.io/github/liquity/usdx?branch=main)
...
- [USDX Redemptions](#usdx-redemptions)
...
- [Stability Pool implementation](#stability-pool-implementation)
  ...
  - [USDX Yield Gains](#usdx-yield-gains)
...
- **User-set interest rates.** When a borrower opens a Trove, they choose their own annual interest rate. They may change their annual interest rate at any point. Simple (non-compounding) interest accrues on their debt continuously, and gets compounded discretely every time the Trove is touched. Aggregate accrued Trove debt is periodically minted as USDX. 

- **Yield from interest paid to SP and LPs.** USDX yields from Trove interest are periodically paid out in a split to the Stability Pool (SP), and to a router which in turn routes its yield share to DEX LP incentives.  Yield paid to the SP from Trove interest on a given branch is always paid to the SP on that same branch.

- **Redemption routing.** Redemptions of USDX are routed by the CollateralRegistry. For a given redemption, the redemption volume that hits a given branch is proportional to its relative “unbackedness”. The primary goal of redemptions is to restore the USDX peg. A secondary purpose is to reduce the unbackedness of the most unbacked branches relatively more than the more backed branches. Unbackedness is defined as the delta between the total USDX debt of the branch, and the USDX in the branch’s SP.
...
- **Unredeemable Troves.** Redemptions now do not close Troves - they leave them open. Redemptions may now leave some Troves with a zero or very small USDX debt < MIN_DEBT. These Troves are tagged as `unredeemable` in order to eliminate a redemption griefing attack vector. They become redeemable again when the borrower brings them back above the `MIN_DEBT`.
...
- **More flexibility for SP reward claiming**.. SP depositors can now claim or stash their LST gains from liquidations, and either claim their USDX yield gains or add them to their deposit.

### What remains the same in v2 from v1?

- **Core redemption mechanism** - swaps 1 USDX for $1 worth of collateral, less the fee, in order to maintain a hard USDX price floor
...
- **Liquidation mechanisms**. Liquidated Troves are still offset against the USDX in the SP and redistribution to active Troves in the branch if/when the SP deposits are insufficient (though the liquidation penalty applied to the borrower is reduced).

- **Similar smart contract architecture**. At branch level the system architecture closely resembles that of v1 - the `TroveManager`, `BorrowerOperations` and `StabilityPool` contracts contain most system logic and direct the flows of USDX and collateral.

- **Stability Pool algorithm**. Same arithmetic and logic is used for tracking deposits, collateral gains and USDX yield gains over time as liquidations deplete the pool.
...
Liquity v2 is a collateralized debt platform. Users can lock up WETH and/or select LSTs, and issue stablecoin tokens (USDX) to their own Ethereum address. The individual collateralized debt positions are called Troves.


The stablecoin tokens are economically geared towards maintaining value of 1 USDX = $1 USD, due to the following properties:
...
2. The stablecoins are fully redeemable - users can always swap x USDX for $x worth of a mix of WETH and LSTs (minus fees), directly with the system.
...
Upon  opening a Trove by depositing a viable collateral ERC20, users may issue ("borrow") USDX tokens such that the collateralization ratio of their Trove remains above the minimum collateral ratio (MCR) for their collateral branch. For example, for an MCR of 110%, a user with $10000 worth of WETH in a Trove can issue up to 9090.90 USDX against it.


The USDX tokens are freely exchangeable - any Ethereum address can send or receive USDX tokens, whether it has an open Trove or not. The USDX tokens are burned upon repayment of a Trove's debt.
...
- There is a single `CollateralRegistry`, a single `UsdxToken`, and a set of core system contracts deployed for each collateral “branch”.
...
- `CollateralRegistry` - Records all LST collaterals and maps branch-level TroveManagers to LST collaterals. Calculates redemption fees and routes USDX redemptions to the TroveManagers of different branches in proportion to their “outside” debt.

- `USDXToken` - the stablecoin token contract, which implements the ERC20 fungible token as well as EIP-2612 permit functionality. The contract mints, burns and transfers USDX tokens.
...
- `BorrowerOperations`- contains the basic operations by which borrowers and managers interact with their Troves: Trove creation, collateral top-up / withdrawal, USDX issuance and repayment, and interest rate adjustments. BorrowerOperations functions call in to TroveManager, telling it to update Trove state where necessary. BorrowerOperations functions also call in to the various Pools, telling them to move collateral/USDX between Pools or between Pool <> user, where necessary, and it also tells the ActivePool to mint interest.

- `TroveManager` - contains functionality for liquidations and redemptions and calculating individual Trove interest. Also contains the recorded  state of each Trove - i.e. a record of the Trove’s collateral, debt and interest rate, etc. TroveManager does not hold value (i.e. collateral or USDX). TroveManager functions call in to the various Pools to tell them to move collateral or USDX between Pools, where necessary.
...
- `StabilityPool` - contains functionality for Stability Pool operations: making deposits, and withdrawing compounded deposits and accumulated collateral and USDX yield gains. Holds the USDX Stability Pool deposits, USDX yield gains and collateral gains from liquidations for all depositors on that branch.
...
- `ActivePool` - holds the branch collateral balance and records the total USDX debt of the active Troves in the branch. Mints aggregate interest in a split to the StabilityPool as well as a (to-be) yield router for DEX LP incentives (currently, to MockInterestRouter)

- `DefaultPool` - holds the total collateral balance and records the total USDX debt of the liquidated Troves that are pending redistribution to active Troves. If an active Trove has pending collateral and debt “rewards” in the DefaultPool, then they will be applied to the Trove when it next undergoes a borrower operation, a redemption, or a liquidation.
...
- `redeemCollateral(uint256 _usdxAmount, uint256 _maxIterations, uint256 _maxFeePercentage)`: redeems `_usdxAmount` of USDX tokens from the system in exchange for a mix of collaterals. Splits the USDX redemption according to the [redemption routing logic](#redemption-routing), redeems from a number of Troves in each collateral branch, burns `_usdxAmount` from the caller’s USDX balance, and transfers each redeemed collateral amount to the redeemer. Executes successfully if the caller has sufficient USDX to redeem. The number of Troves redeemed from per branch is capped by `_maxIterationsPerCollateral`. The borrower has to provide a `_maxFeePercentage` that he/she is willing to accept which mitigates fee slippage, i.e. when another redemption transaction is processed first and drives up the redemption fee.  Troves left with `debt < MIN_DEBT` are flagged as `unredeemable`.
...
- `withdrawColl(uint256 _troveId, uint256 _amount)`: withdraws _amount of collateral from the caller’s Trove. Executes only if the user has an active Trove, must result in the user’s Trove `ICR >= MCR` and must obey the adjustment [CCR constraints](#critical-collateral-ratio-ccr-restrictions).
 ...
 - `withdrawUsdx(uint256 _troveId, uint256 _amount, uint256 _maxUpfrontFee)`: adds _amount of USDX to the user’s Trove’s debt, mints USDX stablecoins to the user. Must result in `ICR >= MCR` and must obey the adjustment [CCR constraints](#critical-collateral-ratio-ccr-restrictions). An `upfrontFee` is charged, based on the system’s _average_ interest rate, the USDX debt drawn and the `UPFRONT_INTEREST_PERIOD`. The fee is added to the Trove’s debt. The borrower chooses a `_maxUpfrontFee` that he/she is willing to accept in case of a fee slippage, i.e. when the system’s average interest rate increases and in turn increases the fee they’d pay.	

 - `repayUsdx(uint256 _troveId, uint256 _amount)`: repay `_amount` of USDX to the caller’s Trove, canceling that amount of debt. Transfers the USDX from the caller to the system.

 - `closeTrove(uint256 _troveId)`: repays all debt in the user’s Trove, withdraws all their collateral to their address, and closes their Trove. Requires the borrower have a USDX balance sufficient to repay their Trove's debt. Burns the USDX from the user’s address.
...
- `adjustTrove(
        ...
        bool isDebtIncrease,
        uint256 _maxUpfrontFee
    )`:  enables a borrower to simultaneously change both their collateral and debt, subject to the resulting `ICR >= MCR` and the adjustment [CCR constraints](#critical-collateral-ratio-ccr-restrictions). If the adjustment incorporates a `debtIncrease`, then an `upfrontFee` is charged as per `withdrawUsdx`.
...
-  `setRemoveManager(uint256 _troveId, address _manager)`: sets a “Remove” manager for the caller’s chosen Trove, who has permission to remove collateral from and draw new USDX from their Trove.

- `setRemoveManagerWithReceiver(uint256 _troveId, address _manager, address _receiver)`: sets a “Remove” manager for the caller’s chosen Trove, who has permission to remove collateral from and draw new USDX from their Trove to the provided `_receiver` address.
...
- `urgentRedemption(uint256 _usdxAmount, uint256[] calldata _troveIds, uint256 _minCollateral)`: Executes successfully only when the collateral branch has already been shut down.  Redeems only from the branch it is called on. Redeems from Troves with a slight collateral bonus - that is, 1 USDX redeems for $1.01 worth of LST collateral.  Does not flag any redeemed-from Troves as `unredeemable`. Caller specifies the `_minCollateral` they want to receive.

### StabilityPool

- `provideToSP(uint256 _amount, bool _doClaim)`: deposit _amount of USDX to the Stability Pool. It transfers _amount of USDX from the caller’s address to the Pool, and tops up their USDX deposit by _amount. `doClaim` determines how the depositor’s existing collateral and USDX yield gains (if any exist) are treated: if true they’re transferred to the depositor’s address, otherwise the collateral is stashed (added to a balance tracker) and the USDX gain is added to their deposit.

- `withdrawFromSP(uint256 _amount, bool doClaim)`: withdraws _amount of USDX from the Stability Pool, up to the value of their remaining deposit. It increases their USDX balance by _amount. If the user makes a partial withdrawal, their remaining deposit will earn further liquidation and yield gains.  `doClaim` determines how the depositor’s existing collateral and USDX yield gains (if any exist) are treated: if true they’re transferred to the depositor’s address, otherwise the collateral is stashed (added to a balance tracker) and the USDX gain is added to their deposit.
...
When a Trove is opened, borrowers commit an amount of their chosen LST token as collateral, select their USDX debt, and select an interest rate in range `[INTEREST_RATE_MIN, INTEREST_RATE_MAX]`.
...
- The pending aggregate interest is minted by the ActivePool as fresh USDX. This is considered system “yield”.  A fixed part (72%, final value TBD) of it is immediately sent to the branch’s SP and split proportionally between depositors, and the remainder is sent to a router to be used as LP incentives on DEXes (determined by governance).

This is the only way USDX is ever minted as interest. Applying individual interest to a Trove updates its recorded debt, but interest is always minted in aggregate.


### Redemption evasion mitigation

In healthy system states (TCR > CCR) a borrower may adjust their Trove’s interest rate or debt at any time, as well as close their Trove. As such, a borrower may evade a redemption transaction by either frontrunning it with an interest rate adjustment, or closing and reopening their Trove. Both "hard" and "soft" frontrunning are viable: savvy borrowers may watch the mempool for redemption transactions, or simply watch the USDX peg, and take evasive action when it is below $1 and redemptions are likely imminent.
...
The upfront borrowing fee is equal to 7 days of average interest on the respective collateral branch. It is charged in USDX and is added to the Trove's debt.
...
```
  const USDXAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 USDX
...
Any USDX holder (whether or not they have an active Trove) may redeem their USDX directly with the system. Their USDX is exchanged for a mixture of collaterals at face value: redeeming 1 USDX token returns $1 worth of collaterals (minus a dynamic redemption fee), priced at their current market values according to their respective oracles. Redemptions have two purposes:


1. When USDX is trading at <$1 on the external market, arbitrageurs may redeem `$x` worth of USDX for `>$x` worth of collaterals, and instantly sell those collaterals to make a profit. This reduces the circulating supply of USDX which in turn should help restore the $1 USDX peg.
...
A given USDX redemption is split across branches according in proportion to the **outside debt** of that branch, i.e. (pseudocode):

`redeem_amount_i = redeem_amount * outside_debt_i / total_outside_debt`

Where `outside_debt_i` for branch i is given by `usdx_debt_i  - usdx_in_SP_i`.

That is, a redemption reduces the outside debt on each branch by the same percentage.

_Example: 2000 USDX is redeemed across 4 branches_
...
When USDX is redeemed for collaterals, the system cancels the USDX with debt from Troves, and the corresponding collateral is removed.
...
Upon each redemption of x USDX:

- `baseRate` is decayed based on time passed since the last fee event and incremented by an amount proportional to the fraction of the total USDX supply to be redeemed, i.e. `x/total_usdx_supply`
...
USDX depositors in the Stability Pool on a given branch earn:

- USDX yield paid from interest minted on Troves on that branch
- Collateral penalty gains from liquidated Troves on that branch


Depositors deposit USDX to the SP via `provideToSP` and withdraw it with `withdrawFromSP`. 

Their accumulated collateral gains and USDX yield gains are calculated every time they touch their deposit - i.e. at top up or withdrawal. If the depositor chooses to withdraw gains (via the `doClaim` bool param), all their collateral and USDX yield gain are sent to their address.

Otherwise, their collateral gain is stashed in a tracked balance and their USDX yield gain is added to their deposit.
...
USDX yield gains for Stability Pool depositors are triggered whenever the ActivePool mints aggregate system interest - that is, upon most system operations. The USDX yield gain is minted to the Stability Pool and a USDX gain for all depositors is triggered in proportion to their deposit size.

To efficiently and accurately track USDX yield gains for depositors as deposits decrease over time from liquidations, we re-use the above product-sum algorithm for deposit and gains.


The same product `P` is used, and a sum `B` is used to track USDX yield gains. Each deposit gets a new snapshot of `B` when it is updated.
...
1. Offset under-collateralized Troves’ debt against the branch’s SP containing USDX tokens, and award the seized collateral to the SP depositors
...
Liquity primarily uses the USDX tokens in its Stability Pool to absorb the under-collateralized debt, i.e. to repay the liquidated borrower's liability.

Any user may deposit USDX tokens to any Stability Pool. This allows them to earn the collateral from Troves liquidated on the branch. When a liquidation occurs, the liquidated debt is cancelled with the same amount of USDX in the Pool (which is burned as a result), and the seized collateral is proportionally distributed to depositors.
...
If the liquidated debt is higher than the amount of USDX in the Stability Pool, the system applies both steps 1) and then 2): that is, it cancels as much debt as possible with the USDX in the Stability Pool, and then redistributes the remaining liquidated collateral and debt across all active Troves in the branch.


## Minimum 1 USDX token in the SP

Once the SP has reached at least 1 USDX in size, the system enforces that at least 1 USDX remain in it - e.g. that `totalUsdxDeposits >=1e18`.  Specifically:

- SP withdrawals check that the resulting `totalUsdxDeposits >= 1e18`. This means that if a depositor's withdrawal would result in `totalUsdxDeposits < 1e18`, then it will revert. The depositor can just withdraw slightly less, in order to leave 1e18 `totalUsdxDeposits` in the SP. At worst, they forego 1 USDX. This situation should be rare, and can be remedied by anyone else depositing 1 USDX to the SP, allowing them to withdraw.

- When `totalUsdxDeposits >= 1e18` USDX, offsets leave `1e18` USDX in the SP. In `batchLiquidateTroves` we track `usdxInSPForOffsets`, and it is `1e18` less than the total SP. So all liquidation logic remains the same, and only the offset boundary has shifted.

- In the special case of `totalUsdxDeposits < 1e18` (e.g. before at least 1 USDX has been deposited), liquidations do a pure redistribution and do not perform an offset against the SP. This means that if somehow a liquidation occurs when `totalUsdxDeposits < 1e18`, then it will still succeed and will not deplete the SP.

## Liquidation logic

| Condition                         | Description                                                                                                                                                                                                                                                                                                                  |
|-----------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ICR < MCR & SP.USDX - 1 >= Trove.debt | USDX in the StabilityPool equal to the Trove's debt is offset with the Trove's debt. The Trove's seized collateral is shared between depositors.                                                                                                                                                                                    |
| ICR < MCR & SP.USDX - 1 < Trove.debt  | The offsettable USDX in the SP is offset with an equal amount of debt from the Trove. A portion of the Trove's collateral corresponding to the offset debt is shared between depositors. The remaining debt and seized collateral (minus collateral gas compensation) is redistributed to active Troves.  |
| ICR < MCR & SP.USDX = 0           | Redistribute all debt and seized collateral (minus collateral gas compensation) to active Troves.                                                                                                                                                                                                                                    |
...
Remove Managers may withdraw collateral or draw new USDX debt.

- Only the designated Remove manager, if any, and the Trove owner, are allowed 
- A receiver address may be chosen which can be different from the Remove Manager and Trove owner. The receiver receives the collateral and USDX drawn by the Remove Manager.
...
Under extreme conditions such as collateral price collapse or oracle failure, a collateral branch may be shut down in order to preserve wider system health and the stability of the USDX token.
...
During shutdown the redemption logic is modified to incentivize swift reduction of the branch’s debt, and even do so when USDX is trading at peg ($1 USD). Redemptions in shutdown are known as “urgent” redemptions.
...
- Pay a slight collateral bonus of 1% to the redeemer. That is, in exchange for every 1 USDX redeemed, the redeemer receives $1.01 worth of the LST collateral.
...
- Frontrun with redemption transaction for $x worth of USDX and receive `$x` - fee worth of collateral
...
The value extracted is excessive, i.e. above and beyond the arbitrage profit expected from USDX peg restoration. In fact, oracle frontrunning can even be performed when USDX is trading >= $1.
...
Using an out-of-date price obviously has undesirable consequences, but it’s the best that can be done in this extreme scenario. The impacts are addressed in [Known Issue 4](https://github.com/liquity/usdx/blob/main/README.md#4---oracle-failure-and-urgent-redemptions-with-the-frozen-last-good-price).
...
The impacts of downward market manipulation are bounded - it could result in excessive liquidations and branch shutdown, but should not affect other branches nor USDX stability.

However, upward market manipulation could be catastrophic as it would allow excessive USDX minting from Troves, which could cause a depeg.
...
`outside_debt_i = usdx_debt_i  - usdx_in_SP_i`.
...
This sequence - deposit to SPs on unwanted branches, then redeem from chosen branch(es) -  can be performed in one transaction and a flash loan could be used to obtain the USDX funds for deposits.
...
The manipulation does not change the fee the attacker pays (which is based purely on the `baseRate`, the redeemed USDX and the total USDX supply).
...
The redemption fee formula is path-dependent: that is, given some given prior system state, the fee incurred from redeeming one big chunk of USDX in a single transaction is greater than the total fee incurred by redeeming the same USDX amount in many smaller chunks with many transactions (assuming no other system state change in between chunks).
...
- Redemptions in Liquity v1 (with the same fee formula) have broadly functioned well, and proven effective in restoring the USDX peg.
...
|------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------|
| lastGoodPrice > market price | Urgent redemptions return too little LST collateral, and may be unprofitable even when USDX trades at $1 or below                                   |
...
- In the first case, the final result is that some uncleared USDX debt remains on the shut down branch, and the system carries this unbacked debt burden going forward.  This is an inherent risk of a multicollateral system anyway, which relies on the economic health of the LST assets it integrates.
...
- Borrowing (and selling) USDX with too little collateral provided
...
- `SUM(deposit_i)_over_all_depositors_i < totalUsdxDeposits`
...
On the other hand, upward price manipulation would result in excessive USDX minting, which is detrimental to the entire system health and USDX peg.
...
This could in the worst case lead to bank runs: a portion of the system debt can not be cleared, and hence a portion of the USDX supply can never be redeemed.

Even though the entire system may be overcollateralized in aggregate, this unredeemable portion of USDX is problematic: no user wants to be left holding unredeemable USDX and so they may dump USDX en masse (repay, redeem, sell).

This would likely cause the entire system to evaporate, and may also break the USDX peg. Even without a peg break, a bank run is still entirely possible and very undesirable.

**Potential solutions**

Various solutions have been fielded. Generally, any solution which appears to credibly and eventually clear the bad debt should have a calming effect on any bank run dynamic: when bad debt exists yet users believe the USDX peg will be maintained in the long-term, they are less likely to panic and repay/redeem/dump USDX.

1. **Redemption fees pay down bad debt**. When bad debt exists, direct normal redemption fees to clearing the bad debt. It works like this: when `x` USDX is redeemed, `x-fee` debt on healthy branches is cleared 1:1 for collateral, and `fee` is canceled with debt on the shut down branch. This would slowly pay down the debt over time. It also makes bank runs via redemption nicely self-limiting: large redemption volume -> fee spike -> pays down the bad debt more quickly. 

2. **Haircut for SP depositors**. If there is bad debt in the system, USDX from all SPs could be burned pro-rata to cancel it.  This socializes the loss across SP depositors.
...
5. **Governance can direct USDX interest to pay down bad debt**. USDX interest could be voted to be redirected to paying down the bad debt over time.  Although this would not directly clear the bad debt, economically, it should have the same impact  - since ultimately, it is the redeemability of _circulating_ USDX that determines the peg.  When an amount equal to the bad debt has been burned, then all circulating USDX is fully redeemable. See this example:

<img width="537" alt="image" src="https://github.com/user-attachments/assets/3045cba9-45a3-46b4-a5d0-58bed7f38a04">

This provides a credible way of eventually "filling the hole" created by bad debt (unlike other approaches such as the SP haircut, which depends on SP funds). No additional core system code nor additional governance features are required. Governance may simply propose to redirect USDX interest to a burn address. 

If there is remaining collateral in the shutdown branch (albeit perhaps at zero USD value) and there are liquidateable Troves, Governance could alternatively vote to direct fees to a permissionless contract that deposits the USDX to the SP of the shutdown branch and liquidates the Troves against those funds. The resulting collateral gains could, if they have non-zero value, be swapped on a DEX, e.g. for USDX which could be then directed to LP incentives. All deposits and swaps could be handled permissionlessly by this governance-deployed contract.
...
7. **Pro-rata redemptions at TCR < 100% (branch specific, not routed)**. Urgent redemptions are helpful for shrinking the debt of a shut down branch when it is at `TCR > 100%`. However, at `TCR < 100%`, urgent redemptions do not help clear the bad debt. They simply remove all collateral and push it into its final state faster (and in fact, make it slightly worse since they pay a slight collateral bonus).  At `TCR < 100%`, we could offer special pro-rata redemptions only on the shut down branch - e.g. at `TCR = 80%`, users may redeem 1 USDX for $0.80 worth of collateral. This would (in principle) allow someone to completely clear the bad debt via redemption. At first glance it seems unprofitable, but if the redeemer has reason to believe the collateral is underpriced and the price may rebound at some point in future, they may believe it to be profitable to redeem pro-rata.

**Conclusion**

Ultimately, no measures have been implemented in the protocol directly, so the protocol may end up with some bad debt in the case of a branch shut down.  Here there is a theoretical possibility that the USDX supply may be reduced by either users accidentally burning USDX, or that borrower's interest could be directed by governance to burn USDX, which would restore its backing over time.
...
The StabilityPool doesn't automatically compound Usdx yield gains to depositors. All deposits are added to `totalUsdxDeposits` - but claimable yields are not part of `totalUsdxDeposits`.

When a depositor claims their USDX yield they may choose to add it directly to their deposit - which does increase `totalUsdxDeposits`.
...
This simply means that frequently claiming and adding USDX yield gains to one's deposit is the preferred strategy.
...
As such, the calculation in `getCompoundedDeposit` overflows for a a deposit of 1e24 USDX, since `initialDeposit * P` in `getCompoundedUsdxDeposit` is of magnitude `1e24 * 1e18 * 1e36 = 1e78`, i.e. larger than the max uint256:

https://github.com/liquity/usdx/blob/b2ff26c5f09e72eaa5ad7eb24210aded4e80f00e/contracts/src/StabilityPool.sol#L530

The same bound can be found for the sums `G` and `B`, e.g. where `B` is updated in: https://github.com/liquity/usdx/blob/b2ff26c5f09e72eaa5ad7eb24210aded4e80f00e/contracts/src/StabilityPool.sol#L372

An upper bound of ~1e23 USDX before overflow is deemed acceptable - the USD value of total global wealth is many orders of magnitudes lower. However, forks should consider overflow calculations if they further increase precision or expect a much higher supply of their minted asset.
...
- TroveManager L1281 https://github.com/liquity/usdx/blob/9b42a46d3f7ee9382be9558acf013ea8d49dbe1b/contracts/src/TroveManager.sol#L1281 L1537 https://github.com/liquity/usdx/blob/9b42a46d3f7ee9382be9558acf013ea8d49dbe1b/contracts/src/TroveManager.sol#L1537 
L1593 https://github.com/liquity/usdx/blob/9b42a46d3f7ee9382be9558acf013ea8d49dbe1b/contracts/src/TroveManager.sol#L1593
L1734 https://github.com/liquity/usdx/blob/9b42a46d3f7ee9382be9558acf013ea8d49dbe1b/contracts/src/TroveManager.sol#L1734 These `assert` check that certain debt quantities on a live Trove are not 0, which always holds true. 

- MainnetPriceFeedBase L52: https://github.com/liquity/usdx/blob/6a793b24b294f6f1581746e021bcd6845fc3dc06/contracts/src/PriceFeeds/MainnetPriceFeedBase.sol#L52  This is irrelevant now that contracts have been deployed.
...
Though long-term depositors may miss out on some liquidation gains in the case of large JIT deposits, their incentive to remain in the SP long-term comes from the USDX yield generated by interest paid on Troves.

#### Reclaiming borrow fees via JIT deposits

When new debt is drawn on a Trove, an upfront fee is charged.  Part of this fee is sent as yield rewards to the SP. A borrower may reclaim some of their borrow fees by sequentially drawing their debt in chunks, and depositing their issued USDX into the SP. As such, they'll then earn back a portion of their borrow fee paid on the next chunk of debt. They can repeat this process until they reach their target debt.
...
A collection of issues identified in security audits which nevertheless do not require a fix [can be found here](https://github.com/liquity/usdx/issues?q=label%3Awontfix+).
